...
uv run scrolls archive show <id> [--all]  # recover one item's latest archived prior capture as a re-importable JSONL line; --all emits the item's whole archived history, newest first (H285); restore via `archive show <id> | import items /dev/stdin --accept-incoming`, to stdout
uv run scrolls archive restore <id> [--hash H | --at ISO] [--dry-run]  # restore a *specific* archived prior in place via the accept-incoming adoption (no new write path): --hash picks the prior with that content hash, --at the newest archived at/before the boundary, default the latest; the displaced copy is itself archived (reversible), idempotent on the held copy (H286, ADR 0106), as JSON
uv run scrolls archive diff <id> [--hash H | --at ISO]  # decide before you restore: compare the held copy against a selected archived prior (same --hash/--at selector, default the latest) — held↔prior content hash + fidelity tier, the model-complete changed fields, and would_restore (the H286 idempotency predicted before the write); a read, no write (H288, ADR 0106), as JSON
uv run scrolls archive prune (--before ISO | --keep N) [--apply]  # bound the append-only recovery store by a retention policy: --before drops priors archived before a boundary, --keep N keeps the most recent N per item (N>=1, so `archive show` survives); report-only until --apply, never touches a held row (H282, ADR 0106), as JSON
...
uv run scrolls maintain       # one scheduled custody pass: recheck the stale set (not seen since last run) → regenerate views → audit → custody delta (+ one-line custody headline + suggested repair commands), as JSON
...
`--unverified` only the never-checked ones (the bucket doctor flags),
`--stale-before <ISO>` only those not re-checked since a boundary (the act-side
of the `--since` family `scrolls history <id>` and `scrolls export events`
already window by), and `--drift <posture>` only those at a given drift posture
...
not just its current posture. Every browse/inspect row and node (`scrolls list`/
`search`/`show`, plus `scrolls related` hits and `scrolls graph` nodes + their
MCP twins) carries the per-item custody picture inline: the `fidelity` tier, the
`drift` posture, and `last_checked` — *when* that posture was taken, or `null`
when never re-checked — so an agent reading a row, following a related edge, or
traversing the graph sees how much is held, whether the source moved, and as of
when, then picks a `verify --stale-before <ISO>` boundary straight from it
(H84/H86). That custody record is **portable**: a
