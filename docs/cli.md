...
flagged source's own tally, a one-line `reason` naming the loss (e.g. `"2 drifted, 1
rotted"`), and the exact **`command`** to re-check it — `scrolls verify --source
<source>` — so the report names not just *which* source is weakest but the act that
re-verifies it (a recheck, not a `doctor --fix` repair, so it rides `attention`
beside the source it names, never `suggested`). Honest **`null`** when nothing
