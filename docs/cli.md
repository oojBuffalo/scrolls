...
`drift_change`/`coverage_change`/`stale_change`/`at_risk_change` (and the scalar
`score.change`) by reading only the window's *endpoints* (`compute_trend`), while
...
Like `coverage_change`/`stale_change` it is a **reported axis, never a `posture`
trigger** — it re-views the same `fidelity`/`drift` the score and `drift_change`
already move the posture on, so folding it in would double-count the loss. The
...
`coverage_change`, `stale_change`, and `at_risk_change` all null) and
