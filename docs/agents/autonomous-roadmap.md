...
pair) and **H364** (this run) have shipped. **H364** is the MCP holdings-immutability
contract guard: one `tests/test_mcp.py` test pins that the registered MCP tool surface
(`mcp_server._TOOLS`, asserted ≡ what `build_server` registers) is *exactly* an allow-list of
17 read + 7 custody-safe-write tools, **and** that no non-feed registered tool name carries a
capture-destroying verb (`delete`/`remove`/`drop`/`purge`/`prune`/`rm`/`overwrite`/`merge`,
token-based on snake_case so `transform`-style names never false-positive on `rm`) — so an
agent driving MCP can never delete or overwrite raw (the M2 tested-contract shape lifted to the
tool *registry*, custody-vision §2.4; H355's "the prune is the shell's explicit `scrolls rm`,
never an MCP act" premise made regression-proof). A sabotage `delete_scroll` fails *both* halves
(allow-list mismatch + verb backstop). H365–H368 + the new H374 import-axis cell remain un-started.
...
**H363** (the first pivot cell, `kb` recompile-determinism) and **H364 shipped this run** (the
MCP holdings-immutability contract guard, the second pivot cell); H365–H368 + the new H374
remain un-started.
...
— take the next one whose preconditions are met (H365 next):
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  two-pass whole-tree-hash no-op plus a cross-`PYTHONHASHSEED` subprocess pair) and **H364
  ...
  halves; suite **4190 passed**). Remaining, un-started: **H365** (`doctor --fix` convergence),
  **H366** (`context --budget` nesting), **H367** (`status`↔`doctor` scalars), **H368**
  ...
  the import-axis sibling). Prefer H365 next. Close a de-prioritized guard (H244–H249) only if
