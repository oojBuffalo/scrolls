...
`custody_snapshot` fails *only* the cross-seed while the per-tool shape tests stay green). **H384 is next**
(the `export events` JSONL determinism + round-trip guard); H385–H388 are queued behind it, and **H389**
(the MCP `get_maintenance_history` trend-read whole-payload determinism guard, the read sibling of
`run_maintenance` folding `read_log` + `compute_trend`) was appended this run to keep the buffer ≥6.
...
the `archive show` JSONL determinism twin **H385** is next, and H386–H390 are queued behind it.
...
— take the next one whose preconditions are met (H385 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H385 | **`scrolls archive show` JSONL determinism + restore round-trip guard — pins that the archive-recovery transport (the superseded prior capture the un-launderable integrity alarm preserves, ADR 0106/H278/H285) is a *reproducible artifact*: `scrolls archive show <id>` of the same unchanged archived snapshot twice yields **byte-identical** JSONL, AND a real `archive show <id>` → `import items /dev/stdin --accept-incoming` re-adopts the archived prior (the documented `archive show … | import items --accept-incoming` restore, line ~4605). The eighteenth **forward-hardening cell** — the *archive-recovery-transport* sibling of H379's `export items` determinism: both fold `dump_items_export`, but `archive show` folds it over `item_archive` rows (`latest_archived`/`archived_snapshots`, `ORDER BY id DESC`), a **distinct code path** from `list_items` no determinism test pins; deliberately *not* another content-identity guard. → cap 9.** `dump_items_export` is the shared backup root H379 pinned reproducible *over the held set*, but the recovery read serializes the *archived* priors, and an operator restoring from `archive show` on two machines must get byte-identical JSONL — a set-iteration leak in `archived_snapshots`' multi-prior fold (or an unsorted `item_archive` read) would pass the H278/H285 restore-outcome tests (which assert the *adopted* disposition, not the JSONL bytes) yet make two recovery reads disagree. Add a section to `tests/test_cli.py` (or `tests/test_roundtrip.py`): seed an item with ≥1 superseded prior, `archive show <id>` twice with no DB change, assert byte-identical JSONL; then pipe `archive show` through `import items --accept-incoming` into the same/fresh home and assert the prior is re-adopted (the round-trip restore), and (with `--all`, the multi-prior fold) assert the whole multi-line document is byte-stable across two reads. **Decisive choice:** compare the *whole JSONL document* (not one row), exercise the `--all` multi-prior fold (the order-sensitive path the single-prior default never hits), and include the restore round-trip; pair the cross-seed `PYTHONHASHSEED` face only if `archived_snapshots` folds a `set` (else the same-process two-read check suffices — it is an `ORDER BY id DESC` fold). **Sabotage check:** a counter appended to one shown prior row must fail the byte-identity. Test-only, no production change (`archive show` is already deterministic — ordered `item_archive` fold, no wall-clock). Precondition: `_cmd_archive_show`/`archived_snapshots`/`latest_archived` (H278/H285), `dump_items_export`, `scrolls import items --accept-incoming`, the `tests/test_cli.py` harness. | → cap 9 |
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  *only* the settle and a posture-`reasons` `set`-fold in `custody_snapshot` fails *only* the cross-seed
  while the per-tool shape tests stay green; suite **4213 passed**).
  Remaining, un-started: **H384** (the `export events` JSONL determinism + round-trip guard) — next — then
  **H385–H389** (the `archive show` determinism + restore round-trip, the `import archive` re-import
  idempotency guard, the MCP `get_link_graph` whole-payload determinism guard, the MCP `get_works`
  whole-payload determinism guard, and the MCP `get_maintenance_history(trend=True)` trend-read
  determinism guard appended this run to keep the buffer ≥6). Close a de-prioritized guard (H244–H249)
  only if nothing better is ready. The next once-per-24h full re-derivation is **due 2026-06-27** (the
  3-day plan boundary).
