...
| H426 | Native X bookmarks collection on-ramp (production, `src/scrolls/x_bookmarks.py` / `x_session.py` / `x_graphql.py`) — `scrolls sync x --bookmarks` pulls the collection over the browser's own session cookies against X's internal GraphQL endpoint (ADR 0108, superseding 0009; issue #6), reading Chrome/Brave/Arc/Edge/Vivaldi/Chromium/Firefox profiles or `SCROLLS_X_AUTH_TOKEN`/`SCROLLS_X_CT0`; ids stay `x:<tweetId>` so a pull dedupes against `add`, `saved_at` falls back to sync time with `provenance.saved_at_source` saying so, a rotated query id is never reported as an empty collection, and **no `x` fetch adapter exists** so `verify` fails honestly instead of returning a false clean verdict; verified live 2026-08-18 (479 bookmarks, 6 pages, clean end-of-collection stop, re-sync all skips) | cap 4 |
| H427 | Wikipedia reading-lists collection on-ramp (production, `src/scrolls/wikipedia_lists.py` / `browser_cookies.py`) — `scrolls sync wikipedia --reading-lists` pulls every article across the account's reading lists over the browser's own CentralAuth cookies (ADR 0109; issue #4), the API verified live via `action=paraminfo` before building as the issue required; **enumerate-only, not capture-at-pull** — an entry carries no article text, so items enter at stage `detected` and the ADR 0002 fetch adapter captures them, which also means `verify` reports drift here (what ADR 0108 conceded X cannot do); auth is CentralAuth's not the local wiki's (`centralauth_User`+`centralauth_Session`; a valid `enwikiSession` alone answers `notloggedin`), so the cookie machinery generalized into `browser_cookies.py` behind a `CookieSpec` with `x_session.py` unchanged on top; ids are minted by running the built URL through the same `detect_source` as `add`, `saved_at` is the entry's real `created` time, named lists become `tags` while the default list does not, and an article in two lists collapses to one item unioning tags (the 0030 rule — found only by the live run, where 566 items imported with exactly **one** tag surviving); non-Wikipedia projects are reported with their project named, never guessed; verified live 2026-08-18 (2 lists, 586 entries, 6 pages, 566 distinct articles, 0 failures, re-sync all skips, all 20 named-list tags kept, `doctor` 0 issues/posture sound) | cap 4 |
| H428 | Wikipedia article capture deepened to the link graph and figures (production, `src/scrolls/sources/wikipedia.py`) — the ADR 0002 adapter kept prose only, so every article pulled by the H427 reading-lists on-ramp held `links == []` and `media == []`; invisible for a single `add`, decisive for a 566-article library, since `graph.py` already resolves `links` into edges and capturing none left **no Wikipedia in the graph at all** (ADR 0110, amending 0002 — whose "one GET" no longer holds, as `generator=images` replaces the page set and needs a second, best-effort request); mainspace links become article URLs through the *same encoder* the collection uses so a captured link and a pulled entry converge on one item, external links follow as the citations, and backlinks are answered from the library rather than MediaWiki's thousands-of-mostly-unsaved `list=backlinks`; editorial chrome is filtered by narrow name patterns (six of ten files on *Dihydrocodeine* were icons) kept deliberately tight because a false drop loses content silently; `utm_*` analytics params are stripped from every `imageinfo` URL, rasters over 1600px are captured at the 1280px thumbnail with `original_url` recording what was passed over while smaller files keep their original (a vector's "thumbnail" is a larger, lossier raster), media `type` follows the file's MIME so an `.ogg` is not called an image, and `links` joins the scroll frontmatter beside `media`; verified live 2026-08-18 over the real 566-article library — 566 fetched/0 failures, 15 MB prose, 222,895 links, 3,815 graph edges across 546 connected articles (was 0), 5,416 figure refs of which 1,383 capped, every ref captured to disk (5.7 GB against 44 MB of scrolls, 24 files over 20 MB and one 2.6 GB `.webm` — a byte-size cap for non-image media is the deferred follow-up, since the 1600px rule caps rasters by pixel width and says nothing about video) | cap 4/9 |
| H429 | Bulk media capture paced against host rate limits (production, `src/scrolls/media.py`) — the first full Wikipedia figure run earned HTTP 429 (*"your bot is making too many requests"*) and reported **456 of 473 items failed**, a wrong reading since the files were there and the request rate was the problem; `capture_media` now paces downloads and waits out a 429 with exponential backoff honouring the host's own `Retry-After`, while a persistent limit fails saying it was *rate-limited and the file is still there* — because that wants a different response from the operator than a 404, which stays unretried since it will not become a 200 by asking again; a property of bulk capture generally rather than of Wikipedia, and the seam is injectable so tests never sleep; verified live by resuming the interrupted run to **5,416/5,416 refs captured, 0 failures** | cap 4/9 |
| H430 | Captured math rendered as KaTeX instead of MathML token soup (production, `src/scrolls/mathtext.py` + `render.py`) — MediaWiki's `explaintext` cannot render a `<math>` tag, so it emits the MathML fallback (one symbol per indented line) then the real TeX in a `{\displaystyle …}` wrapper; **8,280 blocks across 142 of 566 scrolls** (522 in `binary-relation.md`; one `modus-ponens.md` sentence spanning ~280 lines) left a quarter of the library unreadable to the human *and* noisy to the agent, the two readers the KB exists for (ADR 0111, amending 0110); `$$…$$` for a standalone formula, `$…$` for one inside a sentence with the sentence **rejoined** around it (6,541 of 8,280 sat mid-sentence); normalization runs at **render** time so SQLite keeps the capture verbatim — what `content_hash` covers and `verify` re-checks — and a re-render never moves a custody hash; source-agnostic, with text carrying no marker returned unchanged; **every literal dollar escaped** because making `$` a delimiter puts prose currency in math scope (990 dollars across 115 scrolls, 65 in `Nvidia`, 10 carrying both), and **URLs written as CommonMark autolinks** since links render outside the normalizer's reach and 12 captured URLs hold a dollar — one a `$$` that would have opened a display block; two hazards surfaced only by live data and now pinned — brace balancing must skip escaped braces or `\left\{` truncates the formula (3 blocks), and 15 formulas end in a TeX control space whose stripping bares a backslash that escapes the closing `$`; verified over the live library 2026-08-19 — **566 re-rendered/0 failures, 1,716 display + 6,522 inline formulas, 0 unprocessed blocks, 0 unbalanced files**, `doctor` 0 issues, scrolls 42 MB against 44 MB as the extract shed 14.2% of pure duplication, and the 8,795-word drop traced word by word to the literal `displaystyle`/`textstyle` markers plus fallback duplicates that survive inside the TeX — no prose lost; **a bulk re-render CLI path is the named gap** (`scrolls md` bare takes only stage `fetched`, `maintain` regenerates KB views not scrolls) | cap 4/9 |
| H431 | Bulk media capture bounded by configurable size limits (production, `src/scrolls/config.py` + `media.py` + `sources/http.py` + `cli.py`) — ADR 0110 capped rasters by *pixel width* and left bytes unbounded, and the full run showed the cost: **23 of 5,416 files held 4.1 GB of 5.7 GB**, one `.webm` being **2.5 GB, 43% of the library in a single file** no agent can read, while the other 5,393 came to 1.7 GB together (ADR 0112, amending 0011/0110); the limits are **configuration, not baked-in policy** — `[media] max_bytes` (default 25 MB) + `max_image_width`, a per-type `[media.max_bytes_by_type]` table, `0` meaning no limit, and `--max-bytes` beating the file on the same precedence `--engine` has over `default_engine`; **documents stay uncapped by default** because a PDF is the one large class that is *text* — "big" and "worthless" are different axes, and the 62.6 MB file in the tail is a CIA document holding exactly the knowledge the library is for; a **declined ref keeps `type`/`url`/`title` and records `oversize` + real `bytes`**, so the scroll says what was passed over and raising the limit re-captures it (0110's `original_url` move), while `has_pending_media` treats it as settled so a bounded library is a clean no-op; size comes from **`Content-Length` before the body**, so declining 2.5 GB costs one set of headers; media collapses to **one network seam** after the two-seam first cut left the capped path unpatched and a suite run **silently hit the real network**; verified live 2026-08-19 — 22 declined, **4.04 GB freed, 5.7 GB → 1.8 GB**, 5,394 files, `doctor` 0 issues, 566 `rendered`, `media` returning all-zero, CIA PDF kept; known gap: `application/ogg` types `.ogv`/`.ogg` as `file` not `audio`/`video` (cap still correct); deferred: PDF text extraction into the scroll | cap 4/9 |
...
---
| H432 | Whole-library verify sweep checkpointed, and the dogfood flow run live (production, `src/scrolls/cli.py`) — `docs/dogfood.md`'s four legs had only ever run on synthetic fixtures; the live library had **1 custody event across 566 items**, so *hold* was the only leg real data had exercised; the first whole-library `verify` exposed a durability gap: `record_events` was called **once after the loop**, so an interrupt at item 500 of 566 discarded 500 live round-trips *and* left the ledger claiming those items were never checked — the ledger is append-only, so a mid-run checkpoint carries no ordering risk, and verdicts now flush every 25 with a `finally` flush so an interrupt keeps every check already paid for (a 566-item sweep costs ~23 transactions, not 566); verified live 2026-08-23 over the real library — **566 checked in ~8 min, 511 unchanged / 55 drifted / 0 rotted / 0 errors**, drift landing exactly where an edited encyclopedia would put it (`Anthropic`, `ChatGPT`, `GPT-3`, `Large_language_model`, `GitHub_Copilot`) while historical articles held still; the custody narrative held as written — **integrity score stayed 100 while posture moved `sound` → `at_risk`** (`source_drift`) and `unverified` 565 → 0, and `works` surfaced **5 at-risk** where the pre-verify picture showed 0, the worst being the backprop DOI `10.1038/323533a0` held in 2 representations with none both full and unmoved; the carry leg round-tripped **109 scrolls byte-for-byte** through `export bundle` → `import bundle` → `doctor --fix`, idempotent on re-import (`0 imported / 109 unchanged`) with the fresh library reproducing the origin's posture, `as_of`, 84/25 drift split and at-risk work, and scoring its own custody at 100; media is the one thing a markdown bundle cannot carry (1,479 refs), so a fresh library holds the re-fetchable references, not the files | cap 1/11 |

| H433 | X bookmarks captured into the live library, and the `x` fetch adapter built so they can be verified (production, `src/scrolls/sources/x.py` + `x_graphql.py` + `x_bookmarks.py` + `sources/__init__.py` + `custody.py`) — the collection on-ramp shipped 2026-08-18 but had only ever run against a throwaway library, so the live one held **566 items, every one wikipedia and zero X**; the pull put **482 bookmarks in over 6 pages in 10s** (477 new + the 5 already held, 0 failures, going back to 2023) and rendered **482/482** scrolls, and the first `verify --source x` immediately exposed what ADR 0108 had accepted in the abstract: **all 482 came back `error` — `no fetch adapter for source 'x'`** — 46% of the library permanently unprovable and stamping unactionable rows into an append-only ledger on every sweep; 0108's guessed fix (*"most likely a re-sync that compares instead of skipping"*) was not the path that existed — X's own web client reads a single post through **`TweetResultByRestId`**, the same internal GraphQL API over the same session cookie, so the adapter is per-item like every other and needed no special re-sync mode; the query id was **read out of x.com's own JS bundle rather than guessed** (104 operations recovered; the bookmarks id lives in a lazily-loaded chunk and was left as pinned) and is dated beside it; **the browser session is the route, deliberately not the easier one** — X's keyless syndication CDN returns matching text but is *anonymous*, so it cannot see a protected account you follow and a re-capture through it would report drift that never happened (the rule is now standing repo policy in `CLAUDE.md`: logged-in browser session primary, stored credentials secondary and optional, since many platforms hand out no API keys at all); **both captures run one parser** (`item_from_tweet_result`) so a re-capture's `content_hash` is comparable to the original's *by construction* — had the two extractions been allowed to drift apart, every unchanged post would report drift; **a deleted post is rot, not an error** — X answers deletion with a normal **HTTP 200 whose `data.tweetResult` is empty**, so there is no 404 for `custody._is_gone` to read and the new `SourceGone` `FetchError` subtype carries the fact as a type rather than inventing a status that never crossed the wire or letting a message string decide custody, while an expired session or rotated query id stays `error` (rot would claim the post is gone when only our access is); a re-capture keeps the held `saved_at` and carries `saved_at_source`/`sort_index` across, because a second read knows no more about when you saved something than the first did; verified live 2026-08-24 — **482 checked, 482 unchanged, 0 drifted, 0 rotted, 0 errors**, with the rot path proved separately against live X (a gone id → `rotted`) so the clean sweep is a real result and not a false negative; **`x` was the last detected source without an adapter**, so every source Scrolls detects can now be fetched *and* verified — the six tests that used `x` as their stand-in for "adapterless" now simulate it (one had begun silently reaching live X through the new adapter) | cap 4/11 |

| H434 | Bulk re-render CLI path: `scrolls md --all` re-renders every held scroll from the store (production, `src/scrolls/render.py` + `cli.py`) — ADR 0111 named the gap and one week proved it twice (the KaTeX re-render and the 0110 links re-render both meant looping over ids by hand, and one such rebuild was killed externally mid-run); `--all` takes stages `fetched` *and* `rendered`, renders from the stored capture only, and **byte-compares against the file on disk before writing** (`refresh_scroll`), so the report says what actually moved — `rendered` / `unchanged` / `failed` — and a second pass is a **total no-op** (the H365 `doctor --fix` convergence idiom, spelled the way `import bundle` spells `0 imported / 109 unchanged`); custody never moves — render-time only, `content_hash` untouched, pinned by test; resume-safety needs no checkpoint machinery because **each item settles on its own** (scroll file, then DB row), so a failure or interrupt costs at most the item in flight and the re-run reports the rest `unchanged`; an identical by-id re-render now honestly reports `unchanged` rather than `rendered`; verified 2026-08-29 — 4599 tests green, 15 `md` contracts including partial-failure containment and second-pass no-op; issue #9 | cap 6 |

| H435 | The external network is structurally blocked for the whole test suite (test infra, `pyproject.toml` + `tests/test_network_guard.py`) — offline safety had rested on every test remembering to monkeypatch the right seam, and that convention failed twice in one month (ADR 0112: a suite run silently hit the real network and surfaced an HTTP 400; H433: a test began silently reaching live X), so `pytest-socket` now runs in the addopts — `--disable-socket --allow-unix-socket --allow-hosts=127.0.0.1,::1` — blocking every outward connection while loopback stays open for the tests that deliberately run real localhost servers (`test_http.py`, `test_x_oauth.py`) and unix sockets stay open for local plumbing; two teeth tests pin the guard itself (an external connect raises before any packet leaves; a loopback server round-trip still works); **the guard caught its first real bug on day one** — `test_list_sources_counts_items` was still ingesting an x.com URL under the pre-H433 "stays detected" assumption and had been dialing live X (a Cloudflare edge IP) on every run, the *seventh* stale adapterless-x test where H433 found six — fixed with the same `delitem(FETCH_ADAPTERS, "x")` idiom; verified 2026-08-29 — 4596 tests green with the guard on; issue #11 | cap 2 |

| H436 | Ogg-container media typed by extension so the per-type size caps apply as an operator expects (production, `src/scrolls/sources/wikipedia.py`) — ADR 0112's named gap: MediaWiki reports the bare container `application/ogg` for `.ogv` and `.ogg` alike, so both typed `file`, and a `[media.max_bytes_by_type]` override aimed at `video` would silently miss a `.ogv` while one capping `file` (aimed at archives) would catch it; the extension now carries the kind for exactly that MIME — `.ogv` → `video`, `.ogg`/`.oga`/`.opus` → `audio` — while the multiplexed `.ogx`, which can hold anything, honestly stays `file` rather than guessed at; every unambiguous MIME is untouched; verified 2026-08-29 — 4595 tests green, the typing pinned across all five extensions; issue #13 | cap 2 |
