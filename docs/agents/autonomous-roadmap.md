...
| H25 | **Stale-ruleset drift signal in `doctor` (makes H20's fingerprint actionable).** `scrolls doctor` reports items classified under a *superseded* ruleset — a stored `classified_ruleset` ≠ the current `RULESET_FINGERPRINT` — as a re-derivable "would re-classify under today's rules" signal, the enrichment-provenance counterpart to the custody `drift` block. It is honest, not alarmist: a stale fingerprint means *the ruleset changed*, not that the category is wrong (the recorded method is still valid for the ruleset that produced it); the report says how many, never auto-reclassifies (raw/derived is regenerated on request, never silently overwritten — custody §2.4). Tests pin the count + the no-op-when-current case. Ties the cap-8 fingerprint into the cap-1 health report and the H22 maintenance flow. | → cap 8, cap 1 |
...
- **Day 3 (2026-06-18 → 2026-06-19):** Post-MVP. The cap-8 enrichment
  ...
  shared `classification` view) are **shipped**. Next: the confidence/recency
  marker (H21), then make H20's fingerprint actionable in `doctor` (H25
  stale-ruleset signal) and extend the classification view to `search` hits
  (H26 parity). Begin framing the hourly worker as scheduled custody
  maintenance (H22 draft).
