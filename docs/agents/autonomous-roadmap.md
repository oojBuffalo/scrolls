...
The next lead slot is **H239** (the topmost un-started row), now that **H238 shipped
this run** — the *bundle*-surface corner of the H216/H224/H231 round-trip matrix: the scoped
`export bundle` → `import bundle` round-trip rebuilds the `partial` capture's `content_hash`-less
scroll (and, since the scope captures every item, the compiled `library/` pages) byte-for-byte, the
bundle envelope the whole-library byte-identity tests never exercise — completing the round-trip-depth
matrix's fourth cell (tier × bytes × {whole-library, bundle}). H239 is the *completeness complement* of
H233 — the dry-run `new`/`held` lists *partition* the bundle's distinct ids (union = the distinct set,
intersection empty), so no id the merge touches is invisible to review or counted in both lists, H240 is
...
one bundle), and H244 (appended this run) is the *bundle-artifact* analogue of
...
**This run shipped H238** — the *bundle*-surface corner of the H216/H224/H231 round-trip matrix
