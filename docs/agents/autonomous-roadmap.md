...
| H29 | **LLM concept-summary provenance view + doctor stale-summary signal (the summary-axis counterpart of H20/H25).** `kb_llm` summaries already store `engine` + a `members_hash` fingerprint of the scrolls synthesized (the re-derivation key). Surface a derived `summary_provenance` view (`by`/`members_hash`, freshness vs the live members) wherever a concept summary is read, and — mirroring H25's stale-ruleset block — a `doctor` signal for summaries whose *current* members_hash ≠ the stored one (members changed since synthesis → regenerable, report-only, never auto-regenerated; custody §2.4). The concept-summary half of "auditable enrichment stays" (vision §3.6). Tests pin the view + the stale/fresh/never-summarized buckets. | → cap 8, cap 1 |
