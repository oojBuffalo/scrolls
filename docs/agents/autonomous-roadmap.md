...
- **Work-level custody consolidation — a work has an aggregate custody posture *and*
  ...
  *consolidation-loss-over-time* leg** — the at-risk-works count now rides the
  ...
  `delta`/`recorded_at` (a documented distinction). The remaining leg is the **compiled
  surface** (the at-risk alarm on `library/index.md`, H269).
...
The next lead slot is **H269** — the compiled `library/index.md` landing page carries
the at-risk-works alarm, the consolidation alarm reaching the *static compiled* surface
(the at-risk counterpart of the library-wide custody headline on `index.md`, H96). The
KB compiler already writes the whole-library `custody_headline` beneath the `index.md`
header (H96); H269 emits the shared `works.render_at_risk_works` line (H264) over the
whole rendered library beneath it, inside the sentinel fence (M1, ADR 0102) so a
re-compile never clobbers hand annotations, with an honest no-op when no multi-rep work
is at risk. So a human browsing the compiled library reads the same consolidation alarm
an agent reads from `doctor`. **With H268 (this run) the trend-over-time leg's *readable*
side is done** — the at-risk count is now readable on the `maintain` report and the
`--trend` summary (`maintain.at_risk_headline`), so consolidation loss reaches every
agent-JSON, readable-briefing, *and* maintenance-report/trend surface; only the
**compiled-library** surface (H269) is left. **With H266 work-level custody was complete
across every *point-in-time* axis** — readable (H261), filterable (H262), alarmed (H263
JSON), readable-briefing (H264), browsable (H265), and stats-summarized (H266); **H267
opened the *trend-over-time* leg** (the at-risk count rides the `maintain`
snapshot/delta/`--trend` and `status`'s `custody.at_risk`) and **H268 (this run) gave it
a readable line**; H269 (compiled surface) is the last leg. The **budget/tier
...
H269 first; reach for a guard cell only when no capability is ready.
...
**This run (2026-06-22) shipped H268** — the at-risk-works count is now **readable** on
