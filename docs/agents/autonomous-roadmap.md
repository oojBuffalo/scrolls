...
- **New theme opened — conflict-on-import is surfaced, never silently swallowed
  ...
  **Deferred to H283 (the `at_risk` → H267/H268 analogue):** the cross-run
  `delta`/`--history`/`--trend` treatment of the conflict scalar + a readable `_Conflicts:_`
  `maintain` line — recorded in the snapshot today, not yet differenced. **With H279 the
...
| --- | --- | --- |
| H283 | **The conflict scalar gains a cross-run `delta` / `--history`/`--trend` axis and a readable `maintain` `_Conflicts:_` line — the `at_risk` → H267/H268 analogue, closing H279's deferred *conflict-over-time* leg.** H279 put the unresolved-conflict count on `status`'s `custody` snapshot and the shared `maintain.custody_snapshot`, but (unlike the at-risk-works count, H267/H268) it is recorded today *without* being differenced: `maintain`'s `delta`, the `--history`/`--trend` `conflicts_change` axis, and a readable `_Conflicts: N (▲M since last run)_` / `_Conflicts: N (▲M over K runs)_` line do not exist. This slice lifts the exact H267/H268 machinery to the conflict axis: thread `conflicts` through the `maintain` snapshot `delta` (the cross-run signed change), add the `conflicts_change` trend axis to `--history`/`--trend`, and render the readable line via a shared `maintain.conflicts_headline(count, change, *, span)` (the conflict twin of `maintain.at_risk_headline`) — `▲` worse, `▼` better, `0` no-change, bare `_Conflicts: N._` when there is no baseline (the H267 honesty); the MCP↔CLI convergence test strips it beside `delta`/`recorded_at` (the H268 run-position distinction). A *recorded but not differenced* scalar made a *trend*, never a `posture` trigger (a conflict count re-views the same divergence the score already reflects). Test only-ish (`tests/test_maintain.py` + `tests/test_cli.py`, beside the at-risk-trend twins). Precondition: H279 (the conflict scalar on the snapshot, shipped), H267/H268 (the at-risk delta/headline machinery to mirror). | → cap 10, cap 1 |
| H280 | **The prior-content archive travels in the portable bundle — `export bundle`/`export items` carry the `item_archive` snapshots so "take it with me" includes the recovery store (ADR 0106's first deferred extension).** ADR 0106 made adoption custody-safe locally (the superseded prior is archived, recoverable via `scrolls archive show`), but the archive is a **local** store: a `superseded` event travels in the lossless round-trip while the archived prior *bytes* stay behind, so a library rebuilt from a bundle can read *that an adoption happened* (the event) but cannot recover the prior copy. The decisive design question: *does the archive belong in the lossless round-trip, and if so as what?* The likely shape is a new sentinel-fenced **archive block** in the bundle (and an `export archive` JSONL sibling of `export events`) carrying the `item_archive` snapshots, with an idempotent `import` that dedups by `(item_id, prior_hash)` (the events-dedup precedent, H67) — so the recovery store round-trips losslessly and `archive show` on the rebuilt library re-emits the same prior. Opt-in or always-on is the sub-question to resolve (the archive can be large; likely a `--with-archive` flag, the bundle staying lean by default). Precondition: H278 (the archive + `export items` shape, shipped), ADR 0106, H67 (the events-block dedup precedent). | → cap 9, cap 5 |
| H281 | **An MCP archive *read* twin — `get_archived(item_id)` / a library-archive list over MCP, so agents see the recovery store (ADR 0106's deferred MCP twin, read side only).** `archive list`/`archive show` are CLI-only today; the read side (what was superseded, with before/after hashes; the recoverable prior snapshot) is a custody fact an agent should reach over MCP, exactly as `get_library_health`'s `custody.conflicts` already travels. The write (`import --accept-incoming`) stays operator-gated (custody §2.4) — this is the *read* twin only, folding the same `items.list_archived`/`latest_archived` the CLI reads, so the surfaces converge by construction. Decisive choice: the snapshot payload shape (metadata-only list vs. the full re-importable snapshot per item) — likely the `archive list` metadata for the list tool and the model-complete snapshot for a single-item `get_archived`, mirroring the CLI split. Precondition: H278 (the archive reads, shipped), the MCP read-tool conventions. | → cap 6, cap 1 |
| H282 | **`scrolls archive prune` — a retention act bounding the append-only archive (ADR 0106's deferred retention).** The `item_archive` grows unbounded (every adoption appends a prior snapshot, and the symmetric restore round-trip appends more). A custody-safe prune lets an operator reclaim space: drop snapshots older than a boundary (`--before ISO`, the `verify --stale-before` precedent) or beyond `--keep N` per item (keep the most recent N priors), **report-only by default** with `--dry-run` predicting the drop set (the H245/H273 discipline) and a loud summary of what was removed. The decisive question: *is pruning the archive a custody violation?* — answered no, because the archive is a *recovery convenience*, not the root of trust (raw-is-sacred protects the **held** copy; a superseded prior is already a deliberate replacement), but the act must be explicit, dry-run-able, and never touch a held row. CLI-only, idempotent. Precondition: H278 (the archive store, shipped), ADR 0106. | → cap 1 |
| H284 | **An end-to-end *adopt a peer's better capture* dogfood — the accept-incoming flow on both surfaces, narrated in `docs/dogfood.md` with captured before/after output (the M5 dogfood discipline on the new write).** The existing dogfood proves *hold → prove → detect → take it with me*; H278 added a genuinely new custody move — *adopt* a diverging peer capture while keeping the prior recoverable — that no dogfood exercises end-to-end. This slice adds the flow to `tests/test_dogfood.py` (offline, fixture-driven): import a peer bundle (conflict surfaced + recorded) → review via `doctor`/`history` → `import … --accept-incoming` (adopt + archive + clear the aggregate) → `archive show \| import --accept-incoming` (restore the prior) — asserting the conflict aggregate moves 0→1→0 and the held content flips and flips back, with the integrity score never lowered (adoption is a recorded event, the prior archived — §2.4). Narrate it in `docs/dogfood.md` beside the drift-detection proof. Precondition: H278 (accept-incoming + archive, shipped), H206/H210 (the self-healing dogfood shape to mirror). | → cap 8, cap 3 |
...
| H256 | **Buffer refresh checkpoint** (maintenance rule). *Serviced 2026-06-22, post-H278*: the conflict-on-import / reconciliation theme is now **closed on *both* resolution directions across detect → read → resolve** — detect (H272 `import items`, H273 `import bundle`, H274 the recorded conflict event, ADR 0104), read (H275 `doctor`'s `custody.conflicts` + MCP twin, H277 the `_Conflicts:_` briefing line on both bundle forms + `context`, H279 the `status` JSON scalar), and **resolve** both ways (H276 `reconcile --keep-held` — affirm the held copy, ADR 0105; **H278 `import … --accept-incoming` — adopt the incoming copy with prior-content archival, ADR 0106 + schema v8**). With the theme closed, the queue was **re-derived** from ADR 0106's deferred section (the archive's reach + retention) and H279's deferred (the conflict-over-time leg): the new **archive-recovery sub-theme** — H283 (the conflict cross-run delta/trend + readable `maintain` line, the H267/H268 analogue), H280 (the archive travels in the portable bundle, the lossless-round-trip reach), H281 (an MCP archive *read* twin), H282 (`archive prune` retention), H284 (an accept-incoming end-to-end dogfood) — plus the de-prioritized budget/tier guards H244–H249 below. So **5 fresh capability slots** are held above the 6 guards. **Still due at the next checkpoint:** a full 3-day/week-plan re-derivation with absolute dates re-confirmed against `docs/product/mvp.md` once the archive-recovery sub-theme is ~half shipped, or when the capability queue drops below ~6 un-started slots. Bi-temporal drift framing stays deferred unless an agent workflow shows the H274 event record insufficient. | maintenance |

The next lead slot is **H283** (the conflict cross-run delta/trend + readable `maintain`
`_Conflicts:_` line — the clean H267/H268 analogue, closing H279's deferred conflict-over-time
leg), then the **archive-recovery sub-theme** H280–H282/H284. The **conflict-on-import /
reconciliation theme is now closed on *both* resolution directions across all three legs**, with
...
partition). Take H283 next (H278 shipped 2026-06-22, closing the conflict-on-import theme on
both resolution directions), then the archive-recovery sub-theme H280–H282/H284; reach
