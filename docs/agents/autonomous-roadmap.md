...
The next lead slot is **H238** (the topmost un-started row), now that **H237 shipped
this run** — the *corrupt-bundle analogue* of H220's byte-identity: the dry-run's whole `events` block
(`{imported, skipped, orphaned, orphaned_items}`) equals the real import's over an orphan-bearing
bundle, including the resolvable-event accounting the two surfaces compute through *different*
functions (`preview_import_events` vs. `import_events`), so the preview never under- or over-reports
the orphan loss *or* the `{imported, skipped}` split relative to reality. H238 is the *bundle*-surface
corner of the H216/H224/H231 round-trip matrix — the scoped `export bundle` → `import bundle`
round-trip rebuilds the `partial` scroll byte-for-byte too, the bundle envelope the whole-library
byte-identity tests never exercise, H239 is the *completeness complement* of H233 — the dry-run
`new`/`held` lists *partition* the bundle's distinct ids (union = the distinct set, intersection
empty), so no id the merge touches is invisible to review or counted in both lists, H240 is the
*deeper-tier-headline analogue* of H234 — the whole `index`/`connected`/`full` budget ladder stays
...
untruncated/truncated × CLI/MCP fidelity-convergence matrix, and H243 (appended this run) is the
...
**This run shipped H237** — the *corrupt-bundle analogue* of H220's byte-identity guarantee
