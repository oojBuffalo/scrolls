...
(round-trip, regeneration-safety, surface-parity, completeness-honesty) — H390 (the CLI `export archive`
determinism guard, genuinely distinct from the MCP read surface) survives as the next slice, with
H394–H398 the new consolidation cells.
...
pruned; see the Status snapshot). The forward buffer now pivots to a **contract-consolidation** theme:
**H390** (the CLI `export archive` determinism guard — genuinely distinct from the MCP read surface)
survives as the next slice, with **H394–H398** the new consolidation cells.
...
and completeness-honesty — the PRD success metrics). Take the next one whose preconditions are met (H390
next):
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  **whole MCP read-surface determinism contract**, one completeness-asserted guard over every registered
  ...
  H389/H391/H392/H393 were subsumed and pruned; suite **4222 passed**).
  Remaining, un-started (the **contract-consolidation** theme, re-derived this run off the PRD success
  metrics): **H390** (the CLI `export archive` determinism + round-trip guard) — next — then **H394** (CLI
  read-surface determinism contract, the CLI sibling of H388), **H395** (round-trip transport contract),
  **H396** (regeneration-safety contract, ADR 0102), **H397** (surface-parity contract), and **H398**
  (completeness-honesty contract, M2) — each a single completeness-asserted invariant that auto-covers new
  surfaces, replacing a *family* of per-surface tests. Close a de-prioritized guard (H244–H249) only if
  nothing better is ready. The next once-per-24h full re-derivation is **due 2026-06-27** (the 3-day plan
  boundary).
