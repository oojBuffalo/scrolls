...
| --- | --- | --- |
| H287 | **A *restore-by-version* dogfood — roll back to a specific earlier version across multiple supersessions, the H284 analogue on the H285/H286 reads.** H284 dogfoods *adopt a peer's better capture* (one supersession, restore-the-latest); it never exercises **multi-supersession restore-by-version** — an operator who made several adoptions then realizes a *specific earlier* version was right and rolls back to it by `--hash`/`--at`, not just the latest. A new roll-back-to-a-specific-earlier-version case in `tests/test_dogfood.py` holds a topic, adopts three successive divergent captures (v1→v2→v3, archiving the original + two intermediates), inspects the full history via `archive show <id> --all` (H285), then `archive restore <id> --hash <H>` (and a second leg via `--at <ISO>`) rolls back, pinning **three custody points**: (a) the held `content_hash` flips to the *chosen* prior (not merely the latest), (b) the displaced v3 is itself archived — `archive show` recovers it, so the rollback is reversible (§2.4), and (c) `doctor`'s `custody.score` holds at 100 throughout (restore-by-version swaps one full-fidelity capture for another; the displaced one is archived, never lost). Narrated in `docs/dogfood.md` with captured before/after CLI output (the M5 dogfood discipline on the new write). Test-only + docs, no production change. Precondition: H286 (`archive restore`, **shipped 2026-06-22**), H285 (`archive show --all`, shipped), H284 (the dogfood shape, shipped). | → cap 3, cap 8 |
