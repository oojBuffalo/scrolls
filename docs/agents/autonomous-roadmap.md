...
buffer's active **contract-consolidation** theme has since shipped **H394–H411** (CLI read-surface
...
convergence, raw-immutability act-surface, append-only custody-ledger, stale-set drill, and facet-aggregate
drill); the live queue (below) is the authoritative lead, now leading with **H412**.
...
exemption]; suite **4316 passed**; see the Status snapshot / Shipped ledger). The forward buffer's
**contract-consolidation** theme now leads with **H412** (enrichment-provenance convergence), with
**H413–H418** the remaining cells.
...
and completeness-honesty — the PRD success metrics). Take the next one whose preconditions are met (H412
next; H408 raw-immutability act-surface + H409 append-only custody-ledger + H410 stale-set drill + H411
facet-aggregate drill shipped — see the Status snapshot / Shipped ledger):
...
| H413 | **Recency (`last_checked`) convergence contract — the *time*-axis sibling of H397's custody-axis surface matrix and H407's rank-axis matrix: one completeness-asserted invariant that for *every* held item its `last_checked` recency (the time axis of drift, H84/H88 — when the latest verify verdict was taken, or `null` when never re-checked) reads **identically** across every surface that carries it: `search`/`list`/`show` rows + the MCP `search_scrolls`/`list_scrolls`/`get_scroll` twins + `related`/`get_related_scrolls` neighbour rows, against one canonical projection `last_checked(latest_events(db).get(id))` (`custody.py`). H397 explicitly named `last_checked` a *non-custody* row field (carried, not an H397 axis); this consolidates the scattered per-surface H88 recency tests the way H407 consolidated the strength tests. The twentieth **contract-consolidation** cell whose **completeness keystone** — `_RECENCY_SURFACES` ∪ a named `_NO_RECENCY_AXIS` exemption set (the reads with no per-item recency field: `facets`/`works`/`graph`/`doctor`/`status`/`history`/`maintain`/`archive *`/`context` + their MCP twins) must partition `_CLI_READ_PATHS`/H394 ∪ `_MCP_READ_TOOLS`/H388 *exactly*, so a *new* recency-bearing read fails until it declares convergence. → PRD success-metric: per-item custody picture everywhere (cap 1/8).** Over the H397 `_seed_surface_parity_mix` (a re-checked item, a drifted item, a never-checked item → recency spans a real timestamp *and* `null`), assert each surface's per-item `last_checked` equals the canonical for that id. **Sabotage:** one surface (`cli.item_summary`, distinct from `mcp_server.item_summary`/`hit_payload`) re-deriving one item's recency as a wrong/fabricated timestamp fails *only* that surface's leg (the H397/H407 binding-isolation precedent). **Decisive choice:** include the `null` (never-checked) case so a surface fabricating a recency for an unverified item is caught (the M2 anti-fabrication tie on the time axis). Test-only. Precondition: `custody.last_checked`/`latest_events` (H84/H88), the per-surface `last_checked` fields, the H397 fixture + read registries. | → cap 8 |
| H414 | **Could-not-check (G1) error-parity contract — the *error-path* sibling of H400's CLI↔MCP read-parity matrix (H400 pins clean-read parity; this pins the failure shape): one completeness-asserted invariant that every read surface answering on a specific target returns the **same honest could-not-check signal** across its CLI↔MCP twin — an unknown id (`show`/`get_scroll`, `related`/`get_related_scrolls`, `history`/`get_scroll_history`, `works <ref>`/`get_works`, `archive show`/`get_archived`) and an unknown closed-vocab value (`--fidelity`/`--drift`/`--strength` on `search`/`list`/`related` + twins) both surface as a typed error, never a silent empty result (the M2 G1 axis, custody-vision §2.6). The twenty-first **contract-consolidation** cell whose **completeness keystone** — `_G1_TWINS` (the id-resolving + vocab-validating read twins) ∪ a named `_NO_G1_AXIS` set (reads with no specific target to miss: `list`/`facets`/`graph`/`doctor`/`status` whole-library reads) partition the target-bearing slice of `_CLI_MCP_TWINS`/H400 *exactly*, so a *new* targeted read fails until it declares its error parity. → PRD success-metric: completeness honesty / could-not-check ≠ empty (cap 7).** For each twin, drive the unknown id and the bad vocab value and assert CLI (stderr error envelope, exit 1) and MCP (raised `ValueError`) carry the same error class/message for the same bad input. **Sabotage:** one surface swallowing the error into an empty list (a silent could-not-check) fails *only* its twin's leg. **Decisive choice:** compare the *typed* failure, not mere non-zero exit, so a generic catch-all that loses the distinction is caught. Test-only. Precondition: the per-surface `resolve_item_id`/closed-vocab `ValueError` paths (G1, `test_completeness.py`), `_CLI_MCP_TWINS`/H400, `_MCP_READ_TOOLS`/H388. | → cap 7 |
| H415 | **Bundle-briefing ↔ live-audit convergence contract — the *portable-vs-live* sibling of H401's two-form bundle-parity matrix (H401 pins Markdown≡HTML; this pins briefing≡`doctor`/`status`): one completeness-asserted invariant that *every* custody fact a shareable `export bundle` briefing renders equals the live audit read for the **same scope** — the `_Custody:_` fidelity holdings ≡ `doctor --json` `custody.tiers` over the scope, `_Attention:_` ≡ `weakest_source`, `_Conflicts:_` ≡ `custody.conflicts`, `_Archive:_` ≡ `custody.archive`, `_Duplicates:_` ≡ `custody.content_duplicates`, `_Posture:_` ≡ `custody.posture`, `_Refresh:_` ≡ `custody.enrichment/summaries` — so a recipient reading the portable briefing sees exactly what the sender's audit would report. The twenty-second **contract-consolidation** cell reusing H401's `_BRIEFING_LINES` registry (cross-imported from `tests/test_bundle_format_parity.py`) as the **completeness keystone**: each custody-bearing briefing line ∪ a named `_NO_AUDIT_BASIS` set (the lines with no whole-scope audit counterpart, e.g. per-source breakdown / concept-summary provenance) must partition the registry exactly, so a *new* briefing line fails until tied to its audit basis. → PRD success-metric: fidelity/provenance travel with every result (cap 9).** Over a whole-library-scope bundle on the H401 composite fixture, assert each briefing fact equals the `doctor`/`status` projection for that scope. **Sabotage:** a briefing line folding a tampered count (distinct from the audit fold) desyncs *only* its line. **Decisive choice:** scope the bundle to the whole library so the briefing fold and the audit fold cover the same set (a scoped bundle has no whole-library audit twin — the named exemption boundary). Test-only. Precondition: `bundle.py`'s briefing folds, `doctor.run_doctor`/`status` custody block, H401's `_BRIEFING_LINES`, the H401 composite fixture. | → cap 9 |
| H416 | **Preview ↔ live-run parity contract — the *preview-never-drifts* sibling of H409's append-only-ledger and H408's raw-immutability matrices (those pin what a *live* act may touch; this pins that a *preview* touches nothing yet predicts the live effect exactly): one completeness-asserted invariant that every write command with a read-only preview mode (`--dry-run`, or report-only-until-`--apply`) (1) leaves the library byte-identical — the `custody_events` ledger (H409's `_EVENT_IDENTITY` set), the raw `scrolls/` tree (H408's whole-tree hash), *and* the `items`/`item_archive` row counts all unchanged — and (2) predicts the live run's disposition exactly: the same counts/ids the live act then produces. The twenty-third **contract-consolidation** cell whose **completeness keystone** — a `_PREVIEW_ACTS` registry (`import items`/`import bundle`/`import events` `--dry-run`, `archive restore --dry-run`, `archive prune` report-only, `reconcile --dry-run`) ∪ a named `_NO_PREVIEW` set (the write commands with no preview mode) must partition `_CLI_WRITE_COMMANDS`/H394 *exactly*, so a *new* write command shipping a preview mode fails until it declares a parity assertion (the H408/H409 registry-completeness mechanism on the preview axis). → PRD success-metric: preview never drifts / lossless round-trip reliability (cap 7/9).** For each preview act over the shared fixture (its incoming built to a *non-vacuous* disposition — a conflicting import, a real archived prior, an open conflict), snapshot the three durable axes, run the preview, assert all three unchanged AND the predicted disposition equals the live run's (run second on a fresh copy). **Sabotage:** a preview whose predicted count diverges from the live count (e.g. a `_preview_merge_items` that under-counts conflicts) fails *only* its leg; a preview that secretly writes one ledger/raw/row mutation fails *only* its leg's no-op axis. **Decisive choice:** assert the no-op on all three durable axes (ledger + raw + rows), not just the report — a preview that writes behind a clean report is the precise failure mode. Test-only. Precondition: `_preview_merge_items`/`_merge_items` (cli.py), `preview_import_events`/`import_events` (custody.py), the `--dry-run`/`--apply` flags on `import`/`archive`/`reconcile`, H408's whole-tree-hash idiom, H409's `_EVENT_IDENTITY` snapshot, the shared `seed_read_surface_determinism_mix`. | → cap 7 |
...
stale-set drill, H411 facet-aggregate drill); the forward queue is H412–H418. No new adapters.
...
- **Day 3 (2026-06-29 → 2026-06-30): Done early — H405–H411 all shipped** (the lock let the run overrun
  ...
  custody-ledger), H410 (stale-set drill), H411 (facet-aggregate drill). The lead is now **H412**
  (enrichment-provenance convergence), forward queue H412–H418 (the buffer was refreshed with H417
  facet scope-composition + H418 audit-aggregate ↔ browse-drill per maintenance-rule §1). Next once-per-24h
...
- **Active — the contract-consolidation theme.** Each cell replaces a *family* of per-surface
  ...
  **Shipped:** H388 (whole-MCP determinism), H394 (CLI determinism), H395 (round-trip transport),
  ...
  H410 (stale-set drill), **H411 (facet-aggregate drill)**.
  **Forward queue:** H412 (enrichment-provenance convergence), H413 (recency `last_checked`
  convergence), H414 (could-not-check G1 error-parity), H415 (bundle-briefing ↔ live-audit
  convergence), H416 (preview ↔ live-run parity), H417 (facet scope-composition convergence), H418
  (audit-aggregate ↔ browse-drill convergence). The once-per-24h full re-derivation was **performed
