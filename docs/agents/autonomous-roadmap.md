...
The next lead slot is **H224** (the topmost un-started row), now that **H225 shipped
this run** — the whole-library `export items` → `import items` backup sibling of H216:
a library spanning all three fidelity tiers must rebuild *tier-lossless* on that
portable surface too (it is only ever exercised all-`full` today), so the leanest
tier's holdings honesty has a portable-round-trip twin. H227 carries H223's facet-axis
...
on the `export items` → `import items` path too — H226 makes
H220's preview *reviewable* by naming which scrolls are new vs. already held, and
H230 (appended this run) carries H225's orphan-item ids into the *structured*
import summary (`events.orphaned_items`), so a programmatic consumer gets the same
diagnosability the human warning does. Per-slice provenance for every *shipped*
