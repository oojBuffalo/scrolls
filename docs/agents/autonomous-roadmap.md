...
- **New theme opened — conflict-on-import is surfaced, never silently swallowed
  ...
  **Deferred (the `at_risk` → H267/H268 analogue):** the cross-run `delta`/`--history`/`--trend`
  treatment of the conflict scalar + a readable `_Conflicts:_` `maintain` line — recorded in the
  snapshot today, not yet differenced. **With H279 the conflict-on-import theme's *read* leg is
  closed across *every* surface (JSON status, JSON doctor/MCP, readable briefings); the only
  remaining theme slice is H278 (`--accept-incoming`, the content-bearing supersession).**
...
partition). Take H278 next (H279 shipped 2026-06-22, closing the last conflict read gap); reach
