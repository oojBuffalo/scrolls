...
is next** (the `export items` JSONL whole-file determinism + round-trip guard); H380–H383 are queued
behind it, and **H384** (the `export events` JSONL whole-file determinism + round-trip guard, the
events-transport sibling of H379) was appended this run to keep the buffer ≥6.
...
cell — the cross-run maintenance-ledger axis), and **H378 shipped this run** (the `export bundle
--format html` whole-text determinism guard, the eleventh pivot cell — the HTML-render axis); the
H379 `export items` JSONL determinism cell is next, and H380–H384 are queued behind it.
...
— take the next one whose preconditions are met (H379 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H379 | **`export items` JSONL whole-file determinism + round-trip byte-identity guard — pins that the whole-library backup transport (`scrolls export items`, the JSONL the bundle item block wraps) is a *reproducible artifact*: exporting the same unchanged library twice yields a byte-identical JSONL, AND a real `export items` → `import items` into a fresh `SCROLLS_HOME` → re-`export items` reproduces the sender's bytes. The twelfth **forward-hardening cell** — the *whole-library-backup transport* sibling of H368's scoped-bundle determinism (the same `dump_items_export` fold, but the *entire* holdings, no query scope — the JSONL `tests/test_roundtrip.py` rebuilds trees from but never pins for whole-file byte-identity through the export→import→re-export loop); deliberately *not* another content-identity guard. → cap 9.** `dump_items_export` is the backup root — both `export items` and the bundle item block emit it (H238) — so its reproducibility is load-bearing, yet the whole-*file* identity is only *implied*: an unsorted `list_items` fold or a set-iteration leak in a per-row field would pass the round-trip tree tests (which compare rebuilt `scrolls/`, not the JSONL bytes) yet make two backups of one library disagree, breaking an operator's `diff` across two machines. Add a section to `tests/test_roundtrip.py`: build a mixed-source library, `export items` twice with no DB change, assert byte-identical JSONL; then `import items` into a fresh home, re-`export items`, assert it reproduces the sender's bytes. **Decisive choice:** compare the *whole JSONL file*, not one row, and include the round-trip (export→import→re-export), the lossless-backup reproducibility a single-pass identity check misses. **Sabotage check:** a counter appended to one exported row must fail the byte-identity. Test-only, no production change (`export items` is already deterministic — ordered `list_items` fold, no wall-clock). Precondition: `dump_items_export`/`load_items_export`, `scrolls export items`/`import items`, the `tests/test_roundtrip.py` harness (H231). | → cap 9 |
