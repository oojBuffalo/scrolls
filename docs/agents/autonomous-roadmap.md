...
row-count axis). **H381 is next** (the MCP `get_library_health` determinism twin); H382–H385 are queued
behind it, and **H386** (the `import archive` re-import idempotency guard, the *archive-transport* third
member of the ingest-idempotency family after H374/H380 — `import_archive` dedups on
`_ARCHIVE_IDENTITY = (item_id, prior_hash)`, a distinct code path) was appended this run to keep the
buffer ≥6.
...
identity); the MCP `get_library_health` determinism twin **H381** is next, and H382–H386 are queued
behind it.
...
— take the next one whose preconditions are met (H381 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H381 | **MCP `get_library_health` whole-payload determinism guard — pins that the *agent-facing* custody audit (the MCP twin of `scrolls doctor`, H161) is a *reproducible artifact*: calling `get_library_health` twice on the same unchanged library yields a byte-identical serialized payload, same-process AND across a `PYTHONHASHSEED` subprocess pair. The fourteenth **forward-hardening cell** — the *MCP-surface* sibling of H375's CLI `scrolls doctor` determinism: H375 pinned the CLI `print(json.dumps(run_doctor(...)))` path, but the MCP tool is a *separate serialization* an agent reads (`get_library_health` folds the same `run_doctor` report yet returns it through the MCP tool envelope, which may project/re-key the audit — a distinct code path no determinism test pins); deliberately *not* another content-identity guard. → cap 1/2.** `run_doctor` is the trust root H375 pinned reproducible, but an agent driving MCP never calls the CLI — it calls `get_library_health`, and if that twin re-folds any audit sub-block over a `set` (or the MCP envelope orders its keys by dict iteration of a set-derived map), two calls of one library could disagree byte-for-byte, breaking an agent that caches the health payload and re-fetches to detect custody movement (the H201/H204 MCP-driven dogfood loop). Add a section to `tests/test_mcp.py`: seed the H375 `_seed_doctor_determinism_mix` non-vacuous fixture (every order-sensitive sub-block populated — by_source, content-duplicate group, drift + conflict + archive events, an at-risk work), call `get_library_health` twice in-process and assert the serialized payloads are byte-identical, then invoke the MCP tool in two subprocesses under different `PYTHONHASHSEED`s and assert identical output. **Decisive choice:** serialize and compare the *whole* MCP payload (every sub-block + the envelope), not one field, and include the cross-seed pair — the set-iteration leak a same-process pass misses; assert the H186/H180 MCP shape contract still holds so the determinism guard rides the registered-twin invariant. **Sabotage check:** folding one audit sub-block over a `set` in the MCP path must fail the cross-seed byte-identity while the per-tool `get_library_health` shape tests stay green. Test-only, no production change (the MCP audit is already deterministic — it folds the ordered `run_doctor` report, no wall-clock). Precondition: MCP `get_library_health` (H161), `build_server`/`mcp_server._TOOLS` (H364), the H375 `_seed_doctor_determinism_mix` fixture + cross-`PYTHONHASHSEED` subprocess harness, the H186 MCP shape contract. | → cap 1/2 |
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  **4201 passed**). **H377–H380 shipped** (the `maintain` trend-log determinism + no-movement
  ...
  whole-file determinism + round-trip, and — this run — the `import events` re-import idempotency
  guard; suite **4207 passed**). Remaining, un-started: **H381** (the MCP `get_library_health`
  determinism twin) — next — then **H382–H386** (the MCP `get_context_bundle`/`run_maintenance`
  determinism twins, the `export events` JSONL determinism, the `archive show` determinism +
  restore round-trip, and the `import archive` re-import idempotency guard appended this run to
  keep the buffer ≥6). Close a de-prioritized guard (H244–H249) only if nothing better is ready.
