...
staying green); the `export events` JSONL determinism twin **H384** is next, and H385–H389 are queued
behind it.
...
— take the next one whose preconditions are met (H384 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H384 | **`export events` JSONL whole-file determinism + round-trip byte-identity guard — pins that the whole-library custody-events backup transport (`scrolls export events`, ADR 0082/H72 — the JSONL the bundle's custody-events block wraps) is a *reproducible artifact*: exporting the same unchanged library's events twice yields a byte-identical JSONL, AND a real `export events` → `import events` into a fresh `SCROLLS_HOME` → re-`export events` reproduces the sender's bytes. The seventeenth **forward-hardening cell** — the *events-transport* sibling of H379's items-transport determinism (the same whole-library-backup shape on the `dump_events` fold rather than `dump_items_export`, and the *export*-reproducibility complement of H380's `import events` re-import idempotency); deliberately *not* another content-identity guard. → cap 9.** `dump_events` is the custody-backup root — both `export events` and the bundle's custody-events block emit it (H67/H72) — so its reproducibility is load-bearing, yet the whole-*file* identity is only *implied*: an unsorted event fold or a set-iteration leak in a per-row field would pass the round-trip/posture tests (H73/H78, which compare rebuilt custody posture, not the JSONL bytes) yet make two custody backups of one library disagree, breaking an operator who `diff`s two machines' ledgers. Add a section to `tests/test_events_export.py` (or `tests/test_roundtrip.py`): seed a library with recorded verify + conflict events, `export events` twice with no DB change, assert byte-identical JSONL; then `import events` into a fresh home, re-`export events`, assert it reproduces the sender's bytes. **Decisive choice:** compare the *whole JSONL file*, not one row, and include the round-trip (export→import→re-export), the lossless-backup reproducibility a single-pass identity check misses; pair the cross-seed `PYTHONHASHSEED` subprocess face only if a per-row field folds a `set` (else the same-process two-export check suffices — `dump_events` is an ordered fold). **Sabotage check:** a counter appended to one exported event row must fail the byte-identity. Test-only, no production change (`export events` is already deterministic — ordered event fold, no wall-clock). Precondition: `dump_events`/`load_events`, `scrolls export events`/`import events` (H72), the `_EVENT_IDENTITY` content key, the `tests/test_events_export.py` harness. | → cap 9 |
