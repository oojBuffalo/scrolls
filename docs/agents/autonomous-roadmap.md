...
- **New theme opened — conflict-on-import is surfaced, never silently swallowed
  ...
  *derived*-field-only difference (title/category) is `unchanged`, not a conflict (the
  ...
  + key table, `docs/reconciliation.md` "both lossless importers". **The next slice is H274
  (lift a surfaced conflict into a custody ledger event / reviewed `reconcile` resolution).**
...
over taking them. **The active capability lead is now H274** — the last slice of the
conflict-on-import theme (H272 opened it, H273 this run lifted it to the bundle importer);
reach for the de-prioritized H244–H249 only when no capability slice is ready, and prefer
closing one rather than appending more of the same shape.

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H274 | **A surfaced import conflict becomes a recorded custody *event* — the ledger-deepening of H272/H273 (custody §2.4: drift/conflict is a *recorded event*, not just a printed warning).** H272/H273 surface a conflict in the import report + stderr, but it leaves no durable trace: re-run the import and the divergence is re-detected from scratch, never queryable via `scrolls history <id>` / `doctor`'s `custody.drift`. This slice records a conflict as a custody event on the held item (a new `status` beside `drifted`/`rotted`, or a typed `conflict` event carrying the *incoming* `content_hash` that disagreed), so "another capture of this id disagreed with mine, observed at import time" joins the verify ledger — the bi-temporal captured-at vs source-changed-at signal the MVP deferred (`docs/product/mvp.md`), now demanded by a concrete workflow (merging a peer's bundle). The decision the slice resolves: is an import-conflict the *same* custody axis as a verify-drift (one ledger, one `drift_posture`) or a distinct **provenance-of-divergence** axis (a sibling event type)? Resolve against custody §2.4 and ADR 0098's event model; keep it report-only/non-destructive (the held copy is never touched). The conflict surface to wire from is now shared across **both** lossless importers — `_merge_items` (live, both `import items` + `import bundle`) and `_preview_merge_items` (bundle dry-run) — so the event-recording hook has one home. Precondition: H273 (the conflict surface on both lossless importers, **shipped this run**), ADR 0098 (the custody event model). | → cap 1, cap 3, cap 9 |
...
| H256 | **Buffer refresh checkpoint** (maintenance rule). *Partially serviced 2026-06-22* (across this run and the H272 run): the next capability theme was re-derived (conflict-on-import / reconciliation-detection, H272–H274); H272 + H273 recorded as shipped; ≥6 un-started slots held (H274 + the six guard cells H244–H249). **A full refresh is still due** — re-derive the 3-day/week plans with absolute dates and re-confirm the week plan maps to `docs/product/mvp.md` once H274 (the last conflict-on-import slice) ships or when the queue drops below ~6 un-started capability slots. Bi-temporal drift framing stays deferred unless an agent workflow shows the event record insufficient (H274 is the first workflow that may demand it). | maintenance |

The next lead slot is **H274** — a surfaced import conflict becomes a recorded custody
*event* (the ledger-deepening of H272/H273), the **last slice of the conflict-on-import /
reconciliation-detection theme** (H256 re-derived it from `docs/product/mvp.md` /
`docs/product/prd.md`: with the per-item and consolidation custody surfaces closed, the
un-worked custody shape was the **import boundary**, where two libraries' captures collide —
custody §2.4 / the obsidian reconcile adoption *detect, surface, don't rewrite*). H272 closed
the lossless whole-library importer (`import items`); H273 (this run) lifted the same
`_merge_items`/`_warn_conflicts` to `import bundle` and added the read-only twin
`_preview_merge_items` that predicts the conflict set in its `--dry-run`; H274 lifts a surfaced
conflict into a recorded custody *event* (queryable via `scrolls history`/`doctor`).
...
partition). Take H274 first; reach for a guard cell only when no capability is ready. Per-slice
...
**This run (2026-06-22) shipped H273** — the conflict-on-import partition now reaches the
