...
The next lead slot is **H227** (the topmost un-started row), now that **H224 shipped
this run** — the whole-library `export items` → `import items` backup proven
*tier-lossless* (a library spanning all three fidelity tiers rebuilds with doctor's
`custody.tiers` equal to the source spread, no rebuild-side downgrade), the H216
guarantee's other portable surface. H227 carries H223's facet-axis
`_Fidelity:_` scope-honesty onto the read an agent reaches over MCP (`get_context_bundle`
is a read-through of `build_context`, byte-identical to the CLI), H226 makes
H220's preview *reviewable* by naming which scrolls are new vs. already held, H228 is the
*composition* of H221's truncation and H223's facet axes (both filters compose
without double-counting), H229 ties the scoped `_Fidelity:_`
counts to `doctor --source <S>`'s `custody.tiers` (the scoped completion of H213's
convergence loop), H230 carries H225's orphan-item ids into the *structured*
import summary (`events.orphaned_items`), and H231 (appended this run) deepens H224
from tier *counts* to rendered *bytes* — the byte-identical whole-library rebuild
holds across mixed fidelity tiers (the `partial` scroll's byte-shape, never exercised
by the all-`full` fixture). Per-slice provenance for every *shipped*
slot lives in git (`git log --oneline |
