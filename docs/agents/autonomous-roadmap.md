...
| --- | --- | --- |
| H305 | **`export archive --since` is byte-stable across the recovery-store round-trip — a windowed backup re-exported from an `import archive`-rebuilt library reproduces the same stream byte-for-byte.** H280 pins the *whole* `export archive` stream byte-identical across a rebuild (the content-deterministic `(archived_at, item_id, prior_hash)` ordering survives the local-id reshuffle); the untested cell is whether the *windowed* (`--since`) stream is equally stable — the window edge selects a subset, and a naive impl that windowed on the per-library `id` rather than `archived_at` would pick a *different* subset once the rebuild renumbered the rows. Pin it: a multi-prior chain, `export archive --since <mid>` from A, rebuild B via `import items` + `import archive`, then `export archive --since <mid>` from B and assert the two windowed streams are byte-identical. So the incremental backup is reproducible, not just the full one — the lossless-round-trip reach on the time-windowed read. Test only (`tests/test_cli.py`, beside the H300/H301 round-trips). Precondition: H303 (`export archive --since`), H280 (the content-deterministic whole-stream byte-identity). | → cap 4, cap 8 |
| H306 | **The incremental-backup workflow dogfood — a full `export archive` then a later `export archive --since` increment, restored onto a peer, is idempotent and reads the recovery family identically (the operator-workflow layer above H303's test).** H303's test pins the union idempotence at the data layer; the dogfood narrates the *workflow* an operator runs: back machine A up fully at T0, adopt more captures, re-export only `--since T0`, ship both files, and on a fresh machine B `import items` + `import archive <full>` + `import archive <increment>` + `doctor --fix` + `kb` → prove `custody.score` 100, the recovery store holds every prior exactly once (no double-count), and `archive show --all`/`diff`/`restore --dry-run` read identically to A — the H292/H297 dogfood analogue on the `--since` axis. Test (`tests/test_dogfood.py`) + docs (`docs/dogfood.md` new section with captured CLI output). Precondition: H303 (`export archive --since`), H297 (the JSONL-backup recovery dogfood). | → cap 4, cap 8 |
