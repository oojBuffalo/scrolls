...
| --- | --- | --- |
| H288 | **`scrolls archive diff <id> [--hash H \| --at ISO]` — the "decide before you restore" read: compare the held copy against a selected archived prior.** Today an operator chooses a version to restore by eyeballing `archive list`/`archive show --all` (hashes + bodies); there is no read that *compares* what is held now against a specific prior. H288 adds a CLI **read** that folds the **same** `select_archived_snapshot` selector H286 uses (`--hash`/`--at`, default latest) against the currently-held copy (`get_item`), reporting the custody-relevant delta: held vs prior `content_hash`, each side's `fidelity` tier, the set of model-complete fields that differ (a pure `items.diff_snapshot(held, prior)` field-level diff over `item_to_dict` — e.g. `raw_text`/`extracted_text`/`title`/`tags`), and `would_restore` (whether a restore would actually change the held copy — the H286 idempotency predicted *before* the write). So an operator/agent answers "what would I get back, and what would I lose?" before the H286 adoption. CLI-only read (no write — the `archive show` gate, MCP twin deferred); an unmatched selector / unknown id is a could-not-recover (exit 1); honest `would_restore: false` when the prior already *is* the held copy. Test: `tests/test_items.py` (the pure `diff_snapshot` field-delta — changed-field set, empty on identical) + `tests/test_cli.py` (held-vs-prior delta, `would_restore` true/false, selector by hash/at, unmatched exit 1). Precondition: H286 (`select_archived_snapshot`, **shipped 2026-06-22**), H252-era `get_fidelity` (shipped). | → cap 1, cap 6 |
...
read-surface follow-up** (H288), then **hardening/integration** (cross-cutting
reliability, not new shapes) — not a fresh theme.
...
- **Day 2 (2026-06-23):** **H288** — `scrolls archive diff <id> [--hash H | --at
  ISO]`, the "decide before you restore" read: fold the **same**
  `select_archived_snapshot` selector H286 uses against the currently-held copy
  (`get_item`) and report the custody-relevant delta — held vs prior `content_hash`,
  each side's `fidelity` tier, the set of model-complete fields that differ (a pure
  `items.diff_snapshot(held, prior)` field-level diff over `item_to_dict`), and
  `would_restore` (the H286 idempotency predicted *before* the write). CLI-only read
  (the `archive show` gate, MCP twin deferred); unmatched selector / unknown id →
  exit 1; honest `would_restore: false` when the prior already *is* the held copy.
  Tests in `tests/test_items.py` (the pure `diff_snapshot`) + `tests/test_cli.py`
  (held-vs-prior delta, `would_restore` true/false, selector by hash/at, exit 1).
- **Day 3 (2026-06-24 → 2026-06-25):** **A hardening/integration slice** — with the
  capability cushion thin (only the 6 de-prioritized guards remain after H288), pick
  one cross-cutting reliability slice rather than a new shape: a full-suite-reliability
  or doctor/repair deepening, an export/import round-trip edge, or an MCP↔CLI /
  search/list/filter consistency tie that the archive surfaces (`archive
  list`/`show`/`restore`/`diff`) opened. Close a de-prioritized guard (H244–H249)
  only if no such capability is ready, and prefer closing one over appending more of
  the same combinatorial shape.
...
- **This week's horizon — the archive read-surface follow-up, then hardening.** The
  one fresh capability is **H288** (`archive diff`, decide-before-you-restore — the
  read that compares the held copy against a selected prior before an H286 restore).
  After it the archive read/act/recover surfaces are complete; the remaining forward
  work is **hardening/integration**, not a new theme: full-suite reliability,
  doctor/repair depth, export/import round-trip edges, and MCP/search/list/filter
  consistency across the archive surfaces.
