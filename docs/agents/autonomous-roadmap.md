...
and the H366 nesting twin stay green). **H383 is next** (the MCP `run_maintenance` determinism twin);
H384–H387 are queued behind it, and **H388** (the MCP `get_works` whole-payload determinism guard, the
fifth MCP-surface determinism twin — the work-consolidation read an agent caches, a distinct code path
folding the DOI-clustering/at-risk engine rather than the `run_doctor` audit) was appended this run to
keep the buffer ≥6.
...
H186/H180 shape contract); the MCP `run_maintenance` determinism twin **H383** is next, and H384–H388
...
— take the next one whose preconditions are met (H383 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H383 | **MCP `run_maintenance` whole-payload determinism + no-movement settle guard — pins that the *agent-facing* scheduled-maintenance pass (the MCP twin of `scrolls maintain`, H196) is both a *settled* and a *reproducible* read: two `run_maintenance` calls on one unchanged library fold comparable custody scalars that are byte-equal and a no-movement delta on every axis, and the serialized payload is byte-identical across a `PYTHONHASHSEED` subprocess pair. The sixteenth **forward-hardening cell** — the *MCP-surface* twin of H377's CLI `scrolls maintain` settle/determinism: H377 pinned the CLI `_cmd_maintain` path, but an agent driving MCP never calls the CLI — it calls `run_maintenance`, which passes a `skipped_recheck_report` (no live edge, H196) and returns the same `assemble_report` through the MCP tool envelope (a distinct entry point + serialization no determinism/settle test pins); deliberately *not* another content-identity guard. → cap 1/2.** H377 pins the CLI snapshot settles and the report reproduces, but an MCP-driven dogfood loop (H201/H204) that re-fetches `run_maintenance` to detect custody movement reads the *MCP* payload — if that twin re-folds a snapshot/headline section over a `set`, or its envelope phantom-moves a delta, two calls of one unchanged library disagree and the agent cries drift where there is none. Add a section to `tests/test_mcp.py`: seed the H377 `_seed_maintain_determinism_mix` non-vacuous fixture, call `run_maintenance()` twice in-process and assert the comparable snapshot scalars are byte-equal AND the second call's delta is the all-zero/no-movement shape on every axis, then invoke the MCP tool in two subprocesses under different `PYTHONHASHSEED`s and assert byte-identical payload (minus the wall-clock `recorded_at`). **Decisive choice:** assert *both* the settle (no phantom movement, the value face) and the cross-seed determinism (the iteration-order face a same-process pass misses); compare the *whole* payload, and assert the H186/H180 MCP shape contract still holds so the guard rides the registered-twin invariant. **Sabotage check:** a phantom `+1` in the MCP delta fails the settle, and a snapshot section folded over a `set` in the MCP path fails the cross-seed while the per-tool `run_maintenance` shape tests stay green. Test-only, no production change (the MCP pass folds the same deterministic `run_doctor` report, no live recheck, no wall-clock beyond `recorded_at`). Precondition: MCP `run_maintenance` (H196), `build_server`/`mcp_server._TOOLS` (H364), the H377 `_seed_maintain_determinism_mix` fixture + `_no_movement_delta`/cross-`PYTHONHASHSEED` subprocess harness, the H186 MCP shape contract. | → cap 1/2 |
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  **H382 shipped this run** (the MCP `get_context_bundle` whole-payload determinism guard — two
  ...
  same-process read and the H366 nesting twin stay green; suite **4211 passed**).
  Remaining, un-started: **H383** (the MCP `run_maintenance` determinism twin) — next — then
  **H384–H388** (the `export events` JSONL determinism, the `archive show` determinism + restore
  round-trip, the `import archive` re-import idempotency guard, the MCP `get_link_graph` whole-payload
  determinism guard, and the MCP `get_works` whole-payload determinism guard appended this run to keep
  the buffer ≥6). Close a de-prioritized guard (H244–H249) only if nothing better is ready.
  The next once-per-24h full re-derivation is **due 2026-06-27** (the 3-day plan boundary).
