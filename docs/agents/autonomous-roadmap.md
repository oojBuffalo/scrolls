...
- **New theme opened — conflict-on-import is surfaced, never silently swallowed
  ...
  *derived*-field-only difference (title/category) is `unchanged`, not a conflict (the
  ...
  "Shipped: conflict-on-import detection". **The next slices are H273 (the same partition
  on the bundle import — live + the dry-run conflict *prediction*, keeping the
  live≡preview convergence) and H274 (lift a conflict into a custody ledger event /
  reviewed `reconcile` resolution).**
...
over taking them. **The active capability lead is now the conflict-on-import theme
(H273–H274), opened by H272 this run** — take H273 first; reach for the de-prioritized
H244–H249 only when no capability slice is ready, and prefer closing one rather than
appending more of the same shape.

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H273 | **The conflict-on-import surface reaches the bundle importer — `import bundle` partitions its skips into `unchanged`/`conflict` (the same `merge_item`/`_merge_items`/`_warn_conflicts` H272 shipped), *and* its `--dry-run` preview *predicts* the conflict set the live import would surface.** H272 closed the lossless whole-library importer (`import items`); the bundle importer (`_cmd_import_bundle`) still loops `insert_item` and lumps a divergent held id into `skipped`. The live path is a near-mechanical reuse: swap the loop for `_merge_items`, add `conflicts` + `_warn_conflicts` to the report beside the existing `events`/`orphaned` block. The genuinely new part is the **dry-run prediction**: `_preview_import_bundle` already partitions `new`/`held` via `get_item(...) is None` (it holds the held row in hand) — extend that loop to split the `held` set into would-be-`unchanged` vs would-be-`conflict` by comparing the bundle item's `content_hash` to the held one, so the preview an operator confirms names exactly the conflicts the merge would surface (the H245-style "predict the write effect" closure, on the conflict axis). The catch the slice must hold: the live≡preview convergence test (`test_import_bundle_dry_run_counts_match_a_real_import`) — both paths must grow the *same* `unchanged`/`conflict`/`conflicts` fields, and the within-bundle-dup edge (a bundle repeating an id with different content) must classify identically on both (the live `merge_item` sees its own prior insert; the dry-run must simulate that). Test (`tests/test_bundle.py`, beside H226/H233/H239): live bundle conflict surfaced + held-copy-preserved; dry-run predicts the conflict set; live≡preview under a conflicting bundle; within-bundle-dup parity. Precondition: H272 (the `merge_item` primitive + `_merge_items`/`_warn_conflicts`, **shipped this run**), H226 (the reviewable `new`/`held` lists), H233 (the count-level live≡preview tie). | → cap 9, cap 7 |
| H274 | **A surfaced import conflict becomes a recorded custody *event* — the ledger-deepening of H272/H273 (custody §2.4: drift/conflict is a *recorded event*, not just a printed warning).** H272/H273 surface a conflict in the import report + stderr, but it leaves no durable trace: re-run the import and the divergence is re-detected from scratch, never queryable via `scrolls history <id>` / `doctor`'s `custody.drift`. This slice records a conflict as a custody event on the held item (a new `status` beside `drifted`/`rotted`, or a typed `conflict` event carrying the *incoming* `content_hash` that disagreed), so "another capture of this id disagreed with mine, observed at import time" joins the verify ledger — the bi-temporal captured-at vs source-changed-at signal the MVP deferred (`docs/product/mvp.md`), now demanded by a concrete workflow (merging a peer's bundle). The decision the slice resolves: is an import-conflict the *same* custody axis as a verify-drift (one ledger, one `drift_posture`) or a distinct **provenance-of-divergence** axis (a sibling event type)? Resolve against custody §2.4 and ADR 0098's event model; keep it report-only/non-destructive (the held copy is never touched). Precondition: H273 (the conflict surface on both lossless importers), ADR 0098 (the custody event model). | → cap 1, cap 3, cap 9 |
...
| H256 | **Buffer refresh checkpoint** (maintenance rule). *Partially serviced 2026-06-22* (this run, alongside H272): the next capability theme was re-derived (conflict-on-import / reconciliation-detection, H272–H274) and added above; H272 recorded as shipped; ≥6 un-started work slots held (H273, H274 + the six guard cells H244–H249). **A full refresh is still due** — re-derive the 3-day/week plans with absolute dates and re-confirm the week plan maps to `docs/product/mvp.md` once the conflict-on-import theme advances a slice or two. Bi-temporal drift framing stays deferred unless an agent workflow shows the event record insufficient (H274 is the first workflow that may demand it). | maintenance |

The next lead slot is **H273** — the conflict-on-import surface on the bundle importer
(live partition + dry-run conflict prediction), the second slice of the **conflict-on-import /
reconciliation-detection theme H272 opened this run** (the H256 checkpoint re-derived it from
`docs/product/mvp.md` / `docs/product/prd.md`: with the per-item and consolidation custody
surfaces closed, the un-worked custody shape was the **import boundary**, where two libraries'
captures collide — custody §2.4 / the obsidian reconcile adoption *detect, surface, don't
rewrite*). H272 closed the lossless whole-library importer (`import items`); H273 lifts the
same `merge_item`/`_merge_items`/`_warn_conflicts` to `import bundle` + predicts the conflict
set in its `--dry-run`, and H274 lifts a surfaced conflict into a recorded custody *event*.
...
partition). Take H273 first; reach for a guard cell only when no capability is ready. Per-slice
...
**This run (2026-06-22) shipped H272** — conflict-on-import is now surfaced on the lossless
