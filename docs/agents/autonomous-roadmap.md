...
id-keyed `INSERT OR IGNORE` + content-equal conflict suppression). **The H375 doctor-determinism cell
is next; H376–H377 (context/maintain-trend determinism) and H378–H379 (the HTML-bundle + `export items`
JSONL determinism siblings) are queued behind it, and H380 (the `import events` re-import idempotency
sibling) was appended this run to keep the buffer full.**
...
`export bundle` determinism + round-trip byte-identity guard, the sixth pivot cell), and
**H374 shipped this run** (the `import bundle` re-import idempotency guard, the seventh pivot
cell — the ingest-transport settle axis); the H375 doctor-determinism cell is next, and
H376–H379 are queued behind it.
...
— take the next one whose preconditions are met (H375 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H375 | **`doctor --json` whole-report determinism guard — pins that the custody/structural audit every other surface folds is a *reproducible artifact*: running `doctor --json` twice on the same unchanged library yields byte-identical JSON, same-process AND across a `PYTHONHASHSEED` subprocess pair (the H363 whole-tree-hash precedent lifted to the audit axis). The eighth **forward-hardening cell** — a distinct reproducibility axis (the audit-trust-root) beside H363's compile determinism, H368's bundle determinism, and H367's status↔doctor scalar convergence; deliberately *not* another content-identity guard. → cap 1.** `run_doctor` is the trust root — `scrolls status`/`maintain`/MCP `get_library_health` all read it (H367 just pinned status ≡ doctor) — so its reproducibility is load-bearing, yet only *implied*: a set/dict-iteration leak (an unsorted `custody.by_source` map, a `content_duplicates.groups` list ordered by dict iteration, an unsorted `archive.events`/`conflicts.events` list, a `works.at_risk` fold over a `set`) would pass the per-block `doctor` tests yet make two audits of one library disagree byte-for-byte, breaking a custody diff an operator runs across two machines or two runs. Add a section to `tests/test_doctor.py` over a non-vacuous fixture (the H367 mix — by_source split, a content-duplicate group, conflict + archive events, an at-risk work — so every order-sensitive sub-block is populated): assert `json.dumps` of two same-process `run_doctor` reports are byte-identical, then run `scrolls doctor --json` in two subprocesses under different `PYTHONHASHSEED`s (the `test_kb.py` H363 subprocess precedent) and assert identical stdout. **Decisive choice:** hash the *whole* report (every sub-block), not one block, and include the cross-`PYTHONHASHSEED` pair — a same-process pass alone misses a `set`/dict-iteration order leak that only surfaces across hash seeds. **Sabotage check:** making one doctor sub-block fold over a `set` (or drop a `sorted(...)`) must fail the cross-seed byte-identity while the per-block tests stay green. Test-only, no production change (the audit is already deterministic — ordered folds, no wall-clock; this makes the trust-root reproducibility regression-proof). Precondition: `run_doctor`/`scrolls doctor --json`, the `test_kb.py` H363 subprocess-determinism harness, the H367 non-vacuous custody fixture. | → cap 1 |
