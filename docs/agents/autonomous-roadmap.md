...
| --- | --- | --- |
| H232 | **MCP `get_context_bundle(budget="index", source=<S>, limit=k)` — the `(of N)` is honest under facet scope *and* truncation at once, the MCP twin of H228.** H227 carries H223's facet-axis scope-honesty onto the MCP read and H222 carries H221's truncation axis; H228 pins they *compose* on the CLI. This is its MCP twin: over a mixed-fidelity, multi-source library where one source's matching scope exceeds the cap, `get_context_bundle(query, budget="index", source=<S>, limit=k)` carries `_Fidelity: … (of k)._` whose `(of k)` equals the scoped-*and*-truncated `_Coverage:_` `returned` (the post-facet, post-cap kept set) and whose tier counts sum to it — never the library-wide holdings, never the unscoped-but-truncated set, never the scoped-but-untruncated set — and is *byte-identical to the CLI*'s `context --budget index --source <S> --limit k` (the agent-facing bundle and the CLI never diverge under either filter or their composition, just as H214/H222/H227 pin for the untruncated/truncated/facet-scoped line). Test only (`tests/test_mcp.py`, beside the H222/H227 twins). Implementation path: `get_context_bundle` is a read-through of `build_context`, which already passes the post-facet-*and*-post-cap `len(items)` to `render_fidelity_holdings` (`src/scrolls/context.py:252`), so the composition is correct-by-construction; the slice pins that neither axis silently reverts to a pre-filter count when the other is active on the MCP surface. Precondition: H222 (the MCP truncation twin, shipped), H227 (the MCP facet twin, **shipped this run**), H228 (the CLI composition). | → cap 2, cap 7, cap 10 |
...
| H230 | **The import summary names *which* items orphaned in its structured JSON, not only the human stderr warning — the machine-readable half of H225.** H225 named the distinct orphan `item_id`s in the `{"warning": …}` stderr string, but an agent piping `import bundle` *stdout* sees only the `events.orphaned` count (`src/scrolls/cli.py`); to learn *which* items dangle it must scrape the human warning prose off stderr. Make the diagnosable detail structured: add `events.orphaned_items` (the distinct orphan `item_id`s, sorted + deduped — the same set the H225 warning names, but **uncapped**: the machine channel carries the complete loss; the `(+N more)` cap is a human-readability concern, never a structured-field truncation, the M2 ethos) to the import summary JSON on **both** the live import and the `--dry-run` preview. Implementation path: `_warn_orphan_events` already computes `sorted({e.item_id for e in orphan_events})`; lift that into a shared `_orphan_item_ids` helper and fold the list into both summaries beside `orphaned`. Test (`tests/test_bundle.py`): the spliced two-item bundle's import summary carries `events.orphaned_items == ["arxiv:…", "wikipedia:en:Ghost"]` (distinct-item count, sorted), byte-equal between the live import and the dry-run, while `events.orphaned` stays the event count; and a >cap spliced bundle's `orphaned_items` lists *every* distinct id even though the stderr warning shows a `(+N more)` tail (structured completeness vs. human bound). Precondition: H225 (the warning ids, **shipped this run**). | → cap 9, cap 7 |
| H231 | **The byte-identical whole-library rebuild holds across mixed fidelity tiers, not only the all-`full` `_seed_items` — the byte-depth sibling of H224's tier-count guarantee.** H224 pinned that a mixed-fidelity backup rebuilds with doctor's `custody.tiers` equal to the source spread (the tier *counts* survive); `test_export_rebuild_is_byte_identical` already pins the rebuilt *rendered scrolls* and *compiled `library/` pages* are byte-identical — but only over the all-`full` `_seed_items`, so a `partial` capture's scroll (rendered with no `content_hash` to fingerprint it — a different frontmatter byte-shape than a `full` scroll) has never been proven to rebuild byte-for-byte. Pin it: reusing H224's mixed-fidelity fixture, capture the source's `scrolls_dir`/`library_dir` trees, rebuild via `import items` → `doctor --fix` → `kb` in a fresh home, and assert both `_read_tree` snapshots are byte-identical across the boundary — so the lossless round-trip is a guarantee in the rendered *bytes* of a degraded capture, not only in tier counts. Implementation path: the rebuild re-renders each scroll from identical rows (`write_scroll` is deterministic in the item fields), so byte-identity is correct-by-construction; the slice pins it on the partial tier the all-`full` fixture never reaches. Test only (`tests/test_roundtrip.py`, beside `test_whole_library_backup_is_tier_lossless`). Precondition: H224 (the mixed-fidelity backup fixture, **shipped this run**). | → cap 4, cap 9 |
| H233 | **The `new`/`held` review lists dedup honestly under within-bundle duplicate item ids — the dedup half of H226.** H226's `new`/`held` are deduped sets and `len(new) == imported` holds by construction (`item_imported = len(new_ids)`); but a *corrupt* bundle that repeats an item id (a splice/concat — `parse_bundle` appends every custody-block record without deduping, `bundle.py:667`, so duplicates reach `_preview_import_bundle`) is exactly where a naive id-list would phantom-inflate. Pin the honest dedup: over a dry-run of a bundle whose custody block repeats both a would-be-new id and an already-held id, `new` names the repeated-new id exactly once and `held` names the repeated-held id exactly once (sorted), while `imported`/`skipped` still count the raw occurrences — so the reviewable surface never over-claims more distinct items than the bundle holds (the M2 ethos on the dedup axis). The distinguishing asymmetry: `len(new) == imported` survives the duplicate (the count is the distinct-new count), while `len(held)` is < `skipped` (a held-id repeat is skipped twice but named once). Implementation path: splice a duplicate item record into a custody block (concatenate two overlapping single-item exports, or extend the orphan-test bundle helper), dry-run, assert dedup; the set-based collection in `_preview_import_bundle` dedups by construction, so the slice pins it. Test only (`tests/test_bundle.py`, beside the H226 mixed-bundle pin). Precondition: H226 (the reviewable lists, **shipped this run**). | → cap 9, cap 7 |
| H234 | **The scoped `index` `_Fidelity:_` holdings diverge honestly from `doctor --source <S>`'s `custody.tiers` *under truncation* — the boundary/complement of H229.** H229 ties the *untruncated* scoped leanest-tier counts to the canonical scoped audit (they're equal when one query matches all of `<S>`'s held items); but the `_Fidelity:_` line answers *"what does this bundle hold in full"* (the post-cap kept set, `len(items)`), not *"what does this source hold in full"* (the whole-source audit `doctor --source <S>` reports). Pin that boundary: over a mixed-fidelity scope where `<S>`'s matching items exceed the cap, `context --budget index --source <S> --limit k`'s `_Fidelity:_` tier counts sum to `k` (the kept slice) while `doctor --source <S>`'s `custody.tiers` sum to the whole source's held count (`> k`), so the two **genuinely differ** — the leanest-tier holdings never silently inflate to a source-wide custody claim the bundle didn't render. The honest divergence is exactly the cap: the bundle line is a *this-bundle* fact, the audit a *whole-source* fact, and an agent must not read one as the other. Mutation-checked: lifting the cap (`--limit` ≥ the source's held count) collapses the divergence back to the H229 equality, so the gap is the truncation, nothing else. Test only (`tests/test_custody_convergence.py`, beside H229). Implementation path: `build_context` passes the post-cap `len(items)` to `render_fidelity_holdings` while `doctor --source` folds `get_fidelity` over the *whole* scoped row set, so the divergence is correct-by-construction; the slice pins the semantic boundary (bundle-kept vs. source-wide) and that it closes exactly when the cap lifts. Precondition: H229 (the untruncated scoped tie), H228 (the scoped+truncated kept-set counts, **shipped this run**). | → cap 1, cap 7, cap 10 |
...
The next lead slot is **H232** (the topmost un-started row), now that **H228 shipped
this run** — the *composition* of H221's truncation axis and H223's facet axis: the
`index` `_Fidelity:_` `(of N)` stays honest when both scope-narrowing filters apply at
once (`context --budget index --source <S> --limit k`), neither silently reverting to a
pre-filter count when the other is active — `(of k)` equals the scoped-*and*-truncated
Coverage `returned`, the tier counts sum to it, and the scoped Coverage denominator
(web's 6, never the library-wide 8) proves the facet stays applied under the cap. H232 is
the *MCP twin* of H228 — the `(of N)` honest under facet scope *and* truncation at once on
the read an agent reaches over MCP, byte-identical to the CLI; H229 ties the scoped
`_Fidelity:_` counts to `doctor --source <S>`'s `custody.tiers` (the scoped completion of
H213's convergence loop), H230 carries H225's orphan-item ids into the *structured* import
summary (`events.orphaned_items`), H231 deepens H224 from tier *counts* to rendered
*bytes* (the byte-identical whole-library rebuild holds across mixed fidelity tiers — the
`partial` scroll's byte-shape, never exercised by the all-`full` fixture), H233 is the
*dedup* half of H226 — the `new`/`held` review lists dedup honestly under within-bundle
duplicate ids, and H234 (appended this run) is the *boundary/complement* of H229 — the
scoped leanest-tier holdings diverge honestly from `doctor --source <S>`'s whole-source
audit *under truncation* (bundle-kept vs. source-wide; the gap is exactly the cap). Per-slice
provenance for every *shipped* slot lives in git (`git log --oneline | grep '(H<NN>)'`);
the **Shipped ledger** below is the one-line in-file index (maintenance-rule §4: *git is the
changelog*).
...
- **Day 3 (2026-06-22 → 2026-06-23):** Portable-bundle round-trip depth. **H216**
  ...
  *truncation*, CLI), **H222** (its MCP twin), **H223** (the *facet*-axis CLI
  ...
  (the composition of both axes) **shipped 2026-06-20**, and its MCP twin **H232**,
  **H229** (the scoped `_Fidelity:_` ≡ `doctor --source` tie), and **H234** (the
  truncated boundary of H229) remain. Re-derive at the H218 checkpoint.
...
- The fresh horizon **budget/tier custody honesty** (H212–H215, H219) is **closed**:
  ...
  **all shipped 2026-06-20**; **H228** (the composition of both axes) **shipped
  2026-06-20**, and **H232**/**H229**/**H234** (its MCP twin, the scoped `_Fidelity:_`
  ≡ `doctor --source` convergence tie, and the truncated boundary of that tie) remain.
