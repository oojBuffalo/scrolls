...
The next lead slot is **H228** (the topmost un-started row), now that **H226 shipped
this run** — the *reviewable* half of H220's dry-run preview: `import bundle --dry-run`
now names *which* scrolls are new vs. already held (sorted, deduped, uncapped `new`/`held`
id lists), so the counts ("2 new") become confirmable ("`new`: which two") before an
operator commits the merge; dry-run-only, the real import stays terse. H228 is the
*composition* of H221's truncation and H223's facet axes (both filters compose without
double-counting), H229 ties the scoped `_Fidelity:_` counts to `doctor --source <S>`'s
`custody.tiers` (the scoped completion of H213's convergence loop), H230 carries H225's
orphan-item ids into the *structured* import summary (`events.orphaned_items`), H231
deepens H224 from tier *counts* to rendered *bytes* (the byte-identical whole-library
rebuild holds across mixed fidelity tiers — the `partial` scroll's byte-shape, never
exercised by the all-`full` fixture), H232 is the MCP twin of H228 — the `(of N)` honest
under facet scope *and* truncation at once on the read an agent reaches over MCP,
byte-identical to the CLI, and H233 (appended this run) is the *dedup* half of H226 —
the `new`/`held` review lists dedup honestly under within-bundle duplicate ids, never
phantom-inflating beyond the distinct items the bundle holds. Per-slice provenance for
every *shipped* slot lives in git (`git log --oneline | grep '(H<NN>)'`); the **Shipped
...
- **Day 3 (2026-06-22 → 2026-06-23):** Portable-bundle round-trip depth. **H216**
  ...
  *truncation*, CLI), **H222** (its MCP twin), **H223** (the *facet*-axis CLI
  ...
  (the composition of both axes), its MCP twin **H232**, and **H229** (the scoped
  `_Fidelity:_` ≡ `doctor --source` tie) remain. Re-derive at the H218 checkpoint.
...
- The fresh horizon **budget/tier custody honesty** (H212–H215, H219) is **closed**:
  ...
  **all shipped 2026-06-20**; **H228**/**H232**/**H229** (the composition of both axes,
  its MCP twin, and the scoped `_Fidelity:_` ≡ `doctor --source` convergence tie) remain.
