...
| H28 | **`scrolls facets method` — the facets leg of the classification-method parity.** `facets` aggregates the library by `sources`/`categories`/`tags`/`concepts`/`fidelity` (`facets.py`, derived-tier counts like `_fidelity_counts`). Add a `method` field: counts of *held* items by how their category was produced — `rules-v1` (optionally split by `basis`), `llm-v1`, `user-set` (a category present with no `classified_by` stamp), and `unclassified` (no category) — the aggregate counterpart of H20/H26's per-item `classification` view, scope-honest like the other facets (counts over the same `where`/`params`). Completes the "search ≡ list ≡ MCP ≡ facets" parity H21 names, on the *aggregate* axis. Tests pin the buckets (rules/llm/user-set/unclassified) + CLI/MCP echo. | → cap 8, cap 7 |
