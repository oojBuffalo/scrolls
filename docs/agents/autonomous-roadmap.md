...
- **New theme opened — conflict-on-import is surfaced, never silently swallowed
  ...
  **CLI-only** (a custody-changing write is an explicit operator act, not an ambient MCP
  ...
  MCP `reconcile` twin remain deferred.**
...
held copy, recording a `resolved` event that supersedes the open conflict; ADR 0105). **The
active capability leads are now H279** (the small, clean adjacent read — a conflict scalar on
`scrolls status`'s JSON `custody_snapshot`, the status-JSON counterpart of H277's readable
line) **then H278** (the heavy one — `--accept-incoming`, the content-bearing supersession that
adopts the peer's capture; the first import-path write that changes a held capture, needs an
ADR for the prior-content archive). Take H279 first (cheap, correct-by-construction, closes the
last conflict *read* gap); reach for the de-prioritized budget/tier guards H244–H249 only when
no capability slice is ready, and prefer closing one rather than appending more of the same shape.

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H279 | **A conflict scalar on `scrolls status`'s JSON `custody` snapshot (`custody_snapshot`) — the JSON-status counterpart of H277's readable `_Conflicts:_` line.** H277 deliberately scoped the readable conflict count to the three Markdown briefings (`export bundle` ×2 + `context`); `scrolls status` renders no readable `_Attention:_`/`_At-risk work:_` lines, so it carries the *machine* custody snapshot instead (the `custody_snapshot` block: tiers, drift, at_risk). It does **not** yet carry an import-conflict count, so an agent reading `status` for a custody dashboard sees drift and at-risk-works but not unresolved import conflicts. This slice folds the *same* `unresolved_conflicts` over `latest_conflict_events` `doctor`'s `custody.conflicts` and the `_Conflicts:_` line read into a `status` scalar (e.g. `custody_snapshot.conflicts` = the unresolved count, source-scopable for free like the drift scalar since a held item owns a source), converging field-for-field with `doctor`'s `custody.conflicts.items` by construction. A pure ledger fold, no schema change, no extra read beyond the one `latest_conflict_events` query; the MCP `get_status` twin (if one exists) carries it for free. Test only-ish (`tests/test_cli.py`, beside the existing `_cmd_status` tests: the scalar surfaces an unresolved conflict + clears after `reconcile` + converges with `doctor` + source-scopes + honest 0 on a clean library). Precondition: H275 (the `doctor` aggregate + primitives, shipped), H276 (so the clears-after-reconcile assertion is meaningful, shipped). | → cap 10, cap 1 |
| H278 | **`--accept-incoming` — the content-bearing reconcile resolution that *adopts* the peer's capture (ADR 0105's deferred slice; the first import-path write that changes a held capture).** H276 shipped `reconcile --keep-held` (affirm the held copy) but deferred adopting the incoming capture because the conflict event records only the incoming **hash**, never the incoming **content** — the importer discarded the bytes (the held copy was never overwritten). So adopting requires **re-supplying** the content. The decisive design question this slice resolves: *where does the incoming content come from, and how is the prior capture kept recoverable?* The likely shape is an **import-path** variant — `scrolls import bundle/items <file> --accept-incoming` (the content is in hand at merge time) — that, *only on explicit instruction*, replaces the held content with the incoming one while (a) recording a new `superseded` ledger event distinct from `resolved` (custody §2.4 — the prior `content_hash` is preserved on the event), and (b) keeping the **prior content itself recoverable** (the "re-capture-on-accept" ADR 0098 deferred — likely a prior-content archive row, *not* an overwrite-in-place). Opt-in, dry-run-able (`--dry-run` predicts the held→incoming transition, the H245/H273 discipline), idempotent (re-accepting an already-adopted capture is a no-op — the held hash now equals the incoming, the H275 hash gate already clears it). Heaviest slice of the theme; needs an ADR for the archive shape. Precondition: H276 (the `resolved` event + reconcile command, shipped), ADR 0105, ADR 0098. | → cap 1, cap 3 |
...
| H256 | **Buffer refresh checkpoint** (maintenance rule). *Serviced 2026-06-22* (across the H272–H277 runs and the H276 run): the conflict-on-import / reconciliation theme is now **closed on the safe direction across all three legs** — detect (H272 `import items`, H273 `import bundle`, H274 the recorded conflict event, ADR 0104), read (H275 `doctor`'s `custody.conflicts` + MCP twin, H277 the `_Conflicts:_` briefing line on both bundle forms + `context`), and **resolve** (H276, `reconcile --keep-held` — affirm the held copy, recording a `resolved` event that supersedes the open conflict; ADR 0105). The queue was re-derived from ADR 0105's deferred section + the H256 adjacent-read note: **H279** (a conflict scalar on `scrolls status`'s JSON `custody_snapshot`, the status-JSON counterpart of H277's readable line — the clean small lead) and **H278** (`--accept-incoming`, the content-bearing supersession that adopts the peer's capture — the heavy one, needs an ADR for the prior-content archive). So ≥6 un-started slots are held (H279, H278, H244–H249). **Still due at the next checkpoint:** a full 3-day/week-plan re-derivation with absolute dates re-confirmed against `docs/product/mvp.md` once H278 ships (the first import-path *write* that can change a held capture — the heaviest slice of the theme) or when the queue drops below ~6 un-started capability slots. Bi-temporal drift framing stays deferred unless an agent workflow shows the H274 event record insufficient. | maintenance |

The next lead slot is **H279** — a conflict scalar on `scrolls status`'s JSON
`custody_snapshot`, the status-JSON counterpart of H277's readable `_Conflicts:_` line and the
last conflict *read* gap — then **H278** (`--accept-incoming`, the heavy content-bearing
supersession). The **conflict-on-import / reconciliation theme is now closed on the safe
direction across all three legs**: detect (H272 `import items`, H273 `import bundle`, H274 the
recorded conflict event, ADR 0104) → read (H275 `doctor`'s scope-level `custody.conflicts`
aggregate + the MCP `get_library_health` twin, H277 the `_Conflicts:_` briefing line on both
`export bundle` forms + `scrolls context`) → **resolve** (H276, `scrolls reconcile <id>
--keep-held` — affirm the held copy, ADR 0105). `doctor` folds the latest recorded conflict-axis
...
partition). Take H279 first (cheap, closes the last conflict read gap), then H278; reach for a
guard cell only when no capability is ready. Per-slice
