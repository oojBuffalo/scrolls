...
The next lead slot is **H233** (the topmost un-started row), now that **H231 shipped
this run** — the *byte-depth* sibling of H224: the whole-library backup's byte-identical
rebuild now holds across mixed fidelity tiers, not only the all-`full` `_seed_items`, so a
`partial` capture's `content_hash`-less scroll (a different frontmatter byte-shape than a
`full` scroll) is proven to rebuild byte-for-byte, deepening H224's tier-*count* guarantee to
the rendered *bytes*. H233 is the *dedup* half of
...
their agreement under partitioned-out orphans is a genuine cross-implementation guarantee), and
H238 (appended this run) is the *bundle*-surface corner of the H216/H224/H231 round-trip matrix
