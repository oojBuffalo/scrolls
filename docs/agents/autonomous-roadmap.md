...
The next lead slot is **H234** (the topmost un-started row), now that **H233 shipped
this run** — the *dedup* half of H226: the dry-run `new`/`held` review lists name each
distinct id exactly once even when a corrupt/spliced bundle repeats a would-be-new id and an
already-held id in its custody block (`parse_bundle` appends every record without deduping, so
the dups reach the preview), while `imported`/`skipped` keep counting the raw occurrences —
exactly what a real `INSERT OR IGNORE` import reports. H234 is
...
byte-for-byte too, the bundle envelope the whole-library byte-identity tests never exercise, and
H239 (appended this run) is the *completeness complement* of H233 — the dry-run `new`/`held`
lists *partition* the bundle's distinct ids (union = the distinct set, intersection empty), so
no id the merge touches is invisible to review or counted in both lists.
