...
| H7 (next) | **M2 enforce on `context` + `related` + `works`.** Extend the same `--stats`/scope companion: `related` echoes the anchor + limit + truncation; `works` already carries `stats` (confirm/extend scope honesty); `context` already titles its facets (confirm + mark). Tests in the matching suites; flip the G2 marker for these surfaces. | → M2, cap 7 |
