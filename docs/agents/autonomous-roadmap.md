...
- **Work-level custody consolidation — a work has an aggregate custody posture *and*
  ...
  *consolidation-loss-over-time* leg** — the at-risk-works count now rides the
  ...
  filter, alarm, browse, stats, trend, briefing, *and* the static compiled library. The
  remaining adjacent consolidation reads are H270 (per-work custody marker on the compiled
  `works.md` rollup) and H271 (the at-risk line on the HTML `export bundle` form).
...
| --- | --- | --- |
| H270 | **The compiled `works.md` rollup page carries a per-work *aggregate custody marker* — the last un-marked compiled surface for work-level custody, the works-page analogue of the per-item fidelity/drift markers on the list pages.** `works.md` renders each `## <doi>` section with the resolver link, a representation count, and every rep as a bullet (H269-era), but carries **no work-level custody verdict** — a human browsing the works rollup can't see which works are safely held vs. at risk without opening `scrolls works` JSON. This emits one line per `## <doi>` section from the shared `works.work_custody` fold (H261) — e.g. `_Custody: best held <tier>, safest drift <posture> — at risk._` (or `— safely held.` when a full+unmoved rep exists) — beneath the resolver line, inside the page's `@generated` sentinel fence (M1, ADR 0102). Converges with `scrolls works`'s per-work `custody` block by construction (same `work_custody`/`SAFE_DRIFT_POSTURES` primitive, the H261 fold), and an at-risk section's marker agrees with whether `index.md`'s at-risk line / `doctor`'s `custody.works` names it. So the compiled works page reads the same per-work verdict an agent reads from `scrolls works`. Tests in `tests/test_kb.py` / `tests/test_custody_convergence.py`. Precondition: H261 (`work_custody`/`SAFE_DRIFT_POSTURES`), H269 (the at-risk-works compiled-surface precedent), H1/M1 (the sentinel-fence regeneration). | → cap 1, cap 2, cap 6 |
| H271 | **The `_At-risk work:_` line rides the *HTML* `export bundle` form too — the HTML twin of H264's Markdown at-risk line, closing the bundle's two-form parity for the consolidation alarm.** H264 added the readable work-level `_At-risk work:_` line to the Markdown bundle (`build_bundle`) but **not** its HTML twin (`build_bundle_html`): the HTML form renders the custody headline, the source `_attention_html` pointer, `_refresh_html`, and `_by_source_html`, but no work-level at-risk paragraph — so the shareable HTML briefing silently drops the consolidation alarm the Markdown form carries. This adds a `custody-attention`-styled `<p class="custody-at-risk">` (a new amber/red rule beside `.custody-attention`/`.custody-refresh`) beneath the headline, grouped with `_attention_html` and above `_refresh_html`, distilled from the *same* `works.at_risk_signal`/`render_at_risk_works` fold so it names the same work the Markdown line / `doctor`'s `custody.works` do (the `_attention_html`↔`render_custody_attention` HTML-twin precedent, H39). Honest absence (no paragraph) when no multi-rep work in scope is at risk, the `_attention_html` no-op shape. Export-only, never in the lossless JSONL fence. Tests in `tests/test_bundle.py` (beside the HTML attention/refresh twins). Precondition: H264 (the Markdown `render_at_risk_works` line + `at_risk_signal`), H39 (the HTML bundle form + its `_attention_html`/`_refresh_html` twins). | → cap 9, cap 1 |
...
The next lead slot is **H270** — the compiled `works.md` rollup page carries a per-work
*aggregate custody marker* (the works-page analogue of the per-item fidelity/drift
markers on the list pages, folded from the shared `works.work_custody`, H261), the last
un-marked compiled surface for work-level custody. **H271** then closes the bundle's
two-form parity — the `_At-risk work:_` line on the *HTML* `export bundle` form
(`build_bundle_html`), the HTML twin of H264's Markdown line (H264 left the HTML form
without it). **With H269 (this run) the consolidation theme is closed across *every*
surface** — work-level custody is readable (H261), filterable (H262), alarmed (H263
JSON), readable-briefing (H264), browsable (H265), stats-summarized (H266),
trend-over-time (H267), readable-trend (H268), *and* on the static compiled `index.md`
(H269): a human browsing the compiled library now reads the same consolidation alarm an
agent reads from `doctor`, inside the sentinel fence and converging with `doctor`'s
`custody.works` by construction. H270/H271 are the remaining *adjacent* consolidation
reads (the works rollup page; the HTML bundle twin); take them before the guard cells.
The **budget/tier convergence cells H244–H249** sit below as **de-prioritized but valid
regression guards** — each a correct-by-construction cell of the cross-tier × CLI/MCP ×
...
slices (H270, H271) first; reach for a guard cell only when no capability is ready.
