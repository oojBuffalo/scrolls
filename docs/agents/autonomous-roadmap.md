...
wall-clock). **H377 is next** (the `scrolls maintain` trend-log determinism + no-movement settle guard);
H378–H381 are queued behind it, and **H382** (the MCP `get_context_bundle` whole-payload determinism twin
of H376) was appended this run to keep the buffer ≥6.
...
the eighth pivot cell — the audit trust-root axis), and **H376 shipped this run** (the `scrolls
context` bundle determinism guard, the ninth pivot cell — the agent-read-surface axis); the
H377 maintain-trend-settle cell is next, and H378–H382 are queued behind it.
...
— take the next one whose preconditions are met (H377 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H377 | **`scrolls maintain` trend-log determinism + no-movement settle guard — pins that the custody trend log (the durable record of custody movement across runs) is a *stable read*: a fixed log re-reads to the same `compute_trend` (no iteration-order leak), AND a second `maintain` pass with no intervening DB change records a snapshot whose comparable custody scalars equal the prior pass's — so the trend reports a true *no-movement* window, never spurious drift. The tenth **forward-hardening cell** — the *cross-run maintenance-ledger* sibling of H365's repair convergence and H375's audit determinism, on the trend axis; deliberately *not* another content-identity guard. → cap 1.** The `maintain` snapshot/trend (H36/H115/H372) is what an unattended worker reads to decide "is custody degrading?"; every movement clause (coverage, posture, at-risk, conflicts, duplicates deltas) assumes a clean library settles to a no-op delta, but that contract is only *implied* by the single-pass tests: a non-deterministic snapshot fold (an unordered scalar map) or a phantom delta on an unchanged library would pass the per-axis trend tests yet make the worker cry drift where there is none. Add a section to `tests/test_maintain.py`: seed the H367 non-vacuous mix, run `maintain` once (record snapshot S1), run it again with no DB mutation (snapshot S2), assert the comparable scalars of S1 ≡ S2 and that `compute_delta(S1, S2)` is the all-zero/no-movement delta on every axis; and assert `compute_trend` over a fixed two-entry log is invariant across two reads (no dict/set-iteration leak). **Decisive choice:** assert *both* the snapshot-scalar settle (no phantom movement) and the trend read-stability (no iteration leak) — a naïve "delta is zero" check misses an unordered map that re-reads differently. **Sabotage check:** a `maintain` snapshot field folded over a `set` (re-ordering between passes) must fail the settle, and injecting a phantom +1 into a delta axis must fail the no-movement assertion. Test-only, no production change (snapshots are pure folds of the deterministic `run_doctor` report; this makes the cross-run settle regression-proof). Precondition: `scrolls maintain`/`run_maintain`/`compute_delta`/`compute_trend` (H36/H115/H372), the H367 non-vacuous custody fixture, H375's doctor-determinism guard (the snapshot's upstream). | → cap 1 |
