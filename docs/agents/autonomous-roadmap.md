...
value, driven off a `_BrowseFilter` registry held to the live `search`/`list` argparse optionals (suite
**4280 passed**; see the Shipped ledger). The forward buffer's **contract-consolidation** theme now leads
with **H404** (compiled-page custody-honesty), with **H405–H411** the remaining consolidation cells.
...
and completeness-honesty — the PRD success metrics). Take the next one whose preconditions are met (H404
next; H403 browse-filter drill shipped — see the Status snapshot / Shipped ledger):
...
that auto-covers new surfaces (the H388 pattern). Nine cells have shipped (H388 MCP-determinism, H394
...
briefing-parity); the forward queue is H402–H407. No new adapters.
...
- **Day 2 (2026-06-28): H402** — reconcile-safety (ADR 0102, the *removal*-axis sibling of H396): a stale
  compiled page with a `@user` annotation is tombstoned (not deleted), an un-annotated stale page is
  removed, across every reconcilable dir-kind keyed to `kb._GENERATED_DIRS`. → cap 6. Preconditions met
  (`_reconcile_generated`/`_STALE_BODY`, the H396 `_GENERATED_ARTIFACT_KINDS` fixture). If it lands fast,
  pull **H403** (browse-filter drill) forward.
- **Day 3 (2026-06-29 → 2026-06-30): H403–H404** — the browse-filter drill contract (every `--fidelity`/
  `--drift`/`--strength`/`--content-duplicate` filter selects exactly the unfiltered rows at that axis
  value, keyed to a `_BROWSE_FILTERS` registry) then the compiled-page custody-honesty contract (M2, the
  compiled-surface sibling of H398). → cap 3 / cap 2. Close a de-prioritized guard (H244–H249) only if
  nothing better is ready. Next once-per-24h full re-derivation **due 2026-06-28**.
