...
The next lead slot is **H244** (the topmost un-started row; H250/H251 shipped out-of-queue capability,
not these test-only cells) — the *both-axes-corrupt* analogue of H220's whole-summary byte-identity: the dry-run's
...
**This run shipped H251 — `scrolls search --fidelity <tier>` + MCP `search_scrolls(fidelity=)`, the
holdings-axis filter on the *ranked* surface and the search twin of H250's `list --fidelity`.** It is the
capability the H250 steering note named first (`search --fidelity` parity): with H250 you could *enumerate*
a fidelity tier via `list`, but a *search* for a topic couldn't scope to "only the full-fidelity matches I
can re-derive offline" (vision §3, fidelity travels with every result). The one design subtlety that makes
it a genuine slice, not a copy of H250: `list` has no cap (post-SQL Python sieve), but `search` applies a
ranked `LIMIT`, so the tier must scope the *ranked* selection — it rides a new `scrolls_fidelity` SQL UDF
(registered in `items.register_facet_functions`, delegating to `fidelity_tier` so the rule keeps one home;
fed the presence booleans + stage, never the body text) ANDed into both the ranked `_QUERY` (before LIMIT)
and `_COUNT_QUERY` via a shared `_search_filters` helper, so the top-k is the top full-fidelity matches and
the `--stats` truncation denominator counts only the kept tier (never inflated by tiers it never showed,
G2). 15 new tests (`test_search.py` ×8, `test_cli.py` ×5, `test_mcp.py` ×2; full suite **3310 passed**),
docs in `docs/cli.md`. Taken out of queue order (alongside the still-un-taken test-only cells H244–H249),
following the H250 steering note's recommendation to prefer genuine capability over another
convergence-matrix pin. **Steering note for the next run:** the natural next holdings-axis capabilities are
a `--fidelity`-scoped *act* surface (`verify --fidelity <tier>` — re-check only your most fragile
reference-only holdings; or `maintain --fidelity`) and the `search --fidelity` `--stats` `custody` tally's
cross-surface convergence; the H244–H249 cells remain valid regression guards a future run may still take,
and the **H218 checkpoint** (full buffer refresh against `docs/product/mvp.md`) is overdue and a good next
pick.
