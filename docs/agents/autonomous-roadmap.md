...
| --- | --- | --- |
| H292 | **The decide→restore recovery loop survives a machine handoff — `archive diff` → `archive restore` works end-to-end on a library rebuilt from a `export bundle --with-archive`, narrated in `docs/dogfood.md`.** H291 (test) pins the archive read-*family* identical post-rebuild; the dogfood layer is the *operator workflow*: someone hands you a `--with-archive` bundle, you `import bundle` into a fresh library, then *recover* on it — read `archive diff` to decide, run `archive restore` to act, the held copy flips to the chosen prior with `doctor`'s `custody.score` holding at 100. The "take it with me" guarantee (M4/cap 4) extended from "the archive travels" (H280) to "the whole recovery *workflow* travels": you can decide-and-restore on a machine that never saw the original adoptions. A new `docs/dogfood.md` section + a `tests/test_dogfood.py` test (the H287/H289 analogue across the bundle boundary). Precondition: H280 (the archive travels, shipped), H285/H286/H288 (the reads, shipped), H291 (the read-family round-trip tie, **shipped 2026-06-23**). | → cap 4, cap 1 |
| H293 | **A `doctor` archive-integrity check — every archived prior's recorded `prior_hash` equals its snapshot's `content_hash`, report-only.** The archive records `prior_hash` (the advertised fingerprint `archive list`/`select_archived_snapshot --hash` key on) *separately* from the `snapshot` body (which carries its own `content_hash`); at archival time they agree by construction, but a corrupt/hand-edited bundle or a bad `import archive` could land a row where they diverge — and then `archive restore --hash <prior_hash>` silently adopts content with a *different* hash than advertised, a custody-honesty bug invisible until restore. Add a report-only `doctor` check (`custody.archive`, never `issues`/exit code — the archive is a recovery convenience, not the root of trust, ADR 0106; the H263 at-risk-works report-view precedent) that folds over `archived_records` and flags any row whose `prior_hash != snapshot.content_hash`, with the offending `{item_id, prior_hash, snapshot_hash}` and **no fabricated repair command** (no auto-rewrite of the archive — the suggested-block orphan discipline). Pure fold, no schema change; a NULL `prior_hash` is *not* a defect (vacuously skipped, the `import_archive` NULL-safe precedent — document in the commit). Precondition: H280 (the archive is a first-class store, shipped), H285 (the snapshot read, shipped). | → cap 6, cap 1 |
| H294 | **The whole-library JSONL backup round-trips the recovery read-family — `archive show --all`/`diff`/`restore --dry-run` read identically on a library rebuilt from `export items` + `export archive`, the H291 twin on the backup (not bundle) path.** H291 proves the *portable bundle*'s fenced `--with-archive` block carries the whole family; the whole-library `export archive` JSONL (`dump_archive_export`) is a *different* serialization with its own `import archive` restore, and no test pins that the recovery family survives *that* round-trip. Pin it: adopt a multi-supersession chain in A, `export items` + `export archive` to two files, `import items` + `import archive` into fresh B, then assert `archive show --all` (byte-for-byte), `archive diff --hash/--at/default`, and `archive restore --dry-run` agree field-for-field across A and B (the H291 assertion shape on the JSONL-backup path). Correct-by-construction (the same content-determined ordering H291 relies on), so a regression guard; mutation-checked by truncating the `export archive` stream. Test-only (beside the existing `export/import archive` tests). Precondition: H291 (the bundle-path family tie, **shipped 2026-06-23**), the `export archive`/`import archive` commands (shipped). | → cap 4 |
...
recovery dogfood (H292), the `doctor` archive-integrity check (H293), and the
JSONL-backup round-trip twin (H294), then pure hardening/integration, above the 6
