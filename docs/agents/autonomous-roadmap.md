...
- **New theme opened — conflict-on-import is surfaced, never silently swallowed
  ...
  **With H274 the conflict-on-import *detection* leg is closed; the next legs are the scope
  read (H275, `doctor`'s `custody.conflicts`) and the operator act (H276, a reviewed
  `reconcile` resolution) — the detection→read→resolve arc.**
...
over taking them. **The active capability lead is now H275** — the scope-level conflict
*read* (`doctor`'s `custody.conflicts`) after H274 (this run) recorded the conflict *event*
and closed the conflict-on-import theme's **detection** leg (H272 opened it on `import items`,
H273 lifted it to `import bundle`, H274 made the divergence a durable, queryable ledger event,
ADR 0104); the theme's remaining legs are the scope read (H275) and the operator act (H276,
a reviewed `reconcile`). Reach for the de-prioritized H244–H249 only when no capability slice
is ready, and prefer closing one rather than appending more of the same shape.

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H275 | **A `doctor` scope-level *conflict aggregate* — `custody.conflicts`, the read-aggregate sibling of `custody.drift` (ADR 0104's first deferred read).** H274 (shipped) records each import conflict as a per-item ledger event queryable via `scrolls history <id> --status conflict`, but there is **no library-scope view**: an operator who merged several peer bundles cannot ask "how many of my held items carry an unresolved import conflict, and which?" without scanning every item's history. This slice adds a `doctor` `custody.conflicts` block — `{items, events}` (or `{unresolved, events}`) — folding the ledger's `conflict` events for *currently-held* items the way `custody.drift` folds the latest verify verdict (the held-filter discipline ADR 0098 fixed). The decision the slice resolves: is a conflict **unresolved** purely by existence of a `conflict` event, or only while the *latest* such event's `observed_hash` still disagrees with the held copy's current `content_hash` (so a later identical re-import, or a re-capture that adopted the incoming content, *clears* it)? Resolve against ADR 0104 (the conflict is detection, the held copy is never auto-overwritten) and the `latest_events` held-filter precedent; keep it a **report view**, never `issues`/`fixed`/the exit code (doctor cannot repair a divergence it must not silently rewrite). Surface it on `doctor` first; the MCP `get_library_health` twin and a readable `_Conflicts:_` briefing line follow as adjacent reads. Precondition: H274 (the recorded conflict event, **shipped this run**), ADR 0104. | → cap 1, cap 3, cap 9 |
| H276 | **A reviewed `reconcile` resolution for a recorded conflict — the *act* on H274/H275's detection (ADR 0104's second deferred slice; `docs/reconciliation.md` "choose a winner, record the supersession").** Detection (H272–H274) and the scope read (H275) surface a divergence but never resolve it; the held copy is always kept. This slice adds the first **explicit, operator-driven** resolution — `scrolls reconcile <id> --accept-incoming` / `--keep-held` (names TBD) — that, *only on explicit instruction*, adopts the incoming capture (re-render from it, recording a supersession event) or affirms the held copy (recording the conflict as resolved-in-favor-of-held). The decision the slice resolves: how is a supersession recorded so the *prior* capture is never destroyed (custody §2.4 — raw is sacred)? Almost certainly a new ledger event type (`superseded`/`resolved`) plus, for `--accept-incoming`, a custody-safe content replacement that keeps the prior content recoverable (the "re-capture-on-accept" ADR 0098 deferred, now demanded). This is the first import-path *write* that can change a held capture, so it must be opt-in, dry-run-able (`--dry-run` predicts the effect, the H245/H273 discipline), and idempotent. Precondition: H274 (the recorded conflict event, **shipped this run**), H275 (the scope-level conflict read), ADR 0104. | → cap 1, cap 3 |
...
| H256 | **Buffer refresh checkpoint** (maintenance rule). *Serviced 2026-06-22* (across the H272/H273/H274 runs): the conflict-on-import / reconciliation-detection theme's **detection** leg is now closed (H272 `import items`, H273 `import bundle`, H274 the recorded conflict event, ADR 0104), and the next capability slices are re-derived from ADR 0104's deferred section — **H275** (`doctor`'s scope-level `custody.conflicts` read) and **H276** (a reviewed `reconcile` resolution: the detection→read→resolve arc), the two new lead capability slots ahead of the de-prioritized guard cells H244–H249. So ≥6 un-started slots are held (H275, H276, H244–H249). **Still due at the next checkpoint:** a full 3-day/week-plan re-derivation with absolute dates re-confirmed against `docs/product/mvp.md` once H276 ships (the first import-path *write* that can change a held capture — the heaviest slice of the theme) or when the queue drops below ~6 un-started capability slots. Bi-temporal drift framing stays deferred unless an agent workflow shows the H274 event record insufficient. | maintenance |

The next lead slot is **H275** — a `doctor` scope-level *conflict aggregate*
(`custody.conflicts`, the read-aggregate sibling of `custody.drift`), the first read after
H274 lifted a surfaced conflict into a recorded custody *event*. The **conflict-on-import /
reconciliation-detection theme's *detection* leg is now closed** across all three slices
(H256 re-derived it from `docs/product/mvp.md` / `docs/product/prd.md`: with the per-item and
consolidation custody surfaces closed, the un-worked custody shape was the **import
boundary**, where two libraries' captures collide — custody §2.4 / the obsidian reconcile
adoption *detect, surface, don't rewrite*). H272 closed the lossless whole-library importer
(`import items`); H273 lifted the same `_merge_items`/`_warn_conflicts` to `import bundle` and
added the read-only twin `_preview_merge_items` predicting the conflict set in its `--dry-run`;
**H274 (this run) lifted a surfaced conflict into a recorded custody *event*** (ADR 0104) —
a typed `conflict` ledger row queryable via `scrolls history <id> --status conflict`, a
*distinct provenance-of-divergence axis* deliberately excluded from the drift posture
(`latest_events` reads only the verify verdicts), recorded by the shared `cli._merge_items` so
both lossless importers get it while the read-only `_preview_merge_items` records nothing. The
theme's next legs are the scope **read** (H275, `doctor`'s `custody.conflicts`) and the
operator **act** (H276, a reviewed `reconcile` resolution) — the detection→read→resolve arc.
The prior **consolidation theme (H261–H271) is closed across *every* surface** — work-level
...
partition). Take H275 first; reach for a guard cell only when no capability is ready. Per-slice
...
**This run (2026-06-22) shipped H274** — a surfaced import conflict is now a **recorded custody
