...
- **Day 2 (2026-06-23, pulled forward):** **Done — the readable archive-integrity surface
  (H298, then H299) shipped this same day.** **H298** — `custody.archive`'s `mismatched`
  count got a readable `maintain`/`status` headline (`archive_integrity_headline`, the
  `conflicts_headline`/H277 `_Conflicts:_` analogue on the archive axis); the readable count
  ≡ `doctor`'s `custody.archive.mismatched` ≡ the `status` scalar (the H277/H279 honesty
  tie). Then **H299 (this run)** — the same alarm's cross-run delta/trend in `maintain`
  (`▲N`/`▼N`, the H283 conflicts-trend analogue): `compute_delta` subtracts the
  `archive_mismatched` scalar, `compute_trend` differences it (`archive_mismatched_change`),
  and `archive_integrity_headline` embeds the signed movement, so successive maintenance
  passes show new corruption (▲) / a repaired backup (▼ — including a fall to zero, the one
  clean state still surfaced), not just the current count. Reported, never a posture trigger
  (the archive is a recovery convenience, not the root of trust). Suite **3776 passed**.
- **Day 3 (2026-06-23 → 2026-06-26):** **Per-item integration → the `export archive` scope family
  (all done) → the `--since` hardening trio → the due full re-derivation.** **H300–H303 (done)**
  completed **`export archive`'s scope family to match its `export events` sibling** (the recovery
  store had scoped only by `--id`; its ledger twin carries the whole `--source`/`--fidelity`/
  `--drift`/`--since` sieve, H260): H300 (`--id` round-trips the recovery read-family), H301
  (`--source`), H302 (`--fidelity`/`--drift`, **closing the custody-filter family across every export
  surface**), H303 (`--since`, the incremental backup). Then **H304 (done this run)** — the
  cross-command `--since` boundary-contract consistency tie: `history`/`export events`/`export archive`
  all normalize through the one `parse_since` validator and agree on the inclusive `>=` window edge
  (one item carrying a `checked_at` row *and* an `archived_at` prior at the same instant T;
  `test_since_family_agrees_on_the_window_edge`, test-only, correct-by-construction, suite
  **3798 passed**). The forward work is the rest of the `--since` hardening trio — **H305** (the
  windowed `export archive --since` byte-stable round-trip) and **H306** (the incremental-backup
  workflow dogfood). The **once-per-24h full 3-day/week re-derivation (last done post-H288) is due
  2026-06-25** — perform it then: mark done, prune stale, re-derive against `docs/product/mvp.md`.
  Close a de-prioritized guard (H244–H249) only if no capability slice is ready, and prefer closing
  one over appending more of the same combinatorial shape.
...
- **This week's horizon — the readable archive-integrity surface (now closed), then hardening.**
  The JSON-only H293 check left the *readable* axis open; **H298** (`custody.archive`'s
  `mismatched` gets a readable `maintain`/`status` headline, the `conflicts_headline`/H277
  analogue) and **H299** (its cross-run trend, the H283 analogue — `archive_mismatched_change` +
  the `▲`/`▼` clause) both **shipped 2026-06-23**, so the readable archive-integrity surface is
  complete. **H300** (scoped `export archive --id` round-trip, the H294 twin on the per-item path)
  **shipped 2026-06-23** too. The forward lead is the **`export archive` scope family** — **H301**
  (`--source`), **H302** (`--fidelity`/`--drift`), **H303** (`--since`) — completing the recovery
  store's scope coverage to match its `export events` ledger sibling (H260); after that the forward
  work is pure **hardening/integration**, not a new theme: full-suite reliability, doctor/repair
  depth, export/import round-trip edges, and MCP/search/list/filter consistency across the archive
