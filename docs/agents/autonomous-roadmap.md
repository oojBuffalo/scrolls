...
| H31 | **`scrolls kb --stale` — refresh summaries whose members changed (the summary-axis counterpart of H27).** Once H29 surfaces stale concept summaries (stored `members_hash` ≠ the live members), add the explicit on-request regeneration: re-synthesize exactly the concepts doctor flags stale, refreshing the summary + `members_hash` to the live members — mirroring `classify --stale` on the classification axis (one shared stale-summary predicate behind doctor's report and the refresh, so the count converges). Regeneration *on request*, never doctor's silent overwrite (custody §2.4). Closes the summary-axis loop H29 (record/report) → H31 (refresh). Precondition: H29 shipped. Tests pin selection (stale picked, fresh/never-summarized untouched) + the re-synthesis. | → cap 8, cap 1 |
