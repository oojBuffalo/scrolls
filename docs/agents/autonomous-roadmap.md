...
The next lead slot is **H229** (the topmost un-started row), now that **H232 shipped
this run** — the *MCP twin* of H228: the `index` `_Fidelity:_` `(of N)` stays honest on the
read an agent reaches over MCP when both scope-narrowing filters apply at once
(`get_context_bundle(query, budget="index", source=<S>, limit=k)`), neither silently
reverting to a pre-filter count when the other is active, and byte-identical to the CLI's
`context --budget index --source <S> --limit k`. H229 ties the scoped `index` `_Fidelity:_`
counts to `doctor --source <S>`'s `custody.tiers` (the scoped completion of H213's convergence
loop), H230 carries H225's orphan-item ids into the *structured* import summary
(`events.orphaned_items`), H231 deepens H224 from tier *counts* to rendered *bytes* (the
byte-identical whole-library rebuild holds across mixed fidelity tiers — the `partial`
scroll's byte-shape, never exercised by the all-`full` fixture), H233 is the *dedup* half of
H226 — the `new`/`held` review lists dedup honestly under within-bundle duplicate ids, H234 is
the *boundary/complement* of H229 — the scoped leanest-tier holdings diverge honestly from
`doctor --source <S>`'s whole-source audit *under truncation* (bundle-kept vs. source-wide;
the gap is exactly the cap), and H235 (appended this run) is the *MCP twin* of H229 — the
scoped `index` `_Fidelity:_` counts ≡ `get_library_health(source=<S>)`'s `custody.tiers` on
the read pair an agent reaches over MCP. Per-slice provenance for every *shipped* slot lives
in git (`git log --oneline | grep '(H<NN>)'`); the **Shipped ledger** below is the one-line
...
- **Day 3 (2026-06-22 → 2026-06-23):** Portable-bundle round-trip depth. **H216**
  ...
  **H229** (the scoped `_Fidelity:_` ≡ `doctor --source` tie), **H234** (the truncated
  boundary of H229), and **H235** (the MCP twin of H229 — `get_context_bundle` ≡
  `get_library_health`, scoped) remain. Re-derive at the H218 checkpoint.
...
- The fresh horizon **budget/tier custody honesty** (H212–H215, H219) is **closed**:
  ...
  **all shipped 2026-06-20**; **H228** (the composition of both axes) and **H232** (its
  ...
  tie), **H234** (the truncated boundary of that tie), and **H235** (the MCP twin of H229 —
  `get_context_bundle` ≡ `get_library_health`, scoped) remain.
