...
| H426 | Native X bookmarks collection on-ramp (production, `src/scrolls/x_bookmarks.py` / `x_session.py` / `x_graphql.py`) — `scrolls sync x --bookmarks` pulls the collection over the browser's own session cookies against X's internal GraphQL endpoint (ADR 0108, superseding 0009; issue #6), reading Chrome/Brave/Arc/Edge/Vivaldi/Chromium/Firefox profiles or `SCROLLS_X_AUTH_TOKEN`/`SCROLLS_X_CT0`; ids stay `x:<tweetId>` so a pull dedupes against `add`, `saved_at` falls back to sync time with `provenance.saved_at_source` saying so, a rotated query id is never reported as an empty collection, and **no `x` fetch adapter exists** so `verify` fails honestly instead of returning a false clean verdict | cap 4 |
