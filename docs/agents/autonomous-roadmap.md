...
| H6 (next) | **M2 G2 enforce on `search` + `list`.** Add the scope echo + explicit truncation marker (the `stats`-style companion the spec defines) so a result over a filtered/capped slice never implies library-wide completeness; pin with `tests/test_search.py` / `tests/test_cli.py` and flip the spec's G2 status marker for these two surfaces. | → M2, cap 7 |
...
- **Day 1 (2026-06-16):** M1 (refresh-safe generated artifacts, ADR 0102)
  **complete** — shipped for compiled `library/` pages (H1+H2) and generated
  ...
  (H5); G2 (scope echo + truncation) enforcement begins at H6.
