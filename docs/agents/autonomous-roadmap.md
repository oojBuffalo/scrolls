...
| --- | --- | --- |
| H243 | **The dry-run preview's *whole top-level summary* — `{imported, skipped, items, events}` — is byte-identical to the real import's over a bundle corrupt on *both* axes at once: within-bundle item duplicates *and* orphan events. The union of H233 (item-dedup vs. a real import) and H237 (the whole `events` block vs. a real import) on one bundle, proving the two corruption axes don't cross-contaminate.** H233 pins the dry-run's item-level `{imported, skipped}` equal to a real import's under within-bundle *item* dups (empty events); H237 pins the whole `events` block equal under orphan *events* (a single, distinct item). Neither exercises a bundle corrupt on *both* axes simultaneously, where the item-dedup path (`new_ids`/`held_ids` sets vs. `insert_item`'s INSERT OR IGNORE) and the event-accounting path (`partition_resolvable_events` + `preview_import_events`/`import_events`) both run over the one parsed bundle. The gap matters because a naive implementation could let one corruption axis perturb the other's count — an orphan-event partition that miscounts items, or an item-dedup that drops a resolvable event. Pin it: over a spliced bundle whose items block *repeats* a would-be-new id and an already-held id *and* whose events block carries both anchored (with a within-bundle dup → a skipped resolvable event) and orphan events, dry-run first (writes nothing) then real import into the same library, and assert the dry-run's entire summary sans the dry-run-only `{dry_run, new, held}` equals the real import's entire summary — so neither corruption axis silently distorts the other's accounting (the corrupt-bundle analogue of H220's *whole-summary* byte-identity, which H237 proved only for the `events` sub-block). Implementation path: a combined items-list + events helper (extending `_spliced_bundle`/`_items_only_bundle`); the item dedup is the set-based `new_ids`/`held_ids` (read-only twin of INSERT OR IGNORE) and the event split is `partition_resolvable_events` + the `preview_import_events`/`import_events` twins, two independent folds over the one parsed bundle, so the no-cross-talk is correct-by-construction; the slice pins it on the both-axes-corrupt bundle neither H233 nor H237 exercises. Test only (`tests/test_bundle.py`, beside H237/H233). Precondition: H237 (the events-block byte-identity, **shipped this run**), H233 (the item-dedup vs.-real), H220 (the clean whole-summary byte-identity). | → cap 9, cap 7 |
...
The next lead slot is **H243** (the topmost un-started row), now that **H242 shipped
this run** — the *MCP twin of H240* (and the truncated boundary of H241): the whole MCP budget ladder
stays mutually equal under truncation while *together* diverging from `get_library_health(source=<S>)`'s
`tiers`, reconverging to the H241/H235 scoped MCP equality when the cap lifts, completing the
leanest/cross-tier × untruncated/truncated × CLI/MCP fidelity-convergence matrix (on the scoped axis).
H243 is the
*both-axes-corrupt* analogue of H220's whole-summary byte-identity — the dry-run's entire top-level
summary equals the real import's over a bundle corrupt on *both* the item-dedup *and* orphan-event
axes at once, proving the two corruption axes don't cross-contaminate (the union of H233 and H237 on
one bundle), H244 is the *bundle-artifact* analogue of
...
between them, and H248 (appended this run)
...
CLI/MCP × scoped/unscoped fidelity-convergence matrix.
...
**This run shipped H242** — the *MCP twin of H240* (and the truncated boundary of H241), completing the
leanest/cross-tier × untruncated/truncated × CLI/MCP fidelity-convergence matrix on the scoped axis
