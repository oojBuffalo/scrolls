...
- **Work-level custody consolidation — a work has an aggregate custody posture *and*
  ...
  *same* dict `scrolls works`'s per-work `custody` block carries — so the compiled marker and
  ...
  change, no extra ledger read. The last remaining adjacent consolidation read is H271 (the
  `_At-risk work:_` line on the *HTML* `export bundle` form, the HTML twin of H264's Markdown
  line).
...
The next lead slot is **H271** — the `_At-risk work:_` line on the *HTML* `export bundle`
form (`build_bundle_html`), the HTML twin of H264's Markdown line (H264 left the HTML form
without it), closing the bundle's two-form parity for the consolidation alarm. **With H270
(this run) the consolidation theme is closed across *every* surface** — work-level custody
is readable (H261), filterable (H262), alarmed (H263 JSON), readable-briefing (H264),
browsable (H265), stats-summarized (H266), trend-over-time (H267), readable-trend (H268),
on the static compiled `index.md` (H269), *and* per-work on the compiled `works.md` rollup
(H270): a human browsing the works rollup now reads the same per-work `custody` verdict an
agent reads from `scrolls works`, inside the sentinel fence and converging with it by
construction. H271 is the last *adjacent* consolidation read (the HTML bundle twin); take
it before the guard cells. The **budget/tier convergence cells H244–H249** sit below as
**de-prioritized but valid regression guards** — each a correct-by-construction cell of the
cross-tier × CLI/MCP × scoped/unscoped × untruncated/truncated fidelity matrix (H244
bundle-artifact reproducibility, H245 dry-run per-id write prediction, H246 unscoped CLI
ladder-under-truncation, H247 unscoped MCP four-way tie, H248 unscoped MCP
ladder-under-truncation, H249 both-axes three-way id partition). Take the capability slice
(H271) first; reach for a guard cell only when no capability is ready. Per-slice provenance
...
**This run (2026-06-22) shipped H270** — the compiled `works.md` rollup now carries a
