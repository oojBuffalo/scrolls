...
| --- | --- | --- |
| H303 | **`export archive --since <ISO>` — the incremental recovery-store backup, the `export events --since` analogue.** The last scope of the `export events` family the recovery store lacks: only priors archived at/after a timestamp, for an incremental off-machine backup since the last sweep, re-importing the overlapping union staying idempotent (the archive dedups by `(item_id, prior_hash)`, ADR 0106). A *time* scope on `archived_at` (not an item-set sieve — composes orthogonally with `--source`/`--fidelity`/`--drift`): filter `archived_records` to `archived_at >= since`. Test: a `--since` mid-chain backup carries only the later priors, and re-importing the full backup then the incremental one yields the same archive as importing the full backup alone (idempotent union). Implementation (`items.py` `archived_records` gains a `since` arg; `cli.py`; `tests/test_cli.py`). Precondition: H280 (`export archive`), H301 (the scope-resolution scaffold). | → cap 4, cap 8 |
