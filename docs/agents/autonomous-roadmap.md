...
guard cells), not a new theme. **H363 shipped** (this run): the in-place `kb`
recompile-determinism guard — a same-process two-pass whole-tree-hash no-op *plus* a
cross-`PYTHONHASHSEED` subprocess pair, so a timestamp/counter leak, a reconcile that
rewrites unchanged pages, *or* an unsorted page fold all fail the guard (the latter only
the cross-seed pair can see). H364–H368 + the new H374 import-axis cell remain un-started.
...
**H363 shipped this run** (the first pivot cell, `kb` recompile-determinism); H364–H368 +
the new H374 remain un-started.
...
— take the next one whose preconditions are met (H364 next):
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  two-pass whole-tree-hash no-op plus a cross-`PYTHONHASHSEED` subprocess pair). Remaining,
  un-started: **H364** (MCP holdings-immutability), **H365** (`doctor --fix` convergence),
  **H366** (`context --budget` nesting), **H367** (`status`↔`doctor` scalars), **H368**
  (`export bundle` determinism), and the new **H374** (`import bundle` re-import idempotency,
  the import-axis sibling). Prefer H364 next. Close a de-prioritized guard (H244–H249) only if
  nothing better is ready. The next once-per-24h full re-derivation is **due 2026-06-26**.
