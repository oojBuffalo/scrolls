...
| --- | --- | --- |
| H241 | **The scoped `index` `_Fidelity:_` counts ≡ the scoped `connected`/`full` `_Custody:_` headline's `fidelity` section, both read over MCP — the cross-*tier* MCP convergence under scope, the MCP twin of H213's deeper-tier tie.** H235 ties the scoped `index` line to `get_library_health(source=<S>)` *across tools* over MCP and H227 ties it to the *CLI*, but no test pins that the *deeper budget tiers* an agent boots over MCP agree with the leanest one *over MCP*: `get_context_bundle(query, budget="connected", source=<S>)`'s `_Custody:_` headline renders its `fidelity` section via `render_custody_headline` (a **different** function than the `index` line's `render_fidelity_holdings`), so their agreement is a genuine cross-rendering guarantee, not the same code twice. Pin it: over a scoped, untruncated mixed-fidelity library, `get_context_bundle(query, budget="index", source=<S>)`'s `_Fidelity:_` tier counts equal the `fidelity` section of the same query's `budget="connected"` *and* `budget="full"` `_Custody:_` headline (the `index` line still carries no drift verdict, the deeper tiers do — the H214/H219 drift-withholding honesty stays intact), because all three fold `get_fidelity` over the same scoped post-facet `items` via the shared `_fidelity_tokens`. So an agent that boots cheap on `index` then deepens to `connected`/`full` over MCP never sees the held-fidelity counts shift under it. Mutation-checked: dropping a `full` item's body shifts the tier on all three MCP tiers in lockstep. Test only (`tests/test_mcp.py`, beside the H214/H227/H235 twins). Implementation path: all three tiers fold the same `custody_counts`/`custody_headline` over the post-facet `items` in `build_context`, so the cross-tier tie is correct-by-construction; the slice pins the MCP twin of H213's deeper-tier convergence under scope. Precondition: H235 (the cross-tool MCP tie, **shipped this run**), H214 (the unscoped MCP index line), H213 (the CLI cross-tier tie). | → cap 1, cap 2, cap 10 |
...
The next lead slot is **H241** (the topmost un-started row), now that **H240 shipped
this run** — the *deeper-tier-headline analogue* of H234: the whole `index`/`connected`/`full` budget
ladder stays mutually equal under truncation while *together* diverging from `doctor --source <S>`,
reconverging to the H213 scoped equality when the cap lifts. H241 is the *cross-tier MCP convergence*
under scope — the scoped `index` `_Fidelity:_` counts ≡ the scoped `connected`/`full` `_Custody:_`
headline's `fidelity` section, both read over MCP (the MCP twin of H213's deeper-tier tie), H242 is the
...
confirms describes the *actual* merge, not merely the right count of rows, and H246 (appended this run)
...
and H240, scoped, leave between them).
...
**This run shipped H240** — the *deeper-tier-headline analogue* of H234's leanest-line boundary
