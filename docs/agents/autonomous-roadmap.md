...
| --- | --- | --- |
| H302 | **`export archive --fidelity`/`--drift` — the custody-filter family (H260) completed on the recovery-store backup, the last un-filtered export surface.** H259/H260 closed `export items`/`export events --fidelity`/`--drift`, but `export archive` was added *later* (H280) and only ever scoped by `--id`, so the family's "closed across every export surface" claim never reached the recovery store. Lift the same item-set sieve: select the items at the custody value (the H259 `list_items` fidelity/drift sieve), then their *whole* recovery store travels (`archived_records(db, those_ids)`) — "back up the recovery history of my drifted items for a recapture handoff." ANDs with `--source` (H301); rejects an unknown value via argparse `choices` (exit 2). Round-trip guard (the H300 shape under a custody scope). Implementation (`cli.py`; `tests/test_cli.py`). Precondition: H301 (the `export archive` scope-resolution scaffold), H260 (the custody-filter family on `export events`). | → cap 4, cap 8 |
