...
| --- | --- | --- |
| H295 | **The archive-integrity alarm *survives the JSONL-backup round-trip* — a corrupt archived prior (`prior_hash != snapshot.content_hash`) carried through `export archive` → `import archive` is flagged *identically* by `doctor`'s `custody.archive` on the rebuilt library (the H293 × H294 integration tie).** H293 added the report-only archive-integrity check; H294 pins the *clean* recovery family round-trips. The untested guarantee is that a *corruption* is not **laundered** by a backup: an archived prior whose advertised `prior_hash` diverges from its snapshot's `content_hash` (a bit-flip, a tampered/hand-edited store) must still trip the alarm after `export archive` → `import archive`, never read clean on the rebuilt library. It is a genuine custody point (vision §2.4 — provenance/fidelity travel with every result; a backup may not silently repair a corruption it cannot actually fix). Pin it: seed a library whose archive holds one corrupt row (`_tamper_archive`-style `prior_hash` ≠ its snapshot `content_hash`) beside a clean prior, `export archive` → `import archive` into a fresh B (held rows via `export items`/`import items`), then assert `doctor`'s `custody.archive` on A and B name the *same* offending `{item_id, prior_hash, snapshot_hash}` event set and the same `checked`/`mismatched` (the clean row passes on both). Correct-by-construction (`import_archive` stores the snapshot verbatim and dedups by `(item_id, prior_hash)`, never re-deriving `prior_hash`), so a regression guard; mutation-checked by *repairing* the row in the backup before import (the alarm clears on B — the tie is load-bearing). Test-only (`tests/test_doctor.py`, beside the H293 archive-integrity tests). Precondition: H293 (the `custody.archive` check, **shipped 2026-06-23**), H294 (the JSONL-backup round-trip path, **shipped 2026-06-23**). | → cap 4, cap 10 |
| H296 | **The archive-integrity alarm *also travels in the portable `--with-archive` bundle* — a corrupt archived prior carried in the bundle's fenced archive block is flagged identically by `doctor`'s `custody.archive` on the bundle-rebuilt library (the H293 × H291/H280 tie, the bundle-path twin of H295).** H280 makes the prior-content archive travel in the portable bundle; H291 proves the *clean* recovery family round-trips it. The untested cell is whether the integrity *alarm* survives the bundle transport too: a tampered prior re-imported via `import bundle` (`parse_bundle_archive` → `import_archive`) must still be flagged, so the shareable briefing cannot launder a corruption either. Pin it (the H295 assertion shape on the bundle path): seed a corrupt-plus-clean archive in A, `export bundle <Q> --with-archive` → `import bundle` into fresh B, then assert `doctor`'s `custody.archive` names the same offending event set and `checked`/`mismatched` across A and B. Correct-by-construction (`import_archive` is the shared restore behind both transports; the bundle block carries the same `prior_hash`/`snapshot` columns the JSONL does); mutation-checked by repairing the prior before the bundle round-trip. Test-only (`tests/test_bundle.py`, beside the H291 read-family round-trip test). Precondition: H293 (the check), H291 (the bundle read-family round-trip, **shipped 2026-06-23**), H295 (the JSONL-path alarm-survival twin). | → cap 4, cap 9 |
...
## 3-day plan — 2026-06-22 → 2026-06-25

Forward-looking (re-derived at the H256 checkpoint, 2026-06-22). Each day ends on
a committed, tested, clean stopping point; slips roll forward. The MVP M1–M5 and
every post-MVP custody theme are closed, so the forward work is the **archive
read-surface follow-up** (H288, **shipped 2026-06-22**), then its **proof/integration
follow-ups** (H289–H291) and **hardening/integration** (cross-cutting reliability, not
new shapes) — not a fresh theme.

- **Day 1 (2026-06-22):** **Done.** The whole **archive-recovery sub-theme
  (H280–H286)** shipped this day — the prior-content archive travels in the
  portable round-trip (H280), an MCP archive read twin (H281), `archive prune`
  retention (H282), the conflict-over-time leg (H283), the accept-incoming dogfood
  (H284), `archive show --all` full-history read (H285), and `archive restore
  --hash/--at` restore-by-version (H286) — **closing it**. Then **H287 (this run)**
  added the *restore-by-version dogfood* (`tests/test_dogfood.py`): a chain of three
  adoptions over three days, `archive show --all` full-history inspection, then
  roll-back by `--hash` to an intermediate and by `--at` to the original, pinning
  held-flips-to-the-chosen-prior / displaced-copy-archived / score-holds-at-100.
  Narrated in `docs/dogfood.md` with captured CLI output. Full suite **3713 passed**.
  Then **H288 (this day)** — `scrolls archive diff <id> [--hash H | --at ISO]`, the
  "decide before you restore" read: fold the **same** `select_archived_snapshot`
  selector H286 uses against the currently-held copy (`get_item`) and report the
  custody-relevant delta — held↔prior `content_hash`, each side's `fidelity` tier, the
  model-complete `changed_fields` (a new pure `items.diff_snapshot(held, prior)` over
  `item_to_dict`), and `would_restore` (the H286 idempotency predicted *before* the
  write, keyed on `content_hash` like `merge_item`). CLI-only read, no write; exits
  mirror `archive restore`. 11 new tests, sabotage-verified. Full suite **3724 passed**.
- **Day 2 (2026-06-23):** **The H288 proof/integration follow-ups (H289–H291).**
  **H289** — the *decide-before-you-restore* dogfood (the H287 analogue on the H288
  read): read `archive diff` → act `archive restore` → assert the restore's outcome
  matches what the diff predicted, and a second diff reports `would_restore: false`
  (idempotency), narrated in `docs/dogfood.md`. Then **H290** — the cross-command tie
  pinning `archive diff`'s `would_restore` ≡ `archive restore --dry-run`'s `restored`
  for the same id+selector (two reads of one predicted write), and **H291** — the
  archive-read-family round-trip (`archive show --all`/`diff`/`restore` behave
  identically on a library rebuilt from `export bundle --with-archive`, the H280↔H285/
  H286/H288 integration tie).
- **Day 3 (2026-06-24 → 2026-06-25):** **Hardening/integration — the archive-recovery
  hardening trio H292–H294, then its follow-ups.** **H293 (the `doctor` archive-integrity
  check — `custody.archive` flags any prior whose `prior_hash != snapshot.content_hash`,
  report-only)**, **H292 (the cross-machine recovery dogfood — the decide→restore→act
  workflow survives a `--with-archive` bundle handoff, a real recovery write on the rebuilt
  library)**, and **H294 (the whole-library JSONL backup round-trips the recovery read-family
  — `archive show --all`/`diff`/`restore --dry-run` identical across all three selectors on a
  library rebuilt from `export items` + `export archive`, the H291 twin on the backup path)**
  all **shipped 2026-06-23**, closing the trio. The forward hardening/integration work is the
  natural H293×H294 follow-ups: **H295** (the archive-integrity alarm survives the
  JSONL-backup round-trip — a corrupt prior is flagged identically on the rebuilt library,
  never laundered by a backup), **H296** (the same alarm travels in the `--with-archive`
  bundle, the bundle-path twin), and **H297** (the JSONL-backup recovery dogfood, the H292
  twin on the backup path). Close a de-prioritized guard (H244–H249) only if none of these is
  ready, and prefer closing one over appending more of the same combinatorial shape.

---

## Week plan (more tentative) — through 2026-06-29
...
- **This week's horizon — the archive read-surface follow-up shipped, then hardening.**
  **H288** (`archive diff`, decide-before-you-restore — the read that compares the held
  copy against a selected prior before an H286 restore) **shipped 2026-06-22**, so the
  archive read/act/recover surfaces are now complete. The remaining queued capability is
  the H288 **proof/integration follow-ups** (H289 the dogfood, H290 the `diff`↔`restore
  --dry-run` convergence guard, H291 the archive-read-family round-trip); after those the
  forward work is pure **hardening/integration**, not a new theme: full-suite
  reliability, doctor/repair depth, export/import round-trip edges, and MCP/search/list/
  filter consistency across the archive surfaces.
