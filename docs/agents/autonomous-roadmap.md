...
The next lead slot is **H282** (`scrolls archive prune` — a retention act bounding the
append-only archive), then **H284** (the accept-incoming end-to-end dogfood) to close the
**archive-recovery sub-theme**. **H281 (an MCP archive *read* twin — `list_archived`/`get_archived`
...
partition). Take **H282 next** (H281 shipped 2026-06-22 — the MCP archive *read* twin
...
the over-time axis), then **H284** to close the archive-recovery sub-theme; reach
