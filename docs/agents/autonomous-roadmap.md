...
- **New theme opened — conflict-on-import is surfaced, never silently swallowed
  ...
  *read* leg is open on the JSON surface; the next legs are the readable `_Conflicts:_`
  briefing line (H277) and the operator act (H276, a reviewed `reconcile` resolution).**
...
| --- | --- | --- |
| H277 | **A readable `_Conflicts:_` briefing line — the readable completion of H275's JSON conflict aggregate (ADR 0104; the conflict-axis counterpart of the drift custody headline).** H275 (shipped) put the import-conflict aggregate on `doctor`'s `custody.conflicts` JSON read (and the MCP `get_library_health` twin), but the readable briefings an agent skims — `scrolls status`, the shareable `export bundle` (both Markdown and HTML forms), `scrolls context` — carry only the per-item drift/source headlines, never the scope-level conflict count. This slice adds a one-line `_Conflicts: N item(s) carry an unresolved import conflict._` (wording TBD), rendered from the *same* `custody.conflicts` fold so it converges with the JSON block / `doctor` by construction (the `render_at_risk_works` / `render_custody_attention` readable-line precedents). Honest absence (no line) when no held item carries an unresolved conflict — the `_Attention:_` no-op shape. Lives beside the existing custody headline + `_Attention:_`/`_Refresh:_`/`_At-risk work:_` lines, both bundle forms (Markdown `build_bundle` + HTML `build_bundle_html`, the H264/H271 two-form parity), and the `context` briefing. Export-only on the bundle (a derived read view, never inside the lossless `@generated` JSONL fence). No schema change, no extra ledger read (the `verdicts`/conflict events the briefing scope already loads). Precondition: H275 (the JSON conflict aggregate, **shipped this run**), ADR 0104. | → cap 1, cap 3, cap 9 |
...
| H256 | **Buffer refresh checkpoint** (maintenance rule). *Serviced 2026-06-22* (across the H272–H275 runs): the conflict-on-import / reconciliation-detection theme's **detection** leg is closed (H272 `import items`, H273 `import bundle`, H274 the recorded conflict event, ADR 0104) and its **read** leg is open on the JSON surface (H275, `doctor`'s `custody.conflicts` + the MCP twin). The remaining theme legs — re-derived from ADR 0104's deferred section — are **H277** (the readable `_Conflicts:_` briefing line, the readable completion of H275, the new lead) and **H276** (a reviewed `reconcile` resolution: the detection→read→resolve arc's act). So ≥6 un-started slots are held (H277, H276, H244–H249). **Still due at the next checkpoint:** a full 3-day/week-plan re-derivation with absolute dates re-confirmed against `docs/product/mvp.md` once H276 ships (the first import-path *write* that can change a held capture — the heaviest slice of the theme) or when the queue drops below ~6 un-started capability slots. Bi-temporal drift framing stays deferred unless an agent workflow shows the H274 event record insufficient. | maintenance |

The next lead slot is **H277** — a readable `_Conflicts:_` briefing line, the readable
completion of H275's JSON conflict aggregate. The **conflict-on-import /
reconciliation-detection theme's *detection* leg is closed** (H272 `import items`, H273
`import bundle`, H274 the recorded conflict event, ADR 0104), and **H275 (this run) opened
the *read* leg** — a `doctor` scope-level *conflict aggregate* (`custody.conflicts`, the
read-aggregate sibling of `custody.drift`), the first read after H274 lifted a surfaced
conflict into a recorded custody *event*. `doctor` now folds the latest recorded
import-`conflict` event per held item into `{basis, as_of, items, events}` (new primitives
`custody.latest_conflict_events`/`unresolved_conflicts`), counting a conflict **unresolved**
while its latest `observed_hash` still differs from the held copy's `content_hash` (the
resolution-aware predicate — a future `reconcile` that adopts the incoming content clears it
with no special "resolved" event); the two ledger axes stay disjoint by construction
(`latest_conflict_events` reads only `conflict` rows, `latest_events` only the verify
verdicts), held-filtered and `--source`-scopable like the drift block, a report view only,
and the MCP `get_library_health` twin carries it for free. The theme's remaining legs are the
**readable read line** (H277, the `_Conflicts:_` briefing line — the lead) and the operator
**act** (H276, a reviewed `reconcile` resolution) — the detection→read→resolve arc.
...
partition). Take H277 first; reach for a guard cell only when no capability is ready. Per-slice
