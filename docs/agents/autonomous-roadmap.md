...
cross-seed guard). **H382 is next** (the MCP `get_context_bundle` determinism twin); H383–H386 are
queued behind it, and **H387** (the MCP `get_link_graph` whole-payload determinism guard, the fourth
MCP-surface determinism twin — the link-structure read an agent caches, a distinct code path folding
the relatedness engine rather than the `run_doctor` audit) was appended this run to keep the buffer ≥6.
...
subprocess pair byte-identical, riding the H186/H180 registered-twin shape contract); the MCP
`get_context_bundle` determinism twin **H382** is next, and H383–H387 are queued behind it.
...
— take the next one whose preconditions are met (H382 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H382 | **MCP `get_context_bundle` whole-payload determinism guard — pins that the *agent-transport* context bundle (the MCP twin of `scrolls context`, used in the H366 MCP nesting twin) is a *reproducible artifact*: calling `get_context_bundle(query, budget="full")` twice on the same unchanged library yields a byte-identical string, same-process AND across a `PYTHONHASHSEED` subprocess pair. The fifteenth **forward-hardening cell** — the *MCP-surface* sibling of H376's CLI `scrolls context` determinism: H376 pinned the CLI `print(build_context(...))` path, but an agent driving MCP never calls the CLI — it calls `get_context_bundle`, a *separate entry point* (it may re-key/wrap the bundle through the MCP tool envelope, a distinct code path no determinism test pins); deliberately *not* another content-identity guard. → cap 10/1/2.** H376 pins the CLI bundle reproduces and H366 pins the MCP twin *nests*, but neither pins the MCP twin is *reproducible run-to-run*: if `get_context_bundle` re-folds the connected-neighbours or excerpt section over a `set` (or the envelope orders keys by dict iteration of a set-derived map), two MCP calls of one library could disagree byte-for-byte, breaking an agent that caches the bundle and re-fetches to detect custody movement (the H201/H204 MCP-driven dogfood loop). Add a section to `tests/test_context.py` (or `tests/test_mcp.py`): seed the H376 `_seed_context_determinism_mix` wide fixture (eight matches + eight Connected neighbours, so every order-sensitive section is a wide fold), call `get_context_bundle("database", budget="full")` twice in-process and assert byte-identical, then invoke the MCP tool in two subprocesses under different `PYTHONHASHSEED`s and assert identical output. **Decisive choice:** call at `budget="full"` (every order-sensitive section exercised) and include the cross-seed pair — the set-iteration leak a same-process pass misses; assert the H186/H180 MCP shape contract still holds so the determinism guard rides the registered-twin invariant. **Sabotage check:** folding the connected-neighbours section over a `set` in the shared `_connected_lines` must fail the cross-seed byte-identity on the MCP path while the MCP nesting twin (H366) and the per-tool shape tests stay green. Test-only, no production change (the MCP bundle folds the same deterministic `build_context`, no wall-clock). Precondition: MCP `get_context_bundle` (the H366 twin), `build_server`/`mcp_server._TOOLS` (H364), the H376 `_seed_context_determinism_mix` fixture + cross-`PYTHONHASHSEED` subprocess harness, the H186 MCP shape contract. | → cap 10/1/2 |
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  **4201 passed**). **H377–H380 shipped** (the `maintain` trend-log determinism + no-movement
  ...
  Remaining, un-started: **H382** (the MCP `get_context_bundle` determinism twin) — next — then
  **H383–H387** (the MCP `run_maintenance` determinism twin, the `export events` JSONL determinism,
  the `archive show` determinism + restore round-trip, the `import archive` re-import idempotency
  guard, and the MCP `get_link_graph` whole-payload determinism guard appended this run to keep the
  buffer ≥6). Close a de-prioritized guard (H244–H249) only if nothing better is ready.
