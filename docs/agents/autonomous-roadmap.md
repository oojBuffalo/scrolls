...
over taking them. **The active capability lead is now H276** — a reviewed `reconcile`
resolution, the conflict-on-import theme's *act*. The theme's **detection** leg is closed
(H272 `import items`, H273 `import bundle`, H274 the durable, queryable conflict event, ADR
0104) and its **read** leg is closed across both surfaces — JSON (H275, `doctor`'s
`custody.conflicts` + the MCP twin) and readable (H277, the `_Conflicts:_` briefing line on
both bundle forms + `context`); the theme's only remaining leg is the operator act (H276,
...
| H256 | **Buffer refresh checkpoint** (maintenance rule). *Serviced 2026-06-22* (across the H272–H277 runs): the conflict-on-import / reconciliation-detection theme's **detection** leg is closed (H272 `import items`, H273 `import bundle`, H274 the recorded conflict event, ADR 0104) and its **read** leg is now closed across both surfaces — JSON (H275, `doctor`'s `custody.conflicts` + the MCP twin) and **readable** (H277, the `_Conflicts:_` briefing line on both bundle forms + `context`). The only remaining theme leg — re-derived from ADR 0104's deferred section — is **H276** (a reviewed `reconcile` resolution: the detection→read→**resolve** arc's act, the new lead). So ≥6 un-started slots are held (H276, H244–H249). **Still due at the next checkpoint:** a full 3-day/week-plan re-derivation with absolute dates re-confirmed against `docs/product/mvp.md` once H276 ships (the first import-path *write* that can change a held capture — the heaviest slice of the theme) or when the queue drops below ~6 un-started capability slots. A near-term adjacent read still un-queued: a conflict scalar on `scrolls status`'s JSON `custody` snapshot (`custody_snapshot`), the JSON-status counterpart of H277's readable line — H277 deliberately scoped to the three Markdown briefings (status renders no readable `_Attention:_`/`_At-risk work:_` lines), leaving the status-JSON conflict count as a clean follow-up. Bi-temporal drift framing stays deferred unless an agent workflow shows the H274 event record insufficient. | maintenance |

The next lead slot is **H276** — a reviewed `reconcile` resolution for a recorded conflict,
the theme's *act*. The **conflict-on-import / reconciliation-detection theme's *detection*
leg is closed** (H272 `import items`, H273 `import bundle`, H274 the recorded conflict event,
ADR 0104), and its **read leg is now closed across both surfaces** — JSON (H275, `doctor`'s
scope-level `custody.conflicts` aggregate + the MCP `get_library_health` twin) and **readable**
(H277, the `_Conflicts:_` briefing line on both `export bundle` forms + `scrolls context`).
`doctor` folds the latest recorded import-`conflict` event per held item into `{basis, as_of,
items, events}` (primitives `custody.latest_conflict_events`/`unresolved_conflicts`), counting
a conflict **unresolved** while its latest `observed_hash` still differs from the held copy's
`content_hash` (the resolution-aware predicate — a future `reconcile` that adopts the incoming
content clears it with no special "resolved" event); the two ledger axes stay disjoint by
construction (`latest_conflict_events` reads only `conflict` rows, `latest_events` only the
verify verdicts), held-filtered and `--source`-scopable like the drift block, a report view
only. H277's readable line folds the *same* `unresolved_conflicts` through the shared
`custody.render_custody_conflicts`, so the briefing count and the JSON `items` converge by
construction; it names **no command** (the resolution act does not exist yet — the
`at_risk_signal` orphan-command discipline). The theme's only remaining leg is the operator
**act** (H276, a reviewed `reconcile` resolution) — the detection→read→**resolve** arc's close.
...
partition). Take H276 first; reach for a guard cell only when no capability is ready. Per-slice
