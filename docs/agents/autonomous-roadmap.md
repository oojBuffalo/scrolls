...
| H26 | **Classification view on `search` hits (search ≡ list ≡ MCP parity).** Extend H20's derived `classification` view to `search`/`search_scrolls` hits so how a hit's category was produced reads identically on every browse surface — the parity H21's confidence marker assumes. Requires carrying (or re-deriving) provenance onto the search-hit row, which today is built from FTS columns; verify the join stays scope-honest (G2) and the empty/no-method cases omit the key like `list`. Tests pin the echo + omission across CLI and MCP. | → cap 7, cap 8 |
...
- **Day 3 (2026-06-18 → 2026-06-19):** Post-MVP. The cap-8 enrichment
  ...
  **actionable in `doctor`** — the `custody.enrichment` stale-ruleset block
  (H25, shipped). Next: the confidence/recency marker (H21), extend the
  classification view to `search` hits (H26 parity), and act on the H25 signal
  with `classify --stale` (H27). Begin framing the hourly worker as scheduled
