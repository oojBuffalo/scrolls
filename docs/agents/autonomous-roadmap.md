...
| --- | --- | --- |
| H304 | **The `--since` family shares one boundary-normalization contract — `history --since` ≡ `export events --since` ≡ `export archive --since` pick *coherent* windows from the one `parse_since` validator (inclusive `>=`).** H303 added `--since` to `export archive`; the three time-windowed reads (`history` H71, `export events` H75, `export archive` H303) now each normalize their boundary through `parse_since` and compare lexicographically `>= boundary` against a `+00:00` isoformat stamp, but nothing pins that they agree on the *window edge*. Pin it: an item with a `checked_at` ledger row and an `archived_at` prior stamped at the **same** instant T — a `--since T` (exact stamp) includes both the event and the prior (inclusive `>=`) on all three reads, and a `--since` one second after T excludes both. So a maintenance worker using one `--since` value across the backup family windows them coherently (the MVP search/list/filter-consistency theme on the time axis), and a future change to one read's edge cannot silently drift from the others. Test only (`tests/test_cli.py`). Precondition: H303 (`export archive --since`), H75 (`export events --since`), H71 (`history --since`). | → cap 4, cap 10 |
...
- **Day 3 (2026-06-23 → 2026-06-26):** **Per-item integration (done) → the `export archive`
  scope family → the due full re-derivation.** **H300 (done this run)** — scoped
  `export archive --id <ref>` round-trips the recovery read-family identically (the H294 twin
  on the per-item export path: back up *just one item's* recoverable history and prove it reads
  identically on the rebuild, while a sibling item's archive is correctly excluded);
  test-only, correct-by-construction, suite **3778 passed**. The forward work completes
  **`export archive`'s scope family to match its `export events` sibling** (the recovery store
  only scopes by `--id`; its ledger twin carries the whole `--source`/`--fidelity`/`--drift`/
  `--since` sieve, H260): **H301** (`--source`), **H302** (`--fidelity`/`--drift`), **H303**
  (`--since`). The **once-per-24h full 3-day/week re-derivation (last done post-H288) is due
  2026-06-25** — perform it then: mark done, prune stale, re-derive against
  `docs/product/mvp.md`. Close a de-prioritized guard (H244–H249) only if no capability slice
  is ready, and prefer closing one over appending more of the same combinatorial shape.
