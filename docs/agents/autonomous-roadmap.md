...
| --- | --- | --- |
| H301 | **`export archive --source <S>` — the source-scoped recovery-store backup, the `export events --source` analogue on the archive axis.** The recovery store (`export archive`, H280) scopes only by `--id` (one item); its custody-ledger sibling `export events` already carries the whole `--source`/`--fidelity`/`--drift`/`--since` item-set sieve (H260), but the recovery-store export never got it — so an operator can't "back up *one source's* recoverable history." Add `--source`: resolve the source to its held item ids (the `list_items(source=S)` enumeration `export events --source` folds), then `archived_records(db, those_ids)` (already list-typed). `--source` and `--id` are independent single-scope selectors — reject supplying both (a loud usage error, exit 2 / stderr JSON), since one names an item and the other a source. Round-trip + exclusion test (the H300 shape, source-scoped): a fresh B rebuilt from `export items` + `export archive --source S` carries exactly source S's items' recovery histories and excludes the others'. Implementation (`cli.py` `_cmd_export_archive` gains a scope-resolution step; `tests/test_cli.py` beside the H300 round-trip); sabotage-verified non-vacuous; docs `docs/cli.md` `export archive`. Precondition: H300 (the per-item round-trip, **shipped this run**), H280 (`export archive`). | → cap 4, cap 8 |
