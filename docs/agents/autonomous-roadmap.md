...
ingest-idempotency triptych after H374/H380); **H387** (the MCP `get_link_graph` whole-payload determinism
guard) is next, and H388–H392 are queued behind it (the latest, **H392** — the MCP `search_scrolls`
whole-payload determinism guard, the agent's primary discovery read — appended this run to keep the buffer ≥6).
...
the single-prior `test_import_archive_is_idempotent` stays green; suite **4216 passed**); **H387** (the MCP
`get_link_graph` whole-payload determinism guard) is next, and H388–H392 are queued behind it (the latest,
**H392** — the MCP `search_scrolls` whole-payload determinism guard — appended this run to keep the buffer ≥6).
...
— take the next one whose preconditions are met (H387 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H387 | **MCP `get_link_graph` whole-payload determinism guard — pins that the *agent-facing* link-structure read (the MCP graph tool, an agent's whole-link-structure view; `get_link_graph` folds nodes + edges + `stats.custody.by_source`) is a *reproducible artifact*: calling `get_link_graph` twice on the same unchanged library yields a byte-identical serialized payload, same-process AND across a `PYTHONHASHSEED` subprocess pair. The twentieth **forward-hardening cell** and the *fourth* MCP-surface determinism twin — after H381 (`get_library_health`), H382 (`get_context_bundle`), and H383 (`run_maintenance`) — on the **link-graph** read an agent caches to detect structural movement; a *distinct code path* (it folds the relatedness/link-graph engine, not the `run_doctor` audit those twins wrap), so no existing determinism test pins it; deliberately *not* another content-identity guard. → cap 3/10.** An agent driving MCP over the H201/H204 dogfood loop caches the link graph and re-fetches to detect "did the library's structure move?"; if `get_link_graph` folds its node list, edge list, or `stats.custody.by_source` over a `set` (or the envelope keys off a set-derived map), two calls of one unchanged library disagree byte-for-byte and the agent cries structural drift where there is none. Add a section to `tests/test_mcp.py`: seed a library whose link graph is wide enough that every order-sensitive section is a multi-element fold (≥3 nodes with ≥1 shared-link edge across ≥2 sources, so `stats.custody.by_source` is a multi-key map), call `get_link_graph` twice in-process and assert the serialized payloads are byte-identical, then invoke the MCP tool in two subprocesses under different `PYTHONHASHSEED`s and assert identical output. **Decisive choice:** serialize and compare the *whole* MCP payload (nodes + edges + stats + envelope), not one field, and include the cross-seed pair — the set-iteration leak a same-process pass misses; assert the H186/H180 MCP shape contract still holds so the determinism guard rides the registered-twin invariant. **Sabotage check:** folding the node or edge list over a `set` in the link-graph path must fail the cross-seed byte-identity while the per-tool `get_link_graph` shape tests stay green. Test-only, no production change (the MCP link graph folds the same deterministic relatedness engine, no wall-clock). Precondition: MCP `get_link_graph` (registered in `mcp_server._TOOLS`), `build_server`/`mcp_server._TOOLS` (H364), the H375/H381 cross-`PYTHONHASHSEED` subprocess harness, the H186 MCP shape contract. | → cap 3/10 |
