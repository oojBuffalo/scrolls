...
`run_doctor` report — ordered folds, no wall-clock). **H378 is next** (the `export bundle --format html`
whole-text determinism guard); H379–H382 are queued behind it, and **H383** (the MCP `run_maintenance`
whole-payload determinism + settle twin of H377) was appended this run to keep the buffer ≥6.
...
cell — the cross-run maintenance-ledger axis); the H378 HTML-bundle determinism cell is next, and
H379–H383 are queued behind it.
...
— take the next one whose preconditions are met (H378 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H378 | **`export bundle --format html` whole-text determinism guard — pins that the browser-readable briefing (H39) is a *reproducible artifact*: exporting the same unchanged library to HTML twice yields a byte-identical document. The eleventh **forward-hardening cell** — the *HTML-render* sibling of H368's Markdown-bundle determinism (a genuinely distinct code path: `build_bundle_html` walks the same scope but emits `html.escape`d `<h1>`/`<h2>`/`<li>` markup and an `_html_document` wrapper the Markdown form never runs); deliberately *not* another content-identity guard. → cap 9.** H368 pins the Markdown bundle reproduces; the HTML form is a separate fold (verified: `bundle.py` carries no `now()`/`generated_at` in either form) that no test pins for whole-document byte-identity — a set-iteration leak in the per-source/duplicates/at-risk HTML sections, or a timestamp slipped into `_html_document`'s `<head>`, would pass the per-section HTML tests (which assert *substrings*, never whole-document identity) yet break the artifact a recipient who re-exports to forward the HTML relies on. Add a section to `tests/test_bundle.py`: build the H368 multi-source + drift-event fixture (`_seed_multi_source`, materialised via `doctor --fix`/`kb`), `export bundle <q> --format html` twice with no DB change, assert the two HTML documents are **byte-identical** (whole-text, the H368 whole-artifact precedent on the HTML axis), and assert non-vacuity (`<h1>` + a per-source `<li>` present). **Decisive choice:** compare the *whole* HTML document, not one tag — non-determinism anywhere (a section, the `<head>`) is caught. **Sabotage check:** a `random.random()` injected into the HTML heading must fail the byte-identity. Test-only, no production change (the HTML bundle is already deterministic — ordered folds, no wall-clock). Precondition: `build_bundle_html`/`export bundle --format html` (H39), the H368 `_seed_multi_source` fixture + byte-identity harness. | → cap 9 |
