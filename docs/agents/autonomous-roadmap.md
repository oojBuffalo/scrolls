...
| --- | --- | --- |
| H289 | **A *decide-before-you-restore* dogfood — read `archive diff`, then act, end to end (the H287 analogue on the H288 read).** H287 dogfoods restore-by-version (roll back by `--hash`/`--at`); it never exercised the **read that precedes the decision** — an operator inspecting `archive diff` to see *what* a restore would change and *whether* it would change anything, then running exactly the restore the diff predicted. A new decide-before-you-restore dogfood in `tests/test_dogfood.py` holds `_held_topic()`, adopts a divergent peer capture (one archived prior), then runs the flow offline: `archive diff <id>` (read the held↔prior `content_hash` + `fidelity` delta, the `changed_fields`, `would_restore: true`) → `archive restore <id>` (act) → assert the restore's `restored`/`held_hash`/`prior_hash` match exactly what the diff predicted (**read-then-act convergence** — the diff's `would_restore` *is* the restore's `restored`, by the shared `select_archived_snapshot` + content-hash compare), and a second `archive diff` against the now-restored prior reports `would_restore: false` + empty `changed_fields` (the idempotency the first diff predicted). Pins that the diff is a true *read* (the held copy untouched between the two diffs until `restore` runs) and that `would_restore` never lies about the subsequent write. Narrated in `docs/dogfood.md` with captured CLI output. Test-only + docs, no production change. Precondition: H288 (`archive diff`, **shipped 2026-06-22**), H286 (`archive restore`). | → cap 1, cap 6 |
| H290 | **`archive diff`'s `would_restore` ≡ `archive restore --dry-run`'s `restored` for the same id+selector — the cross-command write-prediction tie.** H288's `would_restore` and H286's `--dry-run` `restored` both predict the *same* accept-incoming write (would a restore change the held copy?) from the *same* inputs (`select_archived_snapshot` + the `merge_item` content-hash compare), but no test pins that the two commands *agree* — a reader could trust `diff` to decide and `restore --dry-run` to confirm, and a divergence would be a silent custody-honesty bug. Pin it: over `_seed_with_archived_priors`, for each of the three selectors (`--hash`, `--at`, default-latest) and across the would-change and idempotent-no-op cases, assert `archive diff <id> <sel>`'s `would_restore` equals `archive restore <id> <sel> --dry-run`'s `restored`, and that `diff`'s `prior_hash`/`held_hash` equal the dry-run's — two reads of one predicted write, never disagreeing. Correct-by-construction (both fold the one selector + the one content-hash rule), so a regression guard, not new capability; mutation-checked by forcing one side's predicate. Test-only (`tests/test_cli.py`, beside the H288/H286 tests). Precondition: H288, H286 (both **shipped 2026-06-22**). | → cap 1 |
| H291 | **The archive-recovery *read* family survives the lossless round-trip — `archive show --all`/`archive diff`/`archive restore` behave identically on a library rebuilt from a `export bundle --with-archive`.** H280 makes the prior-content archive travel in the portable bundle; H285/H286/H288 read/restore over it. The untested integration tie is that the *whole read/recover family* works post-rebuild: a library rebuilt from a `--with-archive` bundle must expose the *same* archived priors (`archive show --all` byte-identical newest-first), produce the *same* `archive diff` delta (held↔prior hashes, `changed_fields`, `would_restore`), and restore-by-version to the *same* outcome as the source — proving H280 carries enough for the family, not just `archive show`'s head (the H280 round-trip pinned only the latest-prior recovery). Pin it: adopt a multi-supersession chain in source A, `export bundle <Q> --with-archive` → `import bundle` into fresh B, then assert `archive show --all`, `archive diff --hash`, and `archive restore --dry-run` agree field-for-field across A and B. Test-only (`tests/test_bundle.py`, beside the H280 round-trip tests). Precondition: H280 (the archive travels, shipped), H285/H286/H288 (the reads, **shipped 2026-06-22**). | → cap 4, cap 1 |
...
complete; the forward queue is the H288 follow-ups (H289 dogfood, H290 cross-command
convergence, H291 round-trip integration) then hardening, above the 6 de-prioritized
guards H244–H249.**
...
- **Day 3 (2026-06-24 → 2026-06-25):** **A hardening/integration slice** — with the
  capability cushion thin (the H289–H291 follow-ups + the 6 de-prioritized guards
  above the next checkpoint), pick one cross-cutting reliability slice rather than a
  new shape: a full-suite-reliability or doctor/repair deepening, an export/import
  round-trip edge, or an MCP↔CLI / search/list/filter consistency tie that the archive
  surfaces (`archive list`/`show`/`restore`/`diff`) opened. Close a de-prioritized
  guard (H244–H249) only if no such capability is ready, and prefer closing one over
  appending more of the same combinatorial shape.
