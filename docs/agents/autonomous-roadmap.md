...
The next lead slot is **H226** (the topmost un-started row), now that **H227 shipped
this run** — the MCP twin of H223's *facet*-axis `_Fidelity:_` scope-honesty:
`get_context_bundle(query, budget="index", source=<S>)` over a multi-source,
mixed-fidelity library names only the scoped subset's tiers + `(of k)`, byte-identical
to the CLI's `context --budget index --source <S>` (a read-through of `build_context`,
so shared-by-construction). H226 makes H220's preview *reviewable* by naming which
scrolls are new vs. already held, H228 is the *composition* of H221's truncation and
H223's facet axes (both filters compose without double-counting), H229 ties the scoped
`_Fidelity:_` counts to `doctor --source <S>`'s `custody.tiers` (the scoped completion of
H213's convergence loop), H230 carries H225's orphan-item ids into the *structured*
import summary (`events.orphaned_items`), H231 deepens H224 from tier *counts* to
rendered *bytes* (the byte-identical whole-library rebuild holds across mixed fidelity
tiers — the `partial` scroll's byte-shape, never exercised by the all-`full` fixture),
and H232 (appended this run) is the MCP twin of H228 — the `(of N)` honest under facet
scope *and* truncation at once on the read an agent reaches over MCP, byte-identical to
the CLI. Per-slice provenance for every *shipped* slot lives in git (`git log --oneline |
