...
**Active theme — custody posture** (custody-vision §3.1, ADR 0107): `doctor`'s
...
guard). Prefer a genuine capability (H372) first; reach for these when the active theme is
closed:

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H363 | **In-place `kb` recompile-determinism guard — pins that recompiling an *unchanged* library is a byte-identical no-op: two consecutive `scrolls kb` runs over the same DB produce a byte-identical `library/` tree, and the second pass's ADR-0102 `_reconcile_generated` writes/removes nothing (the steady-state reconcile is a true no-op). The first **pivot cell** toward the forward hardening/integration horizon the H358 pivot signal names (full-suite reliability), deliberately *not* another same-shape content-identity guard. → cap 1, cap 9.** Every compiled-surface dogfood/convergence guard (H334 landing line, H349 group page, H351 the compiled loop just shipped) reads compiled files and silently assumes `kb` is deterministic given fixed inputs — but that contract is only *implied*, never pinned directly: `test_roundtrip.py` proves byte-identical *rebuild-from-re-imported-rows in a fresh library* (content round-trips), and the H351 mutation guard recompiles-without-mutation but asserts only that the `_Duplicates:_` line/markers persist, never whole-tree byte-identity or the reconcile no-op. A regression that made `kb` non-deterministic (unsorted iteration, a timestamp leak, a reconcile that rewrites unchanged pages) would pass every existing test yet break the dogfood loops' reproducibility. Add a test to `tests/test_kb.py`: build a small library, `kb`, hash every file under `library/` (sorted relpath → sha256), `kb` again with *no* DB change, assert the per-file hash map is identical (same file set, same bytes) **and** that the second `compile_kb`'s reconcile removed nothing (no file disappeared, none rewritten). **Decisive choice:** hash the *whole tree* (not just `index.md`), so a non-determinism anywhere — a group page, `graph.md`, `works.md` — is caught, not just the landing surface. **Sabotage check:** if `compile_kb`'s page iteration were unsorted (a real determinism hazard), the second-run hashes would diverge for at least one multi-entry page; assert the maps are equal so that drift fails the guard. Test-only, no production change (the determinism already holds — `write_scroll`/`_emit` are field-deterministic and `list_items` is ordered; this makes it regression-proof). Precondition: the existing `tests/test_kb.py` compile harness, ADR 0102 (`_reconcile_generated`). | → cap 1, cap 9 |
| H364 | **MCP holdings-immutability contract guard — pins that the registered MCP tool surface exposes *no* tool that deletes or overwrites a held capture, so an agent driving MCP can never destroy raw. The second **pivot cell** toward the forward MCP-hardening horizon, the custody-safety invariant H355 leaned on (the prune is the shell's explicit `scrolls rm`, never an MCP act) but never pinned directly — the M2 anti-fabrication/tested-contract shape (custody-vision §2.4, raw-is-sacred) lifted to the MCP *tool registry*, not another content-identity guard (per the H358 pivot signal's caution). → cap 7, cap 1.** Every content-identity MCP dogfood/twin (H325/H338/H355) and the read/act boundary `get_library_health` draws against `scrolls doctor --fix` silently assume the MCP transport holds no capture-destroying tool — but that contract is only *implied* by the current `_TOOLS` membership, never pinned: a future tool addition (a convenience `delete_scroll`, an `update_item` that rewrites `raw_text`/`content_hash`) would pass every existing per-tool test yet breach raw-is-sacred over the agent transport, and break H355's "the prune is a CLI choice" premise. Add a registry guard to `tests/test_mcp.py`: assert the names in `mcp_server._TOOLS` (≡ what `build_server` registers) are exactly an **allow-list of read + custody-safe-write tools** — reads (`search_scrolls`/`list_scrolls`/`list_facets`/`get_scroll`/`get_scroll_history`/`get_related_scrolls`/`get_link_graph`/`get_works`/`get_context_bundle`/`get_concept_page`/`get_tag_page`/`list_sources`/`list_archived`/`get_archived`/`get_library_health`/`get_maintenance_history`/`list_feed_subscriptions`) and the custody-safe writes (`ingest_url` adds, `verify_scroll` records a ledger event, `run_maintenance`/`compile_library` regenerate views, `follow_feed`/`unfollow_feed`/`sync_feeds` manage subscriptions) — and that **no registered tool name matches a destroy/overwrite verb** (`delete`/`remove`/`drop`/`purge`/`prune`/`rm`/`overwrite`/`merge` on a *scroll/item/capture*; `unfollow_feed`/`remove_subscription` is a feed-subscription op, not a held-capture op — the allow-list whitelists it explicitly so the verb scan stays a backstop). **Decisive choice:** assert the *exact set* (a new tool must be classified read-or-safe-write to pass), not just the negative verb scan — so adding a capture-mutating tool fails loudly and forces an ADR, the registry-completeness analogue of M2's search-completeness contract. **Sabotage check:** appending a stub `delete_scroll` to `_TOOLS` must fail the guard (both the allow-list mismatch and the verb scan). Test-only, no production change (the registry already holds the line; this makes the agent-transport custody-safety regression-proof). Precondition: H355 (the MCP read/act boundary this pins), the existing `tests/test_mcp.py` harness + `_TOOLS`/`build_server`. | → cap 7, cap 1 |
| H365 | **`doctor --fix` idempotency/convergence guard — pins that repair *settles*: a second `scrolls doctor --fix` over an already-repaired library is a true no-op (`issues == 0`, `fixed == 0`, every finding list — `missing_scrolls`/`missing_media`/`orphan_scrolls`/`duplicates` — empty) and rewrites nothing on disk. The third **forward-hardening cell**, the *repair-convergence* sibling of H363's `kb` recompile-determinism guard (the compile axis) on the **repair axis**, deliberately *not* another content-identity guard (per the H358 pivot signal's caution). → cap 1, cap 9.** Every dogfood/round-trip loop that runs `doctor --fix` as the documented restore step (H336/H360 and the import-axis loops) silently assumes the repair *converges* — re-running it finds nothing more to fix — but that contract is only *implied* by the single-pass `--fix` tests, never pinned: a regression that made `--fix` re-rewrite an unchanged scroll (a non-deterministic render leak), re-report a cleared finding, or oscillate (fix A reopens B) would pass every existing single-pass test yet break the restore step's settle assumption and any loop that re-audits after it. Add a test to `tests/test_doctor.py`: seed a library with real repairable findings (a deleted scroll file → `missing_scrolls`, an FTS desync), `doctor --fix` (assert it fixes them), capture a `{relpath → sha256}` map of `scrolls/`, `doctor --fix` **again** with no intervening mutation, and assert the second pass reports `issues == 0` / `fixed == 0` with every finding list empty **and** the file-hash map is byte-identical (no scroll silently rewritten). **Decisive choice:** assert *both* the report-level no-op (nothing re-found) *and* the disk-level no-op (nothing re-written) — a `--fix` that re-rendered identical bytes would pass the first but waste the custody guarantee the loops lean on; hashing catches it (the H363 whole-tree-hash precedent on the repair axis). **Sabotage check:** if `--fix` re-rewrote a scroll each pass (a render-nondeterminism regression), the second-pass hash map would diverge; assert equality so the drift fails the guard. The content-identity pair must stay a *non*-finding on both passes (`doctor --fix` never merges a content duplicate, H337), so a byte-identical pair in the seed is left held by both passes. Test-only, no production change (the repair already converges — `_fix` is field-deterministic and INSERT OR IGNORE settles; this makes the restore step's settle contract regression-proof). Precondition: the existing `tests/test_doctor.py` `--fix` harness, H337 (the content-duplicate never-merge discipline). | → cap 1, cap 9 |
...
- **Day 2 (2026-06-26):** **The custody-posture theme — trend.** **H372** — the cross-run
  posture-movement clause on the `maintain` `_Posture:_` line (`sound → attention`), threading
  a posture axis through `compute_delta`/`compute_trend`, reported never a posture trigger
  (the `at_risk`/`conflicts` trend precedent). Then **H373** if time — the cross-surface
  posture convergence guard.
- **Day 3 (2026-06-27 → 2026-06-28):** **Close the custody-posture theme, then harden.**
  **H373** (if not pulled into Day 2) — `status` ≡ `doctor` ≡ `maintain` ≡ bundle/context ≡
  MCP on the posture axis. After H372/H373 the posture theme has its read → render → travel →
  trend → convergence legs; the forward work is then the **forward-hardening guard cells**
  (H363–H368: `kb` recompile determinism, MCP holdings-immutability, `doctor --fix`
  convergence, `context --budget` nesting, `status`↔`doctor` scalars, `export bundle`
  determinism) and integration depth — not a new theme. Close a de-prioritized guard
  (H244–H249) only if nothing better is ready. The next once-per-24h full re-derivation is
  **due 2026-06-26**.
...
- **This week's horizon — the custody-posture theme, then hardening.** `doctor`'s
  `custody.posture` distils the seven custody blocks into one whole-library verdict
  (custody-vision §3.1, ADR 0107): H369 (doctor + MCP) and H370 (the `maintain` line + the
  `status` twin) are shipped; **H371** (this run) lifts it to the shareable/agent briefings;
  **H372** (the trend clause) and **H373** (the convergence guard) close it. After the theme
  closes, the forward work is pure **hardening/integration** — the H363–H368 forward-hardening
  guard cells (reproducibility/safety invariants), doctor/repair depth, export/import
  round-trip edges, MCP/search/list/filter consistency — not a new theme. The once-per-24h
  full re-derivation was **performed 2026-06-25** (this run, bundled with the §6 re-compaction);
  the next is **due 2026-06-26**.
