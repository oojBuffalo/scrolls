...
The next lead slot is **H230** (the topmost un-started row), now that **H229 shipped
this run** — the *scoped sibling* of H213's cross-tier convergence: over a multi-source,
mixed-fidelity library where one query matches all of `<S>`'s held items (no truncation),
`context --budget index --source <S>`'s `_Fidelity:_` tier counts equal `doctor --source
<S>`'s `custody.tiers` (non-zero entries), both folding `get_fidelity` over the same scoped
item set, mutation-checked in lockstep — the scoped completion of the H213 convergence loop.
H230 carries H225's orphan-item ids into the *structured* import summary
...
the gap is exactly the cap), H235 is the *MCP twin* of H229 — the scoped `index` `_Fidelity:_`
counts ≡ `get_library_health(source=<S>)`'s `custody.tiers` on the read pair an agent reaches
over MCP, and H236 (appended this run) is the *MCP twin* of H234 — that same scoped divergence
under truncation over MCP, collapsing back to the H235 equality when the cap lifts. Per-slice
provenance for every *shipped* slot lives in git (`git log --oneline | grep '(H<NN>)'`); the
**Shipped ledger** below is the one-line in-file index (maintenance-rule §4: *git is the
changelog*).
