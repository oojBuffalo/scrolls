...
already deterministic). **H380 is next** (the `import events` re-import idempotency guard); H381–H384 are
queued behind it, and **H385** (the `scrolls archive show` JSONL determinism + restore round-trip guard,
the *archive-recovery-transport* sibling of H379 — it folds `dump_items_export` over `item_archive` rows,
not `list_items`, a distinct code path) was appended this run to keep the buffer ≥6.
...
**H379 shipped this run** (the `export items` JSONL whole-file determinism + round-trip byte-identity
guard, the twelfth pivot cell — the whole-library-backup-transport axis); the H380 `import events`
re-import idempotency cell is next, and H381–H385 are queued behind it.
...
— take the next one whose preconditions are met (H380 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H380 | **`import events` re-import idempotency guard — pins that importing the *same* custody-events backup (`scrolls export events` JSONL, ADR 0082/H72) a second time into a library that already holds those events is a true no-op: the `custody_events` row count is unchanged, and every item's `scrolls history` timeline is byte-identical across the two passes (no duplicated ledger rows). The thirteenth **forward-hardening cell** — the *ledger-ingest* sibling of H374's `import bundle` re-import idempotency, on the **events** transport rather than the items transport (`custody.import_events` content-dedups on the `_EVENT_IDENTITY` 5-tuple, a distinct code path from `merge_item`'s `INSERT OR IGNORE`); deliberately *not* another content-identity guard. → cap 9.** H72/H73 pin the events export→import *round-trip* preserves the posture, but the whole-*ledger* idempotency of a re-import into a populated ledger is only *implied*: a regression that dropped the `_EVENT_IDENTITY` dedup (or keyed it on the per-library autoincrement `id`, which is never exported) would pass the single-import round-trip tests yet double every event row on the second pass, inflating the per-item `history` and any count that folds the raw ledger. Add a section to `tests/test_events_export.py` (or `tests/test_roundtrip.py`): seed a library with recorded verify + conflict events, `export events` to a backup, `import events` it twice into a fresh home, and assert the second pass reports `imported == 0`/`skipped == N`, the `custody_events` row count equals the first pass's, and `item_history(id)` is identical list-for-list across the two passes. **Decisive choice:** assert *both* the raw `custody_events` row count (catches a silent double-insert) and the per-item `history` list identity (catches a dedup that drops the wrong row) — a naïve "import reports skipped" check misses a re-keyed dedup that both skips *and* re-inserts. **Sabotage check:** dropping the `_EVENT_IDENTITY` content-dedup (re-keying on the autoincrement `id`) must fail the row-count axis. Test-only, no production change (`import_events` already dedups by content; this makes the events-ingest settle contract regression-proof). Precondition: `custody.import_events`/`dump_events`/`load_events`, `scrolls export events`/`import events` (H72), the `_EVENT_IDENTITY` content key, the `tests/test_events_export.py` harness. | → cap 9 |
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  **4201 passed**). Remaining, un-started: **H377** (`maintain` trend-log determinism +
  no-movement settle) — next — then **H378–H381** (the HTML-bundle / `export items` JSONL
  determinism + `import events` idempotency + the MCP `get_library_health` determinism twin),
  with **H382** (the MCP `get_context_bundle` determinism twin of H376) appended this run to keep
  the buffer full. Close a de-prioritized guard (H244–H249) only if nothing better is ready. The
  2026-06-25 full re-derivation still spans this window; the next once-per-24h full re-derivation
  is **due 2026-06-27** (the 3-day plan boundary).
