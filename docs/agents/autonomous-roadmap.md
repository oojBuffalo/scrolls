...
- **This week's horizon — the readable archive-integrity surface (now closed), then hardening.**
  ...
  complete. The remaining lead is **H300** (scoped `export archive --id` round-trip, the H294 twin
  on the per-item path). After H300 the forward work is pure **hardening/integration**, not a new
  theme: full-suite reliability, doctor/repair depth, export/import round-trip edges, and
  MCP/search/list/filter consistency across the archive surfaces. The once-per-24h full
  re-derivation (last done post-H288) is **due 2026-06-25**.
