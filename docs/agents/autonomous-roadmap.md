...
per-tool shape tests stay green; suite **4218 passed**). **H388** (the MCP `get_works` whole-payload
determinism guard) is next, and H389–H393 are queued behind it (the latest, **H393** — the MCP
`get_concept_page` whole-payload determinism guard — appended this run to keep the buffer ≥6).
...
failing *only* the cross-seed; suite **4218 passed**); **H388** (the MCP `get_works` whole-payload determinism
guard) is next, and H389–H393 are queued behind it (the latest, **H393** — the MCP `get_concept_page`
whole-payload determinism guard — appended this run to keep the buffer ≥6).
...
— take the next one whose preconditions are met (H388 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H388 | **MCP `get_works` whole-payload determinism guard — pins that the *agent-facing* work-consolidation read (the MCP `get_works` tool, an agent's same-work-across-sources view; folds the DOI-clustered `works` list, each work's `representations`/`canonical`/aggregate `custody` block, and the at-risk ordering) is a *reproducible artifact*: calling `get_works()` twice on the same unchanged library yields a byte-identical serialized payload, same-process AND across a `PYTHONHASHSEED` subprocess pair. The twenty-first **forward-hardening cell** and the *fifth* MCP-surface determinism twin — after H381 (`get_library_health`), H382 (`get_context_bundle`), H383 (`run_maintenance`), and H387 (`get_link_graph`) — on the **works** read an agent caches to detect a work crossing into at-risk; a *distinct code path* (it folds the DOI-clustering/work-consolidation engine + the per-work aggregate `custody` verdict, not the `run_doctor` audit or the relatedness link-graph those twins wrap), so no existing determinism test pins it; deliberately *not* another content-identity guard. → cap 1/2.** An agent driving MCP over the H201/H204 dogfood loop caches the works view and re-fetches to detect "did a work move into at-risk?"; if `get_works` folds its works list, a work's `representations`, or the at-risk ordering over a `set` (or the envelope keys off a set-derived map), two calls of one unchanged library disagree byte-for-byte and the agent cries consolidation drift where there is none. Add a section to `tests/test_mcp.py`: seed a library whose works graph is wide enough that every order-sensitive section is a multi-element fold (≥2 DOI-clustered works each with ≥2 representations across ≥2 sources, ≥1 at-risk so `most_at_risk` is non-null), call `get_works()` twice in-process and assert the serialized payloads are byte-identical, then invoke the MCP tool in two subprocesses under different `PYTHONHASHSEED`s and assert identical output. **Decisive choice:** serialize and compare the *whole* MCP payload (works + per-work representations/custody + the at-risk ordering + envelope), not one field, and include the cross-seed pair — the set-iteration leak a same-process pass misses; assert the H186/H180 MCP shape contract still holds so the determinism guard rides the registered-twin invariant. **Sabotage check:** folding the works list or a work's `representations` over a `set` in the consolidation path must fail the cross-seed byte-identity while the per-tool `get_works` shape tests stay green. Test-only, no production change (the MCP works view folds the same deterministic consolidation engine, no wall-clock). Precondition: MCP `get_works` (registered in `mcp_server._TOOLS`, H64/H261), `build_server`/`mcp_server._TOOLS` (H364), the H375/H381 cross-`PYTHONHASHSEED` subprocess harness, the H186 MCP shape contract. | → cap 1/2 |
| H389 | **MCP `get_maintenance_history(trend=True)` whole-payload determinism guard — pins that the *agent-facing* custody-trajectory read (the MCP trend tool, an agent's "is custody improving, holding, or regressing?" view; `get_maintenance_history(trend=True)` folds `read_log` + the per-run `snapshot_headline` + `compute_trend`'s windowed `posture_headline`/`*_change` block) is a *reproducible artifact*: calling it twice on the same unchanged recorded log yields a byte-identical serialized payload, same-process AND across a `PYTHONHASHSEED` subprocess pair. The twenty-second **forward-hardening cell** and the *sixth* MCP-surface determinism twin — after H381 (`get_library_health`), H382 (`get_context_bundle`), H383 (`run_maintenance`), H387 (`get_link_graph`), and H388 (`get_works`) — and the **read sibling of H383**: where `run_maintenance` *runs* one pass and records `{recorded_at, snapshot, delta}`, this *reads* those records back through `compute_trend`, a **distinct code path** (the windowed trend fold + the `posture_headline` reasons join, not the `assemble_report` pass or the `run_doctor` audit those twins wrap), so no existing determinism test pins it; the H377 CLI `maintain --history --trend` cross-seed guard pins the *CLI* read, but an agent driving MCP re-serializes through the tool envelope, a separate entry point. Deliberately *not* another content-identity guard. → cap 1/11.** An agent caching the trajectory to detect a regression reads the *MCP* payload — if `compute_trend`'s windowed `posture_headline` joins its `reasons` over a `set`, or a per-source trend fold leaks set order, two reads of one unchanged log disagree byte-for-byte and the agent cries a phantom trajectory swing. Add a section to `tests/test_mcp.py`: seed a *fixed* multi-entry maintenance log (the H377 `_determinism_trend_snapshot` shape — a `sound` baseline then a degraded `at_risk` window with multiple posture reasons + non-zero drift/at-risk/conflicts, so every windowed fold is non-vacuous), call `get_maintenance_history(trend=True)` twice in-process and assert byte-identical payload, then invoke the MCP tool in two subprocesses under different `PYTHONHASHSEED`s and assert identical output. **Decisive choice:** `trend=True` (the heaviest read — every windowed/posture fold exercised, not the bare log list), compare the *whole* MCP payload, and include the cross-seed pair (the set-iteration leak a same-process pass misses); assert the H186/H180 MCP shape contract still holds so the guard rides the registered-twin invariant. **Sabotage check:** the windowed `posture_headline` reasons join folded over a `set` (the H377 trend-cross-seed sabotage lifted to the MCP surface) fails *only* the cross-seed byte-identity while the per-tool `get_maintenance_history` shape tests stay green. Test-only, no production change (the trend read folds the same deterministic `compute_trend`; the log's `recorded_at` stamps are fixed in the seeded fixture, so there is no wall-clock to mask). Precondition: MCP `get_maintenance_history` (H198), `build_server`/`mcp_server._TOOLS` (H364), the H377 `_determinism_trend_snapshot` fixed-log fixture + the H375/H381 cross-`PYTHONHASHSEED` subprocess harness, the H186 MCP shape contract. | → cap 1/11 |
| H390 | **`export archive` whole-library JSONL determinism + round-trip byte-identity guard — pins that the whole-library prior-content archive backup transport (`scrolls export archive`, ADR 0106/H280 — the recovery-store sibling of `export events`, the JSONL the bundle's archive block also emits) is a *reproducible artifact*: exporting the same unchanged library's archive twice yields a **byte-identical** JSONL, AND a real `export archive` → `import archive` into a fresh `SCROLLS_HOME` → re-`export archive` reproduces the sender's bytes. The twenty-third **forward-hardening cell** and the *third leg of the transport-determinism triptych* — after H379 (`export items`, the held set) and H384 (`export events`, the verify ledger) — on the **archive** transport: `dump_archive_export` folds `archive_export_dict` over the whole `item_archive` table (`items.py`), a **distinct code path** from both siblings and from H385's per-item `archive show <id>` recovery read (which folds `archived_snapshots`/`latest_archived` over one item, `ORDER BY id DESC`); deliberately *not* another content-identity guard. → cap 9.** `dump_archive_export` is the recovery-backup root — both `export archive` and the bundle's archive block emit it (H280) — so its reproducibility is load-bearing, yet the whole-*file* identity is only *implied*: an unsorted `item_archive` fold or a set-iteration leak in a per-row field would pass the H280/H285 restore-outcome tests (which assert the *recovered* prior, not the JSONL bytes) yet make two recovery backups of one library disagree, breaking an operator who `diff`s two machines' archive stores. Add a section to `tests/test_roundtrip.py` (the H384 events sibling lives there): seed ≥2 items each with ≥1 superseded prior (so the archive spans ≥2 items, a multi-prior fold — drive an `add` → re-`add`-with-changed-content adoption, or seed `item_archive` directly), `export archive` twice with no DB change, assert byte-identical JSONL; then a full restore (`import items` + `import archive`, since `export archive` is item-scoped like `export events`) into a fresh home, re-`export archive`, assert it reproduces the sender's bytes. **Decisive choice:** compare the *whole JSONL file*, not one row, and include the round-trip; seed the archive so the per-item priors are appended in a stable order (the H384 chronological-append precedent — `import_archive`'s dedup/insert order must reproduce the sender's `export archive` read order); pair the cross-seed `PYTHONHASHSEED` subprocess face only if a per-row field folds a `set` (else the same-process two-export check suffices — `dump_archive_export` is an ordered fold). **Sabotage check:** a counter appended to one exported archive row must fail the byte-identity. Test-only, no production change (`export archive` is already deterministic — ordered `item_archive` fold, no wall-clock). Precondition: `dump_archive_export`/`archive_export_dict`/`_ARCHIVE_IDENTITY` (items.py), `scrolls export archive`/`import archive` (H280), the `_seed_items`/`_build_library`/`home` harness in `tests/test_roundtrip.py`. | → cap 9 |
| H391 | **MCP `get_archived` whole-payload determinism + CLI-convergence guard — pins that the *agent-facing* archive-recovery read (the MCP recovery tool, the model-complete latest archived prior an agent recovers over MCP; `get_archived` folds `latest_archived` → `item_to_dict` + the tuple→list snapshot normalization through the MCP tool envelope) is a *reproducible artifact*: calling `get_archived(<id>)` twice on the same unchanged archived snapshot yields a **byte-identical** serialized payload, AND that payload converges with the CLI `archive show <id>` head (the H385 single-prior default) — the two recovery surfaces agreeing by construction. The twenty-fourth **forward-hardening cell** and the *MCP-surface recovery-read twin of H385* (the CLI `archive show` determinism), also the *seventh* MCP-surface determinism member after H381/H382/H383/H387/H388/H389: a **distinct code path** (the MCP tool envelope + the `tags`/`concepts`/`links`/`media` tuple→list snapshot normalization `get_archived` adds, not the `run_doctor`/relatedness/consolidation folds the other MCP twins wrap), so no existing determinism test pins it; deliberately *not* another content-identity guard. → cap 9.** An agent driving MCP over the H201/H204 dogfood loop recovers a superseded prior via `get_archived` and may cache it; if the snapshot serialization (or the tuple→list normalization) were per-call non-deterministic, two recovery reads of one unchanged archive would disagree byte-for-byte and an agent comparing the recovered bytes to a CLI `archive show` restore would see a phantom divergence where the two recovery surfaces must agree. Add a section to `tests/test_mcp.py`: seed an item with ≥2 adoptions (so `latest_archived` selects the head of a *multi-row* `item_archive` fold — the order-sensitive path, the H385 `_seed_with_archived_priors` harness), call `get_archived(<id>)` twice in-process and assert the serialized payloads are byte-identical, then assert the payload equals the JSON of the CLI `archive show <id>` head (the H285 `latest_archived ≡ archived_snapshots[0]` convergence lifted to MCP), and assert the H186/H180 MCP shape contract still holds. **Decisive choice:** serialize and compare the *whole* MCP payload (not one field), include the CLI-convergence leg (the recovery surfaces must agree by construction — H385's head identity lifted to the MCP surface), and seed ≥2 adoptions so the `latest_archived` selection is over a real multi-row archive; not cross-seed — `latest_archived` is an `ORDER BY id DESC` fold with no set to scramble (the H385 reasoning on the MCP surface), so the same-process two-read + CLI-convergence check suffices. **Sabotage check:** a counter folded into the snapshot dict in the `get_archived` path must fail the same-process byte-identity. Test-only, no production change (`get_archived` folds the same deterministic `latest_archived`, no wall-clock). Precondition: MCP `get_archived` (registered in `mcp_server._TOOLS`, H281), `_cmd_archive_show`/`latest_archived` (H278/H285, H385 shipped the CLI determinism), the H385 `_seed_with_archived_priors` harness, the H186 MCP shape contract. | → cap 9 |
| H392 | **MCP `search_scrolls` whole-payload determinism guard — pins that the *agent-facing primary discovery read* (the MCP search tool an agent calls first over the H201/H204 dogfood loop; `search_scrolls` folds the ranked match list + each hit's `match_strength` band + the M2 completeness `--stats`/`truncated` envelope through the MCP tool envelope) is a *reproducible artifact*: calling `search_scrolls(<q>)` twice on the same unchanged library yields a **byte-identical** serialized payload, same-process AND across a `PYTHONHASHSEED` subprocess pair. The twenty-fifth **forward-hardening cell** and the *eighth* MCP-surface determinism twin — after H381 (`get_library_health`), H382 (`get_context_bundle`), H383 (`run_maintenance`), H387 (`get_link_graph`), H388 (`get_works`), H389 (`get_maintenance_history`), and H391 (`get_archived`) — on the **search/ranking** read, a *distinct code path* (it folds the `search_items`/`count_matches` ranking + `match_strength` band engine, not the `run_doctor` audit, the `build_context` bundle, the relatedness link-graph, or the consolidation folds the other twins wrap), so no existing determinism test pins it; deliberately *not* another content-identity guard. → cap 7/8.** An agent re-runs the same query to detect "did new matches land?"; if `search_scrolls` folds its ranked hit list, a tie-break group, or the `--stats` per-source completeness map over a `set` (or the envelope keys off a set-derived map), two calls on one unchanged library disagree byte-for-byte and the agent cries result drift where there is none — and a non-deterministic *tie-break* among equal-score hits is the classic latent leak a same-process pass can mask. Add a section to `tests/test_mcp.py`: seed a library wide enough that the query returns a multi-hit ranked list spanning ≥2 sources with ≥1 score-tie pair (so the tie-break order is order-sensitive and `--stats`/`by_source` is a multi-key map), call `search_scrolls(<q>)` (with the completeness envelope on) twice in-process and assert the serialized payloads are byte-identical, then invoke the MCP tool in two subprocesses under different `PYTHONHASHSEED`s and assert identical output. **Decisive choice:** serialize and compare the *whole* MCP payload (ranked hits + per-hit `match_strength` + the `--stats` completeness envelope), not one field, include a deliberate score-tie pair (the tie-break leak a single-hit fixture never exercises), and include the cross-seed pair (the set-iteration leak a same-process pass misses); assert the H186/H180 MCP shape contract still holds so the determinism guard rides the registered-twin invariant. **Sabotage check:** a `set`-fold of the ranked hit list (or a tie-break that reads dict/set iteration order) must fail the cross-seed byte-identity while the per-tool `search_scrolls` shape tests stay green. Test-only, no production change (the MCP search read folds the same deterministic ranking engine, no wall-clock). Precondition: MCP `search_scrolls` (registered in `mcp_server._TOOLS`), `build_server`/`mcp_server._TOOLS` (H364), `search_items`/`count_matches`/`match_strength` (H312–H318), the H375/H381 cross-`PYTHONHASHSEED` subprocess harness, the H186 MCP shape contract. | → cap 7/8 |
| H393 | **MCP `get_concept_page` whole-text determinism guard — pins that the *agent-facing concept-page read* (the MCP tool an agent follows from a hit to read one concept's compiled page; `get_concept_page` folds the rendered Markdown — the concept's member-scroll list + per-member custody markers + the cross-links — through the MCP tool envelope) is a *reproducible artifact*: calling `get_concept_page(<concept>)` twice on the same unchanged library yields a **byte-identical** string, same-process AND across a `PYTHONHASHSEED` subprocess pair. The first **class-D string-twin** determinism cell (the H194 Markdown-string reads, `_STRING_TWINS`) after the eight JSON-twin cells — a *genuinely distinct shape*: where the JSON twins serialize a dict, this emits *rendered Markdown* where a set-leak surfaces as a re-ordered member list or cross-link line, not a re-keyed map; a *distinct code path* (it folds the concept-page render engine — the same `kb`/`render` fold the compiled `library/concepts/<c>.md` page emits — not the `run_doctor` audit, `build_context` bundle, relatedness graph, or ranking engine the JSON twins wrap), so no existing determinism test pins it; the H363 `kb` recompile-determinism guard pins the *whole-tree* on-disk compile, a same-process two-pass + cross-seed whole-tree hash, but not the *per-tool MCP string read* an agent fetches live; deliberately *not* another content-identity guard. → cap 4/6.** An agent that lands on a `search_scrolls` hit follows `get_concept_page` to read the concept's whole context and may cache it to detect "did this concept's membership move?"; if the page render folds its member-scroll list, the per-member custody markers, or the cross-link block over a `set`, two reads of one unchanged library disagree byte-for-byte and the agent cries membership drift where there is none. Add a section to `tests/test_mcp.py`: seed a library wide enough that the concept page is a multi-element render (≥3 member scrolls of one concept across ≥2 sources, ≥1 cross-link to another concept, so the member list and the cross-link block are both multi-element folds), call `get_concept_page(<concept>)` twice in-process and assert the strings are byte-identical, then invoke the MCP tool in two subprocesses under different `PYTHONHASHSEED`s and assert identical output. **Decisive choice:** compare the *whole rendered Markdown string* (member list + custody markers + cross-links), not one line, and include the cross-seed pair — the set-iteration leak a same-process pass misses; assert the H186/H194 MCP string-twin shape contract still holds (a Markdown string, not a JSON envelope) so the determinism guard rides the registered-twin invariant. **Sabotage check:** folding the member-scroll list over a `set` in the concept-page render path must fail the cross-seed byte-identity while the per-tool `get_concept_page` shape tests stay green. Test-only, no production change (the concept page folds the same deterministic render engine, no wall-clock). Precondition: MCP `get_concept_page` (registered in `mcp_server._TOOLS`, `_STRING_TWINS`/H194), `build_server`/`mcp_server._TOOLS` (H364), the concept-page render engine (`kb`/`render`, H363 pins its on-disk compile determinism), the H375/H381 cross-`PYTHONHASHSEED` subprocess harness, the H186/H194 MCP string-twin shape contract. | → cap 4/6 |
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  **both** the single-prior default *and* the `--all` multi-prior fold, the default ≡ the `--all` head, plus
  ...
  items/events/archive ingest-idempotency triptych; suite **4216 passed**).
  Remaining, un-started: **H387** (the MCP `get_link_graph` whole-payload determinism guard) — next — then
  **H388–H392** (the MCP `get_works` and `get_maintenance_history(trend=True)` whole-payload determinism
  guards, the `export archive` whole-library JSONL determinism + round-trip guard, the MCP `get_archived`
  whole-payload determinism + CLI-convergence guard, and the MCP `search_scrolls` whole-payload determinism
  guard appended this run to keep the buffer ≥6). Close a de-prioritized guard (H244–H249) only if nothing
  better is ready. The next once-per-24h full re-derivation is **due 2026-06-27** (the 3-day plan boundary).
