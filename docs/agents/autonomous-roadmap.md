...
| H27 | **`scrolls classify --stale` — act on the H25 signal (regenerate on request).** A targeted batch path that re-runs the rules engine over exactly the items doctor's `custody.enrichment.stale` flags — categories produced under a superseded ruleset — refreshing both `category` and `classified_ruleset` to the live fingerprint. The explicit, user-invoked counterpart to H25's read-only report: regeneration happens *on request*, never as doctor's silent overwrite (custody §2.4). Today `classify` (no id) picks up only *unclassified* items (`category is None`); `--stale` widens batch selection to superseded-fingerprint items without touching current-ruleset or user-set ones. Closes the loop H20 (record) → H25 (report) → H27 (refresh). Tests pin selection (stale picked, current/user-set untouched) + the re-rendered frontmatter sync. | → cap 8, maintenance framing |
...
- **Day 3 (2026-06-18 → 2026-06-19):** Post-MVP. The cap-8 enrichment
  ...
  **actionable in `doctor`** — the `custody.enrichment` stale-ruleset block
  ...
  parity the confidence marker assumes. Next: the confidence/recency marker
  (H21), the facets leg of that parity (H28 — `facets method`), act on the H25
  signal with `classify --stale` (H27), and the summary-axis counterpart (H29).
  Begin framing the hourly worker as scheduled custody maintenance (H22 draft).
