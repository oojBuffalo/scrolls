...
[H220/H226/H245/H273/H353] into one auto-covering matrix). Suite **4339 passed**. **H417 shipped this run**
(the facet scope-composition convergence contract — the twenty-fourth **contract-consolidation** cell, the
*scope*-axis sibling of H411's value-drill [H411 pins the unscoped count↔rows tie; this pins that *scoping* a
facet aggregate equals *aggregating over the scoped drill*]): one completeness-asserted invariant in
`tests/test_facet_scope_composition.py` that for *every* facet dimension and *every* scope facet,
`facets <dim> --<scope> V` reports exactly the dimension recomputed over only the rows `list --<scope> V`
enumerates, plus the two-facet AND case. The **completeness keystone** — `_SCOPE_FACETS` (the scope optionals
shared by the live `facets`/`list` subparsers, each tied to its `items.item_filters` clause) ∪ a named
`_NON_SCOPE_SHARED` (`--limit`, pagination not a filter) is asserted ≡ the live shared optionals, and the
registry's `filter_param`s ≡ `item_filters`' parameters ≡ `compute_facets`' scope kwargs — so a *new* shared
scope optional fails until it declares a composition tie. **Decisive choices:** (1) the oracle reads each
dimension value's whole-library id set once (the H411 `list --<filter>` drill, or — for the no-filter
`method` dimension — the classification marker the `list` rows already carry) and intersects it with the
scope's id set *in Python*, so it never passes two colliding flags (`--source X --source X`) and stays a
distinct code path from `compute_facets`' single scoped fold; (2) the two-facet AND drives a *strict-subset*
intersection (`--stage rendered --source web` ⊊ each single scope) so a clause-ordering/precedence bug is
visible. **Roadmap correction (the H404/H407/H409/H412/H413/H415 precedent):** the spec named
`_CLI_READ_PATHS`/H394 as the completeness mechanism, but the keystone is driven off `_subparser_optionals`
(the H411 introspection helper, reused via the blessed cross-`test_` import) over the live `facets`∩`list`
shared optionals — the tighter source-of-truth than the whole-read registry. The sabotage proves teeth +
isolation: silently dropping the `source` clause from the *facets* `item_filters` binding (an unscoped fold
behind a scoped call) desyncs *all eight* `--source` dimension legs while the other four scope facets stay
green — the `list`-driven oracle keeps its own unpatched `scrolls.items` binding and catches the divergence
(the H411 isolated-counter sabotage on the scope axis). Test-only, no production change (`compute_facets`
already folds the scope through the shared `item_filters`; this pins the "≡" once over the registry). Suite
**4344 passed**. **H418 shipped this run** (the audit-aggregate ↔ browse-drill convergence contract — the
twenty-fifth **contract-consolidation** cell and the *audit*-side sibling of H411's *facet*-side drill:
one completeness-asserted invariant in `tests/test_audit_drill_convergence.py` that every count the
`doctor`/`get_library_health` custody audit reports equals exactly the rows the matching `list` browse
filter enumerates — `tiers[tier]` ≡ `len(list --fidelity tier)`, `drift[posture]` ≡ `len(list --drift
posture)` (the ledger word `unchanged` is the posture `verified`), `enrichment.stale` ≡ `len(list
--stale-classification)`, `content_duplicates.total_items` ≡ `len(list --content-duplicate)` — on both
the CLI `doctor` and the MCP `get_library_health` transports. The **completeness keystone** drives an
`_AUDIT_DRILL` registry (10 drillable count fields) ∪ a named `_NO_DRILL` set (19 non-drillable counts)
to partition *exactly* the integer count leaves walked off a real empty-library custody block, so a *new*
audit count fails until classified. **Roadmap correction (the H404/H407/H409/H412/H413/H415/H416/H417
precedent):** the spec listed `summaries.stale ≡ len(list --stale-summary)` as a fifth tie, but H410
already established `summaries.stale` counts stale *concepts* not members (a different cardinality from the
member items `list --stale-summary` enumerates) — so it is **non-drillable**, the member drill being H410's
job. The sabotage inflates the `full` tier in `doctor._check_custody_integrity` (distinct from `list_items`'
`--fidelity` SQL filter) → failure set is *exactly* `{(doctor, tiers.full), (get_library_health, tiers.full)}`,
never the drill nor its siblings. Test-only, suite **4347 passed** (see the Shipped ledger). **H419 shipped
this run** — the compiled library group-page classification marker, the *production* slice H412 deferred (the
H89/H93 custody-marker precedent on the *enrichment* axis): `kb._classification_marker(item)` now renders
`· classified by \`<engine>\` (<basis|model>) · confidence <level>[, <freshness>]` — the shared
`items.classification_phrase` the bundle briefing (H35), the `context` excerpt tag (H44), and every per-item
JSON read already render — trailing the `_row_markers` custody/content-duplicate clauses on every compiled
`library/` group-page row (source/category/concept/tag), inside the `@generated` fence (ADR 0102) so a
recompile refreshes it after a re-classify and an outside-fence `@user` note survives, omitted on honest
absence (no engine stamp → no clause; `classification_provenance(item) is None`). The phrase trails the
others because it carries a nested ` · confidence` separator and so must run to end-of-row. Two
`tests/test_kb.py` tests pin the render + the H396 regeneration-safety tie (a moved ruleset flips
`current` → `stale` on recompile, the note survives). Then the **H412 contract is promoted**
(`tests/test_provenance_convergence.py`): `get_concept_page`/`get_tag_page` move out of
`_MCP_NO_PROVENANCE_TOOLS` into `_MCP_PROVENANCE_TOOLS` / `_PROVENANCE_SURFACES` as a third (`rendered`)
surface kind — the per-row phrase parsed off the compiled page, compared to `classification_phrase` of the
canonical `classification_provenance` projection over a multi-method fixture (all five method buckets share
one concept + tag so both pages are non-vacuous and enumerate every held item). The partition keystone
still holds (the two tools just moved between the named sets). The page-leg sabotage proves the teeth: a
`kb.classification_provenance` that calls the stale-ruleset item's category fresh desyncs *only*
`{get_concept_page, get_tag_page}` (they share `_row_markers`) while the JSON per-item surfaces — reading
`items.classification_provenance`, a distinct binding — and the `facets` tally stay green; the existing
`list` sabotage still isolates to `{list}`. **Production change** (`kb.py` marker), suite **4350 passed**.
**H420 shipped this run** — the truncation / scope-echo honesty contract: `tests/test_truncation_scope_honesty.py` adds one registry-backed invariant over the G2 stats axis, partitioning live CLI/MCP reads into truncation-envelope surfaces (`search`/`list`/`related`), non-truncating stats objects (`works`/`get_works`), and named no-stats/no-page exemptions (including the H163 array-only MCP browse twins). A non-vacuous fixture drives `limit < matched` on every truncation surface and asserts `stats.matched` is the full pre-cap count, `stats.truncated == (matched > returned)`, and rows are the top-limit prefix; the sabotage makes `search` report page count as matched and fails only that leg. Test-only, suite **4354 passed**.
...
**H425 shipped this run** — compiled group-page custody-marker convergence: `tests/test_group_page_custody_marker_convergence.py` ties the rendered `get_concept_page`/`get_tag_page` group-page row markers to the canonical per-item custody projection (`items.get_fidelity` + `custody.drift_posture`/`last_checked`) read by `list`/`show`/`get_scroll`, reuses H419's rendered page-tool set, and proves a `kb._custody_marker` page-only drift desync fails only the two rendered legs. Test-only, suite **4370 passed**.

The forward-hardening guard-cell sequence is now closed through **H425**. Per the assessment gate, pause before appending more same-shaped guard cells; choose the next recommendation from current product/runtime evidence rather than automatically extending the contract-consolidation treadmill.
...
| H417 | Facet scope-composition convergence contract (test-only) — the *twenty-fourth* contract-consolidation cell and the *scope*-axis sibling of H411's value-drill (H411 pins the unscoped count↔rows tie; this pins that *scoping* a facet aggregate equals *aggregating over the scoped drill*): five `tests/test_facet_scope_composition.py` tests over one completeness-asserted invariant that for *every* facet dimension and *every* scope facet, `facets <dim> --<scope> V` reports exactly the dimension recomputed over only the rows `list --<scope> V` enumerates, plus the two-facet AND case. (1) `test_scope_facet_registry_partitions_the_shared_facets_list_optionals` — the **completeness keystone**: `_SCOPE_FACETS` (the scope optionals shared by the live `facets`/`list` subparsers, via the H411 `_subparser_optionals` introspection helper) ∪ a named `_NON_SCOPE_SHARED` (`--limit`, pagination not a filter) ≡ the live `facets`∩`list` shared optionals, so a *new* shared scope optional fails until it declares a composition tie. (2) `test_each_scope_facet_drives_an_item_filters_clause` — the registry's `filter_param`s ≡ `items.item_filters`' parameters ≡ `compute_facets`' scope kwargs, and each scope facet alone yields exactly one clause naming its own column. (3) `test_every_scoped_facet_aggregates_exactly_the_scoped_rows` over the H411 `_seed_facet_drill_mix` — the oracle reads each dimension value's whole-library id set once (the H411 `list --<filter>` drill, or — for the no-filter `method` dimension — the classification marker the `list` rows already carry) and intersects it with the scope's id set *in Python*, so it never passes two colliding flags (`--source X --source X`) and stays a distinct code path from `compute_facets`' single scoped fold. (4) `test_two_scope_facets_anded_aggregate_the_intersection` — the decisive choice: `--stage rendered --source web` is a *strict* subset of each single-facet scope, so a clause-ordering/precedence bug is visible. (5) `test_dropping_a_scope_clause_desyncs_only_that_scope_facets_legs` — silently dropping the `source` clause from the *facets* `item_filters` binding (an unscoped fold behind a scoped call) desyncs *all eight* `--source` dimension legs while the other four scope facets stay green (the `list`-driven oracle keeps its own unpatched `scrolls.items` binding). **Roadmap correction (the H404/H407/H409/H412/H413/H415 precedent):** the spec named `_CLI_READ_PATHS`/H394 as the completeness mechanism, but the keystone is driven off `_subparser_optionals` over the live `facets`∩`list` shared optionals — the tighter source-of-truth than the whole-read registry. Test-only, no production change (`compute_facets` already folds the scope through the shared `item_filters`; the H411 unscoped drill matrix + the per-facet `test_facets` scope tests stay as deeper regression guards, the H395 discipline); suite **4344 passed** | cap 7 |
| H418 | Audit-aggregate ↔ browse-drill convergence contract (test-only) — the *twenty-fifth* contract-consolidation cell and the *audit*-side sibling of H411's *facet*-side drill (H411 ties the browse aggregate to the drill rows; this ties the `doctor`/`get_library_health` custody aggregate to the same drill rows): three `tests/test_audit_drill_convergence.py` tests over one completeness-asserted invariant that every count the custody audit reports equals exactly the rows the matching `list` browse filter enumerates — `custody.tiers[tier]` ≡ `len(list --fidelity tier)`, `custody.drift[posture]` ≡ `len(list --drift posture)` (the ledger word `unchanged` is the posture `verified`, the one remap), `enrichment.stale` ≡ `len(list --stale-classification)`, `content_duplicates.total_items` ≡ `len(list --content-duplicate)` — on **both** the CLI `doctor` and the MCP `get_library_health` transports, so an agent reading a number off the audit and drilling `list` is promised exactly that many rows. (1) `test_audit_drill_registry_partitions_the_live_custody_count_fields` — the **completeness keystone**: an `_AUDIT_DRILL` registry (10 drillable count fields, each `(audit-path, list-flags)`) ∪ a named `_NO_DRILL` set (19 non-drillable counts) partition *exactly* the integer count leaves walked off a real empty-library `run_doctor` custody block (the structural skeleton, every `by_source`/list empty), so a *new* custody count field fails until it declares a drill tie or is named non-drillable; the non-drillable counts are the rollups/complements with no single `list` filter (the integrity `score`/`issues`, the cross-posture `drift.checked`/`coverage`, the conflict/works/archive aggregates, the classified/current/unfingerprinted enrichment complements, the whole `summaries` block). **Roadmap correction (the H404/H407/H409/H412/H413/H415/H416/H417 precedent):** the H418 spec listed `summaries.stale ≡ len(list --stale-summary)` as a fifth tie, but H410 already established `custody.summaries.stale` counts stale *concepts* not members (`_StaleAudit(count=None)`) while `list --stale-summary` enumerates the member items of those concepts — a different cardinality — so `summaries.stale` is **non-drillable** (the member drill is H410's job). (2) `test_every_audit_count_drills_exactly_its_browse_rows` over a dedicated `_seed_audit_drill_mix` (7 items spanning all three fidelity tiers, all five drift postures, a content-duplicate pair, and a 3-item stale-classification set — every drilled count a non-empty *proper* subset) asserts every (transport, field) count equals the matching `list` drill. (3) Sabotage `test_an_audit_fold_double_counting_one_tier_fails_only_that_field` inflates the `full` tier in `doctor._check_custody_integrity` (the audit's own tier fold, distinct from `list_items`' `--fidelity` SQL filter) → failure set is *exactly* `{(doctor, tiers.full), (get_library_health, tiers.full)}` (both transports share the patched `run_doctor`), never the drill nor its siblings. Test-only, no production change (every audit count already threads one custody primitive per axis; the per-axis convergence guards `test_drift_facet_converges_with_doctor_custody_drift`/`test_content_duplicate_facet_converges_with_doctor_and_the_drill_filter` + H410's stale audit ties stay as deeper regression guards, the H395 discipline); suite **4347 passed** | cap 1/7 |
| H420 | Truncation / scope-echo honesty contract (test-only) — `tests/test_truncation_scope_honesty.py` pins the G2 stats-envelope family as one registry-backed invariant: live CLI/MCP reads are partitioned into truncation surfaces (`search`/`list`/`related`), non-truncating stats objects (`works`/`get_works`), and named no-stats/no-page exemptions; a `limit < matched` fixture asserts `stats.matched` is the full pre-cap count, `stats.truncated == matched > returned`, and rows equal the top-limit prefix, with a page-count-as-matched sabotage isolating to `search` | cap 7 |
...
| H425 | Compiled group-page custody-marker convergence contract — `tests/test_group_page_custody_marker_convergence.py` parses the H89/H93 `· fidelity · drift · when` clause from the rendered `get_concept_page`/`get_tag_page` rows and proves it equals the canonical per-item `(fidelity, drift, last_checked)` projection read by `list`/`show`/`get_scroll`; the registry reuses H419's rendered page legs, spans every fidelity tier × verified/drifted/unverified posture, and a page-only `kb._custody_marker` sabotage isolates to `{get_concept_page, get_tag_page}` | cap 1/9 |
