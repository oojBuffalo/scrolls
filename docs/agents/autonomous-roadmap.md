...
| --- | --- | --- |
| H242 | **The whole budget ladder stays mutually equal under truncation while *together* diverging from `get_library_health(source=<S>)`'s `tiers`, all read over MCP — the MCP twin of H240 (and the truncated boundary of H241), completing the leanest/cross-tier × untruncated/truncated × CLI/MCP fidelity-convergence matrix.** H241 ties `index` ≡ `connected` ≡ `full` over MCP *untruncated* (the deeper-tier cross-rendering tie under scope); H236 pins only the leanest `index` `_Fidelity:_` line diverging from `get_library_health(source=<S>)`'s `tiers` under truncation over MCP. The untested cell is the *deeper* MCP tiers under a cap: `get_context_bundle(query, budget="connected"/"full", source=<S>)`'s `_Custody:_` headline renders its `fidelity` section via `render_custody_headline` (a **different** function than the `index` line's `render_fidelity_holdings`), yet all three fold over the *same* post-cap kept set — so under truncation the three MCP budget tiers must stay mutually equal (all bundle-kept, summing to `k`) *even as all three collectively diverge* from the scoped MCP audit (summing to the held count `> k`). Pin it: over a scoped mixed-fidelity library where `<S>`'s matches exceed the cap, `get_context_bundle(query, budget={index,connected,full}, source=<S>, limit=k)`'s fidelity counts are all equal to each other and all `≠ get_library_health(source=<S>)`'s `tiers`; lifting the cap (`limit` ≥ held count) reconverges all three to the H241/H235 scoped MCP equality. So the MCP budget ladder never disagrees *with itself* on holdings even under a cap, and never inflates the bundle-kept holdings to a source-wide claim — the M2/principle-3 honesty on the budget-ladder-cohesion axis, on the read pair an agent reaches over MCP. Test only (`tests/test_mcp.py`, beside the H235/H236/H241 twins). Implementation path: all three tiers fold `get_fidelity` over the same post-cap `items` in `build_context` (`index` via `render_fidelity_holdings(tiers, len(items))`, `connected`/`full` via `custody_headline(items, …)`) while `get_library_health(source=)` is the MCP twin of `doctor` folding over the whole scoped rows, so the cohesion-under-truncation is correct-by-construction; the slice pins the MCP twin of H240 and that the cap lift reconverges the ladder. Precondition: H236 (the leanest-line MCP boundary, **shipped this run**), H241 (the untruncated MCP cross-tier tie), H240 (the CLI ladder-under-truncation). | → cap 1, cap 2, cap 10 |
...
The next lead slot is **H242** (the topmost un-started row), now that **H241 shipped
this run** — the *cross-tier MCP convergence* under scope: the scoped `index` `_Fidelity:_` counts ≡ the
scoped `connected`/`full` `_Custody:_` headline's `fidelity` section, all read over MCP (the MCP twin of
H213's deeper-tier tie), the deeper tiers' drift verdict still withheld from the `index` line. H242 is the
*MCP twin of H240* (and the truncated boundary of H241) — the whole MCP budget ladder stays mutually
equal under truncation while *together* diverging from `get_library_health(source=<S>)`, reconverging to
the H241/H235 scoped MCP equality when the cap lifts, completing the leanest/cross-tier ×
untruncated/truncated × CLI/MCP fidelity-convergence matrix, H243 is the
...
and H240, scoped, leave between them), and H247 (appended this run)
...
**This run shipped H241** — the *cross-tier MCP convergence* under scope, the MCP twin of H213's
