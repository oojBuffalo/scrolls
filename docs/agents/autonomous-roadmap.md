...
audit is already deterministic — ordered folds, no wall-clock). **H376 is next** (the `scrolls context`
bundle determinism sibling); H377–H380 are queued behind it, and **H381** (the `get_library_health` MCP
whole-payload determinism twin of H375) was appended this run to keep the buffer ≥6.
...
ingest-transport settle axis), and **H375 shipped this run** (the `scrolls doctor` whole-report
determinism guard, the eighth pivot cell — the audit trust-root axis); the H376 context-bundle
determinism cell is next, and H377–H381 are queued behind it.
...
— take the next one whose preconditions are met (H376 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H376 | **`scrolls context` bundle determinism guard — pins that the model-facing context bundle (an agent's primary read) is a *reproducible artifact*: the same library + query at the same `--budget` twice yields byte-identical output, same-process AND across a `PYTHONHASHSEED` subprocess pair. The ninth **forward-hardening cell** — the *agent-read-surface* sibling of H368's `export bundle` determinism (a distinct code path: `context.py` builds the connected/excerpt sections from search + relatedness folds the export bundle never runs); deliberately *not* another content-identity guard. → cap 10.** H366 pinned the budget tiers are *nested* (Best-Match set identical, Connected byte-identical between `connected`/`full`) but never that one tier is *reproducible run-to-run*: a set-iteration leak in the connected-neighbours or excerpt-selection fold would pass H366's per-tier presence tests yet make two `scrolls context db --budget full` reads of one library disagree, so an agent caching a bundle and re-fetching sees spurious churn. Add a section to `tests/test_context.py`: build a multi-match library (the H366 fixture — three keyword matches + a linked-but-unmatched paper, so connected + excerpts are non-vacuous), run `scrolls context <q> --budget full` twice in-process, assert byte-identical, then run it in two subprocesses under different `PYTHONHASHSEED`s and assert identical stdout. **Decisive choice:** test at `--budget full` (every tier-gated section — Best Match, Connected, Excerpts, the `_Custody:_`/per-source lines — is exercised) and include the cross-seed pair, the order-leak catcher a same-process pass misses. **Sabotage check:** folding the connected-neighbours section over a `set` must fail the cross-seed byte-identity while H366's nesting tests stay green. Test-only, no production change (the bundle is already deterministic — ordered search + relatedness folds, no wall-clock). Precondition: `scrolls context`/`build_context_bundle`, the H366 budget-tier fixture + nesting guard, the H363 subprocess-determinism harness. | → cap 10 |
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  **4196 passed**), and **H374 shipped** (`import bundle` re-import idempotency — a re-import of
  the same bundle is a true no-op on rows + `scrolls/` disk bytes + the conflict ledger; two
  sabotages bite; suite **4197 passed**). Remaining, un-started: **H375** (`doctor --json`
  whole-report determinism, the audit-trust-root) — next — then **H376–H379**
  (context/maintain-trend determinism + the HTML-bundle / `export items` JSONL siblings), with
  **H380** (`import events` re-import idempotency) appended this run to keep the buffer full.
  Close a de-prioritized guard (H244–H249) only if nothing better is ready. The 2026-06-25 full
  re-derivation still spans this window; the next once-per-24h full re-derivation is **due
