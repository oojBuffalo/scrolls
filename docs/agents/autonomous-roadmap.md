...
determinism + restore round-trip guard); **H386** (the `import archive` re-import idempotency guard) is next,
and H387–H391 are queued behind it (the latest, **H391** — the MCP `get_archived` whole-payload determinism +
CLI-convergence guard — appended this run to keep the buffer ≥6).
...
*only* the byte-identity). The MCP-surface recovery-read twin **H391** (the MCP `get_archived` whole-payload
determinism + CLI-convergence guard) was appended this run to keep the buffer ≥6; **H386** (the `import
archive` re-import idempotency guard) is next, and H387–H391 are queued behind it.
...
— take the next one whose preconditions are met (H386 next):

| Slot | Intended slice | Maps to |
| --- | --- | --- |
| H386 | **`import archive` re-import idempotency guard — pins that re-importing the *same* prior-content archive backup (`scrolls export archive` JSONL, ADR 0106/H280) into a library that already holds those priors is a true no-op: the `item_archive` row count is unchanged across two passes, and every item's `archived_snapshots` / `latest_archived` recovery read is identical. The nineteenth **forward-hardening cell** and the *third* member of the ingest-idempotency family — after H374 (`import bundle`, items transport, `merge_item`'s `INSERT OR IGNORE`) and H380 (this run, `import events`, the `_EVENT_IDENTITY` 5-tuple) — on the **archive** transport: `import_archive` content-dedups on `_ARCHIVE_IDENTITY = (item_id, prior_hash)` (items.py), a **distinct code path** from both siblings; deliberately *not* another content-identity guard. → cap 9.** H280/H285 pin the archive export→import *round-trip* recovers the prior, but the whole-*store* idempotency of a re-import into a populated archive is only *implied*: a regression that dropped the `_ARCHIVE_IDENTITY` dedup (or keyed it on the per-library autoincrement `id`, never exported) would pass the single-import recovery tests yet double every archived prior on the second pass, inflating `archived_snapshots` and any count that folds `item_archive`. Add a section to `tests/test_roundtrip.py` (or `tests/test_cli.py`): seed an item with ≥1 superseded prior (so the archive is non-vacuous — a multi-prior `--all` fold if available), `export archive` to a backup, `import archive` it twice into a fresh home, and assert the second pass reports `imported == 0`/`skipped == N`, the `item_archive` row count equals the first pass's, and `archived_snapshots(id)` is identical list-for-list across the two passes. **Decisive choice:** assert *both* the raw `item_archive` row count (catches a silent double-insert) and the per-item `archived_snapshots` list identity (catches a dedup that drops the wrong prior) — the H380 both-axes precedent (a "reports skipped" check is blind to a re-keyed dedup that both skips *and* re-inserts). **Sabotage check:** a dedup that reports `skipped` yet re-inserts anyway must pass the report axis and fail the row-count axis. Test-only, no production change (`import_archive` already dedups by content; this makes the archive-ingest settle contract regression-proof, completing the items/events/archive ingest-idempotency triptych). Precondition: `import_archive`/`_ARCHIVE_IDENTITY`/`archived_snapshots`/`latest_archived` (H280/H285), `scrolls export archive`/`import archive`, the `tests/test_roundtrip.py` harness. | → cap 9 |
...
- **Day 3 (2026-06-27 → 2026-06-28):** **Forward hardening/integration.** With the posture
  ...
  **both** the single-prior default *and* the `--all` multi-prior fold, the default ≡ the `--all` head, plus
  ...
  cross-seed, `archived_snapshots` is an `ORDER BY id DESC` fold; suite **4215 passed**).
  Remaining, un-started: **H386** (the `import archive` re-import idempotency guard) — next — then
  **H387–H391** (the MCP `get_link_graph`, `get_works`, and `get_maintenance_history(trend=True)`
  whole-payload determinism guards, the `export archive` whole-library JSONL determinism + round-trip
  guard, and the MCP `get_archived` whole-payload determinism + CLI-convergence guard appended this run to
  keep the buffer ≥6). Close a de-prioritized guard (H244–H249) only if nothing better is ready. The next
