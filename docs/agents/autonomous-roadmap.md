...
The next lead slot is **H299** (the archive-integrity alarm gets a cross-run delta/trend in
`maintain` — `_Archive: N (▲M since last run)._`, the H283 conflicts-trend analogue on the archive
axis; H298 surfaced the *current* count, H299 differences it across runs), then **H300** (scoped
`export archive --id` round-trips the recovery read-family identically, the H294 twin on the
per-item export path) — two fresh readable-surface/integration slots above the 6 de-prioritized
guards (H299, H300, H244–H249 = **8 un-started work slots — ≥6, buffer healthy, no append**).
