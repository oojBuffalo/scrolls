...
def render_content_duplicates(
    items: list[ScrollItem] | None, db_path: Path | None = None
) -> list[str]:
    """The readable ``_Duplicates:_`` line for byte-identical holdings — the shared
    content-identity renderer (roadmap H331, whole-library generalization H334).

    Folds the *single divergence-truth source* `content_duplicate_groups` (H325) over the
    chosen item set and renders it via the *same* `duplicates_headline` the scheduled
    `maintain` summary uses — **point-in-time, no trend clause** (`change` defaults to
    ``None``: a briefing/landing page carries no cross-run delta, the
    `render_archive_integrity` posture) — so the shareable `export bundle` (H319's
    `_Archive:_`-briefing analogue on the content-identity axis), the agent `context`
    briefing, and the compiled landing `library/index.md` (H334) emit a byte-identical
    line that converges with `doctor`'s `custody.content_duplicates` JSON audit and the
    `maintain` headline for the same item set by construction. The
    `render_archive_integrity`/`render_at_risk_works` sibling on the content-identity axis
    (one fold, one renderer).

    ``items`` chooses the audit scope (the `render_archive_integrity` H321 precedent):

    - a **list of items** — the briefing's in-scope set (the bundle/context scope, the
      ``_Conflicts:_``/``_Archive:_`` precedent), so a content group split across the
      scope boundary reads only its in-scope members — a redundancy fact about the
      holdings the briefing actually carries, not the whole library (a documented scope
      divergence, like `doctor`'s whole-library-only `--source` skip). The caller passes
      the **uncollapsed** matched set (bundle's gathered `items`, `context`'s
      `scope_items`): content identity is a *relational* fact across distinct ids — two
      byte-identical representations of one work (a preprint mirrored into its DOI
      capture, the H329 case) are a content duplicate, but a work-collapse folds them
      into one canonical id and hides the group, so this patterns with
      `render_at_risk_works` (uncollapsed), not with the per-item
      `_Conflicts:_`/`_Archive:_` lines (collapsed);
    - ``None`` — the **whole-library** holdings (``list_items(db_path)``, exactly the set
      `doctor`'s `custody.content_duplicates` folds), the view the compiled landing
      `index.md` takes (H334: a content group spans sources, so the alarm is
      non-source-attributable and whole-library, like the archive/at-risk `index.md`
      lines — *not* in-scope); needs ``db_path``;
    - an **empty list** — the honest in-scope-of-nothing audit (no holdings, no groups),
      distinguished from ``None`` so an empty *scope* never loads the whole library.
    ...
    if items is None:
        if db_path is None:
            raise ValueError(
                "render_content_duplicates(None) needs a db_path for the "
                "whole-library fold"
            )
