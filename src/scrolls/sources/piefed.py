...
# bracket; trailing punctuation a prose scan over-captures is trimmed (Lemmy's
# `_content_links`, ADR 0052).
...
def _content_links(body: Any) -> list[str]:
    ...
    The body is Markdown, so an outbound URL is read straight from it (Lemmy's URL
    scan, ADR 0052): a post pointing at an arXiv paper or github repo wires to it
    through `scrolls related`/`graph` (the cross-source edges, ADR 0044).
    """
