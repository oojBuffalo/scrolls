...
def _format_comments(comments: list[Any] | None) -> str:
    ...
    an answer (ADR 0033) and assembled by the shared thread renderer (ADR 0093).
    The threading `path` is not rendered into the text but survives in `raw_text`
    for a future nested render (Lobsters' `depth`).
    """
